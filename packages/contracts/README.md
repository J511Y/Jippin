...
| 파일 | 컨트랙트 | 근거 |
|---|---|---|
| `common-judgment-schema.schema.json` | `CommonJudgmentSchema` (**1.4.0** — `vlm_supplement.region_assessments` 추가 + `confidence`/`is_floorplan`/`judgment_hints` 계약 정합화, `WindowForm` 을 공용 `$defs` 로 승격해 생성 바인딩 심볼 보존; 1.3.0 `vlm_supplement` null 허용 명문화) | SDD §5.2 핵심 컨트랙트. 선택 벽 종합 판단·창호 경계 자동 반영·오버레이 재제공(#region-assessments, #overlay-reshow). 도면 재제출 VLM 신선도(PR #185) |
