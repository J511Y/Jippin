    ...
    if session.get("selected_floorplan_asset_id"):
        ...
        if analyzed and (walls or window_count):
            ...
            lines.append(
                ...
                f"진행**하고 도면을 다시 요청하지 말 것. 주소는 건축물대장 조회·룰 평가의 "
                f"보조 입력일 뿐이라, 도면이 있으면 주소가 없어도 분석/검토를 이어갈 수 있다."
            )
            if unknown:
                lines.append(
            ...
            lines.append(
                ...
                "평면도**(벽이 선명히 보이는 도면) 업로드를 요청할 것. 벽이 선 하나로만 "
                "그려진 부동산 앱 평면도·단지 배치도·흐릿한 사진이었을 가능성이 크니, "
                "reason 에 그 점을 생활어로 짚고 '카드의 예시처럼 벽 두께가 그려진 "
                "**단위세대 평면도**를 전체가 잘리지 않게 선명하게 올려 달라'는 뜻을 담을 "
                "것. 새 도면이 첨부되면 segment_floorplan 으로 다시 분석한다(새 도면이 "
                "기존 도면을 대체한다)."
            )
        else:
            lines.append(
