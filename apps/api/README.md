...
- **에이전트/LLM**: `AGENT_ENABLED`, `AGENT_MODEL` (기본 `openai:gpt-5.6-luna`), `VLM_MODEL` (도면 VLM 전용 override, 미설정 시 `AGENT_MODEL` 상속), `OPENAI_API_KEY`, `LANGCHAIN_TRACING_V2`, `LANGSMITH_*`. 에이전트 체크포인터는 **direct `:5432` `DATABASE_URL` 필수** (`config.py::_validate_agent_checkpointer_url` 이 부팅 차단)
