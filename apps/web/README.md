...
CMP-618 이후 모바일 퍼스트 IA 가 `app/(app)` route group 으로 봉인되어 있습니다. 헤더 메뉴는 검토 / 가격이며(상담 항목은 CMP-DIRECT 로 제거 — 마이페이지로 이동), 우측에 로그인/마이페이지 진입을 둡니다. `/leads/new` 는 메뉴에 직접 들어가지 않고 CTA 로만 진입합니다. `/auth/*` 는 shell 바깥 (root layout) 에 위치해 OAuth 콜백/익명 가입 흐름과 충돌하지 않습니다.
