...
describe('useAgentStream 전송 중 409 복구', () => {
  ...
  it('409 활성 런이 awaiting_input 이면 같은 메시지를 /resume 로 보낸다', async () => {
    const calls = installFetch((method, url, n) => {
      if (url.startsWith(`${BASE}/messages`)) return json({ messages: [] });
      if (url === `${BASE}/runs/active`) return notActive();
...
describe('useAgentStream 리뷰 회귀 5라운드 (Codex PR #204)', () => {
  it('병합 조회는 마지막으로 아는 서버 메시지 뒤(after 커서)만 받는다', async () => {
    const calls = installFetch((method, url, n) => {
      if (url.startsWith(`${BASE}/messages`)) {
        if (!url.includes('after=')) {
          return json({
            messages: [
              { id: 'u0', role: 'user', content: '처음', ui_components: [] },
              { id: 'a0', role: 'assistant', content: '처음 답', ui_components: [] }
            ]
          });
        }
        return json({
          messages: [{ id: 'a-other', role: 'assistant', content: '다른 탭 답', ui_components: [] }]
        });
      }
      if (url === `${BASE}/runs/active`) return notActive();
      if (url === `${BASE}/runs` && method === 'POST') {
    ...
    expect(calls.some((c) => c.url === `${BASE}/messages?after=a0`)).toBe(true);
    expect(result.current.messages.map((m) => m.content)).toEqual([
      '처음',
      '처음 답',
      '다른 탭 답',
  ...
  it('reconnect drain 이 스트림 내 NOT_RESUMABLE 로 지면 이긴 탭 기준으로 정리한 뒤 새 입력을 보낸다', async () => {
    const calls = installFetch((method, url, n) => {
      if (url.startsWith(`${BASE}/messages`)) {
        return json({
          messages:
            n === 1 ? [] : [{ id: 'a-win', role: 'assistant', content: '이긴 탭 답', ui_components: [] }]
        });
      }
      if (url === `${BASE}/runs/active`) {
        return json({ id: 'run-i', status: n === 1 ? 'interrupted' : 'running' });
      }
      if (url === `${BASE}/runs/run-i/resume` && method === 'POST') {
        return sse(
          [
      ...
      if (url === `${BASE}/runs/run-i`) return json({ id: 'run-i', status: n < 3 ? 'running' : 'succeeded' });
      if (url === `${BASE}/runs` && method === 'POST') return reply('m-new', '새 답', 'run-n');
      return unexpected(method, url);
    });
    ...
    expect(result.current.error).toBeNull();
    expect(result.current.status).toBe('done');
    expect(result.current.messages.map((m) => m.content)).toEqual(['이긴 탭 답', '새 질문', '새 답']);
  });

  it('resume 이 HTTP 409 로 지고 그 런이 이미 끝났으면 이긴 탭의 턴을 합친 뒤 새 런으로 보낸다', async () => {
    const calls = installFetch((method, url, n) => {
      if (url.startsWith(`${BASE}/messages`)) {
        return json({
          messages:
            n === 1
              ? []
              : [
                  { id: 'u-win', role: 'user', content: '이긴 탭 답변', ui_components: [] },
                  { id: 'a-win', role: 'assistant', content: '이긴 탭 답', ui_components: [] }
                ]
        });
      }
      if (url === `${BASE}/runs/active`) return json({ id: 'run-w', status: 'awaiting_input' });
      if (url === `${BASE}/runs/run-w/resume` && method === 'POST') {
        return json({ error: { code: 'AGENT_RUN_NOT_RESUMABLE', message: 'x' } }, 409);
      }
      if (url === `${BASE}/runs/run-w`) return json({ id: 'run-w', status: 'succeeded' });
      if (url === `${BASE}/runs` && method === 'POST') return reply('m-new', '새 답', 'run-n');
      return unexpected(method, url);
    });
    ...
    await waitFor(() => expect(calls.some((c) => c.url === `${BASE}/runs/active`)).toBe(true));
    await act(async () => {
      await result.current.send('내 답변');
    });
  ...
  it('전송에 실패한 옛 낙관적 버블은 내용 대응에 쓰지 않는다(다른 탭의 같은 내용 새 메시지 보존)', async () => {
    const calls = installFetch((method, url, n) => {
      if (url.startsWith(`${BASE}/messages`)) {
        return json({
          messages:
      ...
      if (url === `${BASE}/runs/active`) return notActive();
      if (url === `${BASE}/runs` && method === 'POST') {
        if (n === 1) return json({ error: { code: 'INTERNAL_ERROR', message: 'boom' } }, 500);
        if (n === 2) {
          return json(
            { error: { code: 'AGENT_RUN_ALREADY_ACTIVE' }, detail: { active_run_id: 'run-a', status: 'running' } },
            409
    ...
    await waitFor(() => expect(calls.some((c) => c.url === `${BASE}/runs/active`)).toBe(true));
    await act(async () => {
      await result.current.send('네'); // 500 → 실패한 버블(미수락)이 남는다
    });
