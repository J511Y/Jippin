 ...
 * 있으므로 절대 게이트로 쓰지 않는다.
 *
 * 이미 존재하는 이메일이면 계정을 새로 만들지 않고 app_metadata 만 갱신한다
 * (비밀번호는 건드리지 않음). 따라서 재실행해도 안전하다.
 *
 * 사용법:
 *   SUPABASE_URL=https://<ref>.supabase.co \
 *   SUPABASE_SERVICE_ROLE_KEY=<service_role> \
 *   ADMIN_INITIAL_PASSWORD=<password> \
 *   node tools/admin/create-admin-users.mjs admin@example.com user2@example.com
 *
