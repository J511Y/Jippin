...
| `brand.gridMinor` | `rgba(21,59,92,.05)` | 제도 격자 마이너 라인(32px 간격) | body 배경 격자, 도면 모티프 표면 |
| `brand.gridMajor` | `rgba(21,59,92,.085)` | 제도 격자 메이저 라인(160px 간격) | body 배경 격자 |
...
## 4.2 도면 오버레이 토큰 (Floorplan overlay) — `--floorplan-*`

> 도면 위에 AI 분석 영역을 덧그리는 OVERLAY 모듈 전용 색이다. 구현 정본은
> `apps/web/app/globals.css` 의 `--floorplan-*` CSS 변수이고, 리포트 SVG
> (`apps/api/src/services/report_overlay.py`)가 **같은 값**을 하드코딩으로 맞춘다 —
> 웹 카드와 PDF 리포트에서 같은 벽이 다른 색으로 보이면 안 된다.

**벽 3종의 색은 모델의 벽 어휘와 1:1로 묶인다** (ADR-0010). 어휘가 바뀌면 이 표를 먼저 고친다.

| 토큰 | 값 | 모델 클래스 | 역할 |
|---|---|---|---|
| `--floorplan-wall-load` | `status.danger` | `wall_reinforced_concrete` | 내력벽 후보 — **선택 불가**(철거 대상이 될 수 없음) |
| `--floorplan-wall-nonload` | `status.success` | `wall_nonbearing` | 비내력벽 후보 — 선택 가능. 전문가가 **확정한** 비내력 패턴 |
| `--floorplan-wall-uncertain` | `#8A929B` | `wall_other`(+ 구 `wall_unknown`) | **미확정 벽** — 도면만으로 내력/비내력을 가르지 못해 판단을 보류한 벽. 선택은 가능하되 선택 시 '추가 확인 필요' 흐름을 탄다 |
| `--floorplan-door` | `#3F6F9C` | `door` | 문 |
| `--floorplan-window` | `#3C8896` | `window` | 창호 — 거실-발코니 경계 창 철거 검토용으로 선택 가능 |
| `--floorplan-space-*` | 별도 팔레트 | `space_*` 13종 | 공간(거실·주방·발코니…). brand 청록 톤에 맞춰 채도를 낮춘다 — 형광 원색 금지([`BRAND.md §3`](BRAND.md)) |

### 4.2.1 오버레이 사용 규칙

- **회색(미확정)을 초록(비내력)으로 승격하지 않는다.** 이 구분이 오버레이 색의 존재 이유다 —
  도면만으로 판단을 보류한 벽에 '철거 가능' 인상을 주면 안 된다. 과거 어휘(v3)에서 두 의미가
  뒤바뀐 채 렌더되던 사고가 있었다(ADR-0010 §배경).
- **색 단독 금지**(§6.2): "선택 가능/불가"를 색만이 아니라 **선 모양**(선택 가능 = 점선 →
  선택됨 = 흰 실선)과 **범례 라벨**로 함께 인코딩한다. 적록색맹 사용자가 내력(빨강)/비내력
  (초록)을 색만으로 구분하게 두지 않는다.
- 어휘는 항상 **후보/추정/검토 필요**로 쓴다. '철거 가능 확정'·'내력벽 확정'은 금지
  ([`TYPOGRAPHY.md`](TYPOGRAPHY.md) 안전 어휘).
