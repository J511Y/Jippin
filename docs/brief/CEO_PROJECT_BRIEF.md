...
| US-07 | 사용자로서, 모르는 정보(스프링클러·대피공간 등)는 전문용어 없이 생활어 질문으로 안내받고 싶다 | 부족 케이스 ≥95%가 보류·추가 확인. 동일 질문 한 세션 2회 초과 X. **2026-09-03 개정(운영자 결정)**: 철거 가부를 가르지 않는 항목(스프링클러·대피공간·경량칸막이·방화판)은 리포트 전 선질문 대신 **보수적 가정으로 즉시 판정 + 결과와 함께 생활어 '함께 챙길 것' 안내**(리포트에 미확인 caveat 첨부, 사용자가 답하면 재평가). 가부 전제 항목(벽 종류·창호 경계·층수)만 보류·추가 확인 대상 |
