...
- **2-way (method 기반·단계형)**: 전유부는 `address`(동·호 전까지) 1차 → `CF-03002`. CODEF 는 응답 `data.method`(예: `"hoNum"`)와 함께 **그 단계에 필요한 축만** 후보로 돌려준다 — 동 없는 집합건물은 `reqHoNumList` 만 오고 `reqAddrList`/`reqDongNumList` 는 아예 없을 수 있다(실측: 양천로 400-12). 따라서 세 축을 한꺼번에 요구하지 않고 **응답에 실제로 존재하는 축만** 처리한다: 단일후보는 자동선택, 동·호는 사용자 입력을 정규화해 **유일매칭**, 2차 응답이 또 `CF-03002`(단계형)면 다음 축으로 루프(`_drive_two_way`). 표제부는 `address`+`dong`을 1차에 직접 전달("입력 동으로 조회 가능하면 2-way 미발생").
  - **자동매칭 불가(0건/복수건)** 시에만 `needs_input(kind=dong_ho, field, options)` 폴백 — CODEF 후보(주소/동/호+면적)를 그대로 실어 보내 프론트가 드롭다운으로 제시하고, 사용자가 고른 `selection`(commHoNum 등 식별자)으로 재개한다(자유입력 재추정 금지 → 같은 호번호가 여러 동/면적에 있어도 정확히 특정). 보안문자(`reqSecureNo`) 발생 시 `kind=secure_no`.
