...
> **Supabase 세션 발급 봉인 (rev9 신설 — Codex P2 line 350)**: `auth.admin.createUser()` 는 새 user row 만 만들 뿐 **브라우저에 Supabase 세션 (access/refresh token) 을 발급하지 않는다.** 본 ADR 은 자체 JWT 발급 코드를 폐기했으므로, fallback callback 이 위 (a)~(d) 를 마친 직후 사용자를 `require_permanent_user` 라우트로 보낼 수 있도록 **server-side 세션 발급 경로** 를 봉인한다. 1순위: **`supabase.auth.admin.generateLink({ type: 'magiclink', email: <Naver verified email>, options: { redirectTo: FRONTEND_AUTH_SUCCESS_URL } })`** 로 일회용 sign-in URL 을 받아 callback 응답에서 사용자를 그 URL 로 redirect → 사용자 브라우저가 Supabase 측에서 자동으로 access/refresh token 을 발급받아 새 permanent session 으로 web 으로 복귀. 2순위 (Supabase docs 가 별도 노출 시): Auth Admin 의 직접 session 발급 API (`createSession` 등이 추가되는 경우 그 supported 경로). **자체 `auth.users` row 에 직접 token 을 박지 않는다.** 자식 §5.3 #5 의 fallback PR 이 magiclink redirect 흐름의 사용자 UX (자동 redirect / 명시 click) + email 미검증 케이스 (Naver email 미제공 시) 의 fallback 을 봉인.
