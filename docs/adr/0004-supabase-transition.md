...
**단일 fallback path (rev8) — FK 재할당 + anon row 삭제**: callback 핸들러가 (a) Naver userinfo + Naver verified email 로 Supabase 측에 새 permanent user 를 생성 (`supabase.auth.admin.createUser({ email, email_confirm: true, user_metadata: { provider: 'custom:naver', naver_sub: <Naver subject id>, naver_id_token_iss: <Naver issuer>, naver_id_token_aud: <Naver audience> } })` — Auth Admin API 의 supported 경로, **rev9 정정 (Codex P2 line 350 보조) — issuer/audience/subject 매핑 user_metadata 봉인** 으로 향후 audit 가능), (b) **anon `auth.users.id` 에 묶여 있던 모든 도메인 FK** (도면·리포트·약관 동의 intent 등 `public.*` 테이블의 `user_id` 컬럼) 를 새 permanent user.id 로 **단일 트랜잭션** 안에서 일괄 재할당, (c) 우리 측 영구화 로그 (`public.user_migration_audit` — 신설) 에 `(old_anon_id, new_user_id, migrated_at, source='naver_custom_fallback', naver_sub, naver_iss, naver_aud)` row 기록, (d) Supabase Auth Admin API 의 `deleteUser(anonId)` 로 anon `auth.users` row 삭제 (CASCADE 로 `auth.identities` 의 anon row 도 자동 정리).
...
**`auth` 스키마에는 우리가 직접 쓰지 않는다 — 모든 변경은 Auth Admin API 또는 우리 측 `public.*` 테이블 경계 안에서만 일어난다.** 자식 §5.3 #5 (Naver PoC) 의 fallback PR 이 단일 트랜잭션 격리 수준·rollback 시 안전성·FK 컬럼 매핑 카탈로그를 봉인.

**대안 — application table boundary (rev8 supplemental)**: 위 createUser+migrate+delete 패턴이 운영상 부담스러우면, anon user 와 new permanent user 를 둘 다 `auth.users` 에 유지하면서 우리 측 `public.user_aliases` (신설) 에 `(canonical_user_id = new_permanent_id, alias_user_id = anon_id, source='naver_custom_fallback')` 매핑 row 를 두고, 모든 도메인 조회 가 `canonical_user_id` 를 사용하도록 query layer 를 정리. 이 경우 `auth.identities` 는 손대지 않으며 anon row 는 정리 잡 (§4.5.3 자식 §5.3 #10) 에 위임.

두 패턴 모두 anonymous 도면·리포트의 ownership 을 끊지 않으며, **`auth.identities` / `auth.users.is_anonymous` 등 Supabase 관리 컬럼을 우리가 직접 수정하지 않는다**. 자식 §5.3 #5 (Naver PoC) 가 PoC 실패 분기를 PR 로 만들 때 두 패턴 중 어느 쪽을 채택할지 봉인.
