...
| **OAuth state store** | Redis `oauth_state:*` / `pending_signup:*` TTL ≤10분 | **Supabase 측이 PKCE state · 약관 동의 → 가입 완료 사이 상태를 관리. ⚠ rev8 정정 (Codex P2 line 569)**: Naver Custom OAuth PoC 가 실패해 자체 OAuth fallback (§2.5) 으로 전환되면 Redis `naver_oauth_state:<state> = anon_user_id` (TTL ≤ 10분) 가 **fallback 의 anonymous ownership 보존을 위한 필수 정본**. 따라서 Redis `oauth_state:*` key prefix 의 폐기는 **PoC 성공 + Custom OAuth 채택 확정 시점에만**. Naver 분기에 한해 ADR-0003 §2.2 state store 정본 보존. | ADR-0003 §2.2 supersede — 자체 Redis state store 는 Naver fallback 조건부 보존. Google/Kakao 의 state 는 Supabase 가 관리. Redis 자체는 채팅·세션 캐시 + Naver fallback state store 로 잔존. |
...
`public.users` row 는 Supabase `auth.users` 생성과 같은 운영 경계에서 반드시 생성한다. Google/Naver manual `linkIdentity()` 전환, Kakao Sync 전환, Naver Custom OAuth PoC 성공 전환 모두 API 트랜잭션 또는 Database Webhook/reconciliation 경로가 `public.users(id, role, status)` 를 보장해야 하며, FastAPI 가 Supabase JWT 를 검증했더라도 `public.users` row 가 없거나 `status != 'active'` 이면 영구 사용자로 취급하지 않는다. `role` 은 현행 `/admin/*` RBAC 와 `apps/api/src/services/auth.py` 의 `User.role` 조회를 유지하기 위한 프로필 필드이며 Supabase `app_metadata` 로 이전하려면 별도 ADR 이 필요하다.
...
| (신규) `auth.users` AFTER UPDATE Postgres trigger (`SECURITY DEFINER`) — **kakao_sync 정합 정본 아님 (rev8 정정)** | **rev8 정정 (Codex P2 line 258) — Postgres trigger 는 kakao_sync 검증 경로가 아니다.** Kakao 가 노출하는 “required Sync term tags 동의 결과” 는 **`identity_data` (userinfo) 에 들어오지 않는다** — Kakao 는 별도의 [`/v2/user/service_terms`](https://developers.kakao.com/docs/latest/en/kakaologin/rest-api#service-terms-retrieve-consent-details) REST API 로만 노출하며, 이 호출은 access token 또는 service app admin key 가 필요한 server-side 호출이다. Postgres trigger 는 외부 HTTP 호출이 불가하므로 (`pg_net` extension 사용 시에도 신뢰성·인증 측면에서 부적합) **kakao_sync 검증 경로에서 제외**. provider=kakao 만 보고 trigger 가 `terms_consents (source='kakao_sync')` 를 자동 insert 하면 (a) 실제 Sync 약관 미동의 사용자에게 잘못된 audit row 생성, (b) 또는 audit row 없이 통과하는 잘못된 invariant 둘 중 하나가 발생. 따라서 본 ADR 은 kakao_sync 정합을 **server-side reconciliation 경로로 단일화** 한다 (아래 행). `auth.users` AFTER UPDATE trigger 자체는 **internal_signup 정합에만** 사용 (Google / `custom:naver` intent → consent promote). |
| (신규) `POST /api/auth/kakao-sync-consent-ingest` + Supabase Database Webhook **+ Kakao `/v2/user/service_terms` 검증** | **신설 (1순위 — kakao_sync 단일 정본, rev8 승격)**: Supabase Database Webhook 이 `auth.users` UPDATE (is_anonymous: true→false) 이벤트를 우리 FastAPI 로 push → 핸들러가 ① `auth.identities` provider='kakao' 확인 + ② **Kakao `/v2/user/service_terms` REST API 호출 (Kakao access token 또는 service app admin key 사용) 로 사용자가 동의한 service term tags 셋을 가져옴** + ③ `public.policy.kakao_sync_required_term_tags` 와 **모두 매칭** 되는지 검증 + ④ 매칭 시 `terms_consents (source='kakao_sync')` insert, 미매칭 시 `consent_promotion_audit` 에 “Kakao Sync 약관 미충족 — provider=kakao 이나 required_term_tags 누락” row 별도 기록. ⑤ webhook 실패 시 우리 측 reconciliation 잡 (`apscheduler` 기반, 분당 1회) 이 미처리 사용자를 스캔하여 동일 절차 재시도. **이 경로 외 다른 kakao_sync 정합 path 는 허용하지 않는다.** 자식 §5.3 #3 (DB trigger PR) 의 kakao_sync 부분은 본 webhook 핸들러 PR (자식 §5.3 #5 의 Kakao 분기) 로 이동. |
| (rev8 정리) | ~~Database Webhook 2순위 행 — rev8 에서 위 1순위 행으로 통합되고 server-side Kakao API 검증 의무가 추가됨. 별도 “대체 경로” 행 폐기.~~ |
...
DATABASE_URL=                       # ⚠ rev9 정정 (Codex P2 line 276) — Alembic 마이그레이션 전용 (direct connection, port 5432).
                                    # Alembic 은 DDL / prepared statement 호환성을 위해 pooler 를 사용하면 안 됨 (`apps/api/src/db.py:30-56` + apps/api/README 정본).
DATABASE_POOL_URL=                  # ⚠ rev9 정정 (Codex P2 line 276) — FastAPI request-path 쿼리 전용 (Supabase pooler, port 6543).
                                    # Supabase Pooler (Supavisor) 는 transaction mode + prepared statement OFF 권장.

# 폐기 (ADR-0003 환경변수)
KAKAO_REST_API_KEY=                 # → Supabase Dashboard / Auth Providers 콘솔로 이전. **단 Kakao Sync reconciliation 경로용 server admin key 는 별도 보존 (아래 KAKAO_SERVICE_APP_ADMIN_KEY).**
KAKAO_CLIENT_SECRET=                # → Supabase Dashboard 로 이전.
KAKAO_REDIRECT_URI=                 # → Supabase callback URL 로 이전.
KAKAO_SERVICE_APP_ADMIN_KEY=        # ⚠ rev9 신설 (Codex P2 line 281) — Kakao Sync reconciliation 경로의 `/v2/user/service_terms` 호출용. user access token 이 webhook 컨텍스트에 없으므로 Service App Admin Key 사용. 운영 시크릿 매니저 정본.
GOOGLE_OAUTH_CLIENT_ID=             # → Supabase Dashboard 로 이전.
GOOGLE_OAUTH_CLIENT_SECRET=         # → Supabase Dashboard 로 이전.
GOOGLE_OAUTH_REDIRECT_URI=          # → Supabase callback URL 로 이전.
...
FRONTEND_AUTH_SUCCESS_URL=          # ⚠ rev9 정정 (Codex P2 line 297) — Google/Kakao 흐름은 Supabase redirect URL 로 일원화하되, Naver Custom OAuth PoC 실패 fallback (§2.5) 의 자체 callback (`/auth/callback/naver`) 이 사용자에게 web 으로 돌려보낼 return target 으로 본 2종 변수가 필요. 따라서 폐기는 **PoC 성공 + Custom OAuth 채택 확정 시점에만**.
FRONTEND_AUTH_FAILURE_URL=          # 동일 — Naver fallback 채택 시 보존.
```
...
**PoC 실패 시 처리.** Naver 만 자체 OAuth 코드 (ADR-0003 §2.2 의 `/auth/{provider}/start` + `/auth/callback/{provider}`) 패턴으로 영구 전환하는 fallback 은 본 ADR 에서 채택하지 않는다. Supabase 가 sealed `custom:naver` identity 와 정합되는 세션을 발급할 supported path 를 제공하지 않는 한, PoC 실패 시 익명 사용자는 Google/Kakao 전환 권유 UX 또는 익명 데이터 유지 + PoC 성공 후 재시도 흐름으로만 안내한다. **anonymous 세션 ownership 보존 봉인 (Codex P2 line 329 + P2 line 343 처리, rev5+rev7)**: 사용자가 이미 익명 세션 (`auth.users.is_anonymous=true`) 에서 도면·리포트를 만든 상태로 Naver 로 전환하려는 경우에도, **`auth.admin.createUser()` 로 새 user 를 만들면 안 된다** — 새 user 의 id 가 익명 도면·리포트 FK 와 불일치하여 ownership 이 끊기며, 브라우저 세션도 sealed provider 로 발급되지 않는다.
...
이후 callback 핸들러는 state 로 식별한 anon id 에 대해 다음 패턴으로 anonymous ownership 을 보존한다. **rev8 정정 (Codex P2 line 347) — `auth.identities` 직접 INSERT 금지**: Supabase 의 [auth 스키마는 managed](https://supabase.com/docs/guides/auth/managing-user-data) 이며 컬럼·인덱스·제약·내부 객체가 임의 시점에 변경될 수 있다. `auth.identities` 에 우리가 직접 INSERT 하면 (a) GoTrue 가 issuance·session 관리에서 해당 row 를 정상 처리하지 않을 위험, (b) 향후 Supabase 측 스키마 변경으로 INSERT 가 깨질 위험, (c) Supabase Auth Hook 카탈로그 외 우회 경로로 운영 가시성을 잃을 위험이 누적된다. 따라서 본 ADR 은 **`auth.identities` 직접 INSERT 패턴 (rev5~rev7 의 1순위 / `fn_link_naver_identity` 포함) 을 일괄 폐기** 하고, fallback 의 정합 path 를 다음 단일 경로로 봉인한다.

**보조 안전망 (PoC 성공 후 일시 실패 복구 전용) — FK 재할당 + anon row 삭제**: 아래 createUser/FK 재할당/deleteUser 패턴은 **Naver Custom OAuth PoC 가 성공해 `linkIdentity({ provider: 'custom:naver' })` 가 정본 경로로 봉인된 뒤**, 운영 중 일시 장애로 정상 전환이 중단된 사용자를 수동 복구할 때만 검토한다. PoC 실패 시 fallback 으로 사용하지 않는다. 복구 핸들러가 필요해지는 경우에도 (a) Naver userinfo + Naver verified email 로 Supabase 측에 새 permanent user 를 생성 (`supabase.auth.admin.createUser({ email, email_confirm: true, user_metadata: { provider: 'custom:naver', naver_sub: <Naver subject id>, naver_id_token_iss: <Naver issuer>, naver_id_token_aud: <Naver audience> } })` — Auth Admin API 의 supported 경로, **rev9 정정 (Codex P2 line 350 보조) — issuer/audience/subject 매핑 user_metadata 봉인** 으로 향후 audit 가능), (b) **anon `auth.users.id` 에 묶여 있던 모든 도메인 FK** (도면·리포트·약관 동의 intent 등 `public.*` 테이블의 `user_id` 컬럼) 를 새 permanent user.id 로 **단일 트랜잭션** 안에서 일괄 재할당, (c) 우리 측 영구화 로그 (`public.user_migration_audit` — 신설) 에 `(old_anon_id, new_user_id, migrated_at, source='naver_custom_recovery', naver_sub, naver_iss, naver_aud)` row 기록, (d) Supabase Auth Admin API 의 `deleteUser(anonId)` 로 anon `auth.users` row 삭제 (CASCADE 로 `auth.identities` 의 anon row 도 자동 정리).

> **Supabase 세션 발급 봉인 — magiclink 패턴 폐기 (rev10 정정 — Codex P2 line 356, board 코멘트 `33cf7055`)**: rev9 가 일시적으로 봉인한 `admin.generateLink({ type: 'magiclink', email })` 패턴은 **본 ADR 에서 폐기** 한다. 이유: (i) magiclink 는 [Supabase email/passwordless sign-in 방식](https://supabase.com/docs/guides/auth/auth-magic-link) 이며, 발급되는 세션은 `auth.identities.provider='email'` 로 기록되어 **sealed `custom:naver` provider audit 경로와 정합되지 않는다**. (ii) 본 ADR §2.5 의 “provider 3종 (Google / Kakao / `custom:naver`) 정책” 밖의 **email-login path 를 우회적으로 여는 결과** 가 되어 ADR-0003 §2.2 의 provider ENUM 봉인 정책 + CEO 2026-06-01 Manual linking only 결정과 충돌한다. (iii) `auth.identities.provider='custom:naver'` row 가 결여된 상태로 영구화되므로 향후 `require_permanent_user` / RLS / 감사 로그가 사용자의 provider 를 식별할 수 없다.

> **결과 — Naver Custom OAuth PoC 성공이 Naver provider 지원의 hard 요건 (rev10 봉인)**: Supabase Auth 가 admin 측에서 “이미 만든 user 에게 임의의 sealed provider identity 와 정합되는 세션을 발급” 하는 supported API 를 노출하지 않는 한 (현재 Auth Admin SDK 카탈로그 기준 존재하지 않음), **익명 → 영구 Naver 전환을 fallback 경로 안에서 봉인할 수 있는 정합 path 가 없다**. 따라서 본 ADR 은 다음을 봉인한다.\n> 1. **Naver Custom OAuth PoC 성공 = MVP 의 Naver provider 지원 hard 요건.** PoC 가 §2.5 PoC 성공 기준을 통과해야 Naver 가 MVP scope 에 포함된다. 자식 §5.3 #5 (Naver PoC) 가 충족 못 하면 본 ADR 의 “Naver = Supabase Custom OAuth” 항목은 **CEO 결정 사항으로 escalation** (CMP-573 의 신규 request_confirmation).\n> 2. **PoC 실패 시 fallback 의 anonymous → permanent 전환 불가 명문화.** 사용자가 익명 세션에서 도면·리포트를 만든 뒤 “Naver 로 계속” 을 누른 경우, fallback 경로는 **세션 발급 정합을 만족시킬 수 없으므로** (a) 사용자에게 Google 또는 Kakao 로 전환 권유 UX 노출, 또는 (b) 익명 데이터 유지 + Naver PoC 가 성공한 후 사용자가 다시 와서 `linkIdentity({ provider: 'custom:naver' })` 정상 흐름 진행. (c) “Naver 만 자체 OAuth + 자체 JWT 발급” 의 ADR-0003 패턴으로 회귀하는 것은 본 ADR 의 “자체 JWT 발급 코드 폐기” 봉인을 무력화하므로 **금지** — 그 경로를 채택하려면 본 ADR 을 supersede 하는 신규 ADR (`docs/adr/0005-naver-without-supabase-custom-oauth.md` 또는 다음 번호) 발행이 선결.\n> 3. **`admin.createUser` + FK 재할당 + `deleteUser` 패턴도 PoC 실패 시점에는 사용 불가.** rev9 의 단일 fallback path 가 매력적이지만 session 발급 정합이 빠지면 “permanent user row 는 만들어졌으나 브라우저가 그 user 로 로그인할 supported path 가 없는” 상태가 만들어진다. magiclink 외에는 supported 경로가 없고 magiclink 는 정책 위반이므로, 본 패턴의 (a)~(d) 단계는 PoC 성공 시점의 **공식 `linkIdentity({ provider: 'custom:naver' })` 흐름의 보조 안전망** 으로만 의미를 갖는다 (예: PoC 성공 후 사용자가 anonymous → permanent 전환을 시도했는데 PoC 가 일시 실패한 경우 우리가 server-side 로 마무리). PoC 가 영구 실패하면 본 패턴 자체가 의미를 잃는다.\n>\n> **요약**: §2.5 의 “PoC 실패 시 fallback” 은 사용자 데이터 마이그레이션을 약속하지 않는다. fallback 의 의미는 “Naver MVP 지원 자체를 CEO 결정으로 escalation 한다 + 그 동안 사용자에게 Google/Kakao 전환 또는 익명 유지를 제안한다” 로 좁혀진다. 자식 §5.3 #5 (Naver PoC) 가 (1) PoC 성공 시: `linkIdentity({ provider: 'custom:naver' })` 정상 흐름 봉인, (2) PoC 실패 시: 사용자 UX (Google/Kakao 전환 권유) + 신규 ADR 큐잉을 모두 봉인.

**`auth` 스키마에는 우리가 직접 쓰지 않는다 — 모든 변경은 Auth Admin API 또는 우리 측 `public.*` 테이블 경계 안에서만 일어난다.** 자식 §5.3 #5 (Naver PoC) 의 복구 경로가 필요해지면 단일 트랜잭션 격리 수준·rollback 시 안전성·FK 컬럼 매핑 카탈로그를 별도 PR 에서 봉인한다.

**대안 — application table boundary (rev8 supplemental, PoC 성공 후 복구 전용)**: 위 createUser+migrate+delete 패턴이 운영상 부담스러우면, anon user 와 new permanent user 를 둘 다 `auth.users` 에 유지하면서 우리 측 `public.user_aliases` (신설) 에 `(canonical_user_id = new_permanent_id, alias_user_id = anon_id, source='naver_custom_recovery')` 매핑 row 를 두고, 모든 도메인 조회 가 `canonical_user_id` 를 사용하도록 query layer 를 정리. 이 경우 `auth.identities` 는 손대지 않으며 anon row 는 정리 잡 (§4.5.3 자식 §5.3 #10) 에 위임. 이 역시 PoC 실패 시 사용자 전환 fallback 이 아니다.

두 패턴 모두 anonymous 도면·리포트의 ownership 을 끊지 않으며, **`auth.identities` / `auth.users.is_anonymous` 등 Supabase 관리 컬럼을 우리가 직접 수정하지 않는다**. 자식 §5.3 #5 (Naver PoC) 는 PoC 성공 시 이 복구 경로의 필요 여부를 판단하고, PoC 실패 시에는 신규 ADR 또는 Google/Kakao 전환 권유 UX 로 분리한다.
...
| `require_permanent_user` | JWT 검증 + **`is_anonymous=false` 강제 + `public.users.status='active'` 강제 + 현재 정본 약관에 대한 `public.terms_consents` 매칭 row 존재 확인** (rev7 정정 — Codex P2 line 390. 단순 `EXISTS terms_consents` 가 아니라 `(term_id, version)` 이 `policy.current_required_terms` 셋에 모두 포함되는지 검증). 위반 시 403. | 상담 신청, 리드 생성, 리포트 저장/공유, 결제 등 AGENTS.md §4.7 #2 의 “전환 시점 OAuth 의무” 라우트 전부. |
...
- **RLS 정책도 동일 가드를 DB 레벨에서 중복 봉인** 한다. **rev8 정정 (Codex P2 line 397) — `AS RESTRICTIVE` 강제**: Postgres RLS 의 기본 모드는 PERMISSIVE 이며 여러 정책이 **OR 로 결합**된다 ([Supabase anonymous user docs](https://supabase.com/docs/guides/auth/auth-anonymous#access-control) 가 명시). 즉 `lead` 테이블에 (a) 우리가 추가하는 `is_anonymous=false + current consent` 술어 정책과 (b) 모듈별로 이미 존재하는 ownership 정책 (`auth.uid() = user_id`) 이 함께 있으면, (b) 가 통과하는 한 (a) 가 fail 해도 INSERT 가 허용된다 — 즉 anonymous 사용자가 자기 row 만 만들면 conversion 가드를 우회. 따라서 본 ADR 은 다음을 봉인한다. **① 모든 conversion-only 테이블의 `is_anonymous=false` + current consent 검증 정책은 `AS RESTRICTIVE` 로 작성** 한다 (RESTRICTIVE 는 AND 결합 — 모든 RESTRICTIVE 가 통과해야 access 허용). **② 그 옆에 도메인 ownership 검증용 permissive 짝 정책 (`auth.uid() = user_id`) 을 동시에 둔다** — RESTRICTIVE 만 있으면 access 자체가 막히므로 permissive companion 이 필요. 예: `lead` INSERT 정책 셋 = `permissive_lead_owner` (`FOR INSERT WITH CHECK auth.uid()=user_id`) + `restrictive_permanent_with_consent` (`AS RESTRICTIVE FOR INSERT WITH CHECK auth.jwt() ->> 'is_anonymous' = 'false' AND (SELECT bool_and(EXISTS (SELECT 1 FROM public.terms_consents tc WHERE tc.user_id = auth.uid() AND tc.term_id = rt.term_id AND tc.version = rt.version)) FROM public.policy_current_required_terms rt)`). **rev9 정정 (Codex P2 line 404) — `WITH CHECK` vs `USING` 봉인**: [PostgreSQL CREATE POLICY 문서](https://www.postgresql.org/docs/current/sql-createpolicy.html) 에 따르면 `USING` 은 SELECT/UPDATE/DELETE 에서 **기존 row** 의 visibility 술어, `WITH CHECK` 은 INSERT/UPDATE 가 만드는 **새 row** 의 valid 술어. INSERT 정책에 `USING` 만 두면 (a) Postgres 가 invalid policy 로 거부하거나 (b) 의도한 새-row 검증이 누락된다. 따라서 본 ADR 은 다음을 봉인한다 — **conversion-only 라우트가 INSERT/UPDATE 하는 모든 테이블의 RLS 정책은 `FOR INSERT WITH CHECK`/`FOR UPDATE WITH CHECK` 로 작성** 하고, SELECT/DELETE 정책만 `USING` 을 쓴다. 자식 §5.3 #11 의 RLS 정책 PR 은 이 패턴을 봉인 + INSERT 정책에 `USING` 만 있는 경우를 차단하는 통합 테스트 의무 (`tests/auth/test_rls_uses_with_check.py`). **③ 대안 (fold pattern)**: 자식 §5.3 #11 PR 이 운영상 RESTRICTIVE 정책 관리가 부담스럽다고 판단하면, 모든 기존 permissive 정책의 `USING` 절에 본 가드 술어를 **직접 fold (AND 추가)** 하는 방법으로 대체 가능 — 단 fold 시 정책 누락 검출용 통합 테스트가 의무. 의존성 누락 시 RLS 가 2차 가드. 정확한 라우트별 매핑 및 RESTRICTIVE vs fold 패턴 선택은 자식 이슈 §5.3 #11 (require_permanent_user 가드 PR) 가 정본.
- **Postgres 측 JWT claims 전파 봉인 (Codex P2 line 390 → rev8 P2 line 398 정정).** `apps/api` 는 SQLAlchemy + psycopg 로 Supabase Postgres 에 **직접 연결** 한다 (PostgREST 가 아니다). 그러므로 Supabase 가 PostgREST 경로에서 자동으로 채워주는 `auth.uid()` / `auth.jwt()` helper 는 우리 트랜잭션 안에서 **자동으로 fire 하지 않는다**. RLS 술어 (`auth.uid() = user_id`, `auth.jwt() ->> 'is_anonymous' = 'false'`) 가 사용자 컨텍스트 없이 평가되어 모두 false / NULL 이 되거나, 우리가 `service_role` 로 연결한 경우 RLS 가 통째로 우회된다. 따라서 다음 패턴을 봉인한다. **(1) rev8 정정 — connection pool 역할.** [Supabase 의 role 모델](https://supabase.com/docs/guides/database/postgres/roles#supabase-roles) 에서 `authenticated` / `anon` 은 **PostgREST 가 `authenticator` 역할에서 JWT 검증 후 `SET ROLE` 로 전환하는 대상 역할**이며, 직접 DB login 용으로는 사용하지 않는다. 따라서 `DATABASE_URL` 의 user 를 `authenticated` 로 두면 (a) login 자체가 실패하거나 (b) 우리 측 서버 가드에 필요한 grant 가 부족해진다 (rev6 의 “authenticated 로 connect” 문구는 **잘못된 봉인 — rev8 에서 폐기**). 정정안: **자식 §5.3 #2 (DEVOPS) 에서 신규 backend login role 을 1회성 생성** (`CREATE ROLE app_backend LOGIN NOINHERIT NOBYPASSRLS; GRANT authenticated, anon TO app_backend;`) 하고, `DATABASE_URL` 의 user 를 `app_backend` 로 지정. 이 role 은 RLS 를 bypass 하지 않고 (NOBYPASSRLS), 트랜잭션 시작부의 `SET LOCAL ROLE authenticated` 호출 권한만 갖는다. service_role / supabase_auth_admin 같은 강한 권한 role 로 connection pool 을 묶지 않는다. **(2) 트랜잭션 별 role + claims 주입.** 각 request 의 트랜잭션 시작부에 FastAPI 의존성 / SQLAlchemy `before_cursor_execute` event listener 가 다음을 실행한다 — `SET LOCAL ROLE authenticated; SET LOCAL "request.jwt.claims" = <검증된 JWT payload JSON>;`. 이로써 `auth.uid()` / `auth.jwt()` 가 트랜잭션 범위 안에서 실제 사용자 컨텍스트를 반환하고, role 전환은 트랜잭션 commit/rollback 시 자동 reset. **(3) 별도 admin pool.** 마이그레이션 / DDL / 운영 잡 등 RLS 우회가 필요한 작업만 별도 connection pool 로 분리하되, **`supabase_auth_admin` 이나 `service_role` 처럼 Supabase 가 사전 정의한 강한 권한 role 로 우리 측 application 풀을 묶지 않는다** — 운영 잡 전용으로 별도 backend admin role (`app_backend_admin`, 필요 시 BYPASSRLS) 을 만들고 그 풀만 별도로 사용. **이 패턴은 자식 §5.3 #4 (FastAPI Supabase JWT 의존성 + RLS 정책 초안) PR 이 봉인** 하고, **통합 테스트 (`tests/auth/test_rls_claims_propagation.py`) 가 다음을 보장**: ① anonymous JWT 가 propagate 된 트랜잭션에서 `lead` INSERT 가 RLS 로 거부됨. ② permanent JWT 가 propagate 된 트랜잭션에서 동일 INSERT 가 통과. ③ JWT propagation 자체가 누락된 트랜잭션 (e.g., 의존성 미주입 → role 이 default `app_backend` 로 남음) 에서도 RLS 가 거부 (NOBYPASSRLS + NULL claims). 자식 §5.3 #11 의 RLS 가드는 이 propagation 위에서만 의미를 갖는다.
...
**4.5.2 Kakao Sync source 분리 저장 PoC.** **Auth Hook 가정 금지** — 현재 Supabase Auth Hook 카탈로그에는 `after_user_created` 가 존재하지 않으며, `before_user_created` 는 `auth.users` row commit 이전이라 `terms_consents.user_id` FK insert 가 불가능하다. Kakao Sync 는 `provider='kakao'` 만으로 약관 동의를 증명할 수 없고, required term tags 는 Kakao `/v2/user/service_terms` REST API 로 검증해야 하므로 Postgres trigger 단독 경로도 금지한다. 본 PoC 는 Database Webhook + 서버 측 Kakao API 검증 + reconciliation 잡 단일 경로를 검증한다.

| # | 경로 | 메커니즘 | 평가 축 |
|---|---|---|---|
| **1순위** | **Supabase Database Webhook → 우리 FastAPI → Kakao `/v2/user/service_terms` 검증** | `auth.users` UPDATE 이벤트를 Supabase Database Webhook 으로 우리 FastAPI `/api/auth/kakao-sync-consent-ingest` 에 push. 서버에서 provider='kakao' 확인 후 Kakao access token 또는 `KAKAO_SERVICE_APP_ADMIN_KEY` 로 `/v2/user/service_terms` 를 호출해 required term tags 를 모두 검증하고, 충족 시에만 `terms_consents(source='kakao_sync')` 를 insert. | 법적 감사 정합 ✅, 운영 가시성 ✅, webhook 실패 시 reconciliation 잡으로 재시도. |
| **fallback** | **server-side reconciliation job** | webhook 누락/실패 시 분 단위 잡이 `auth.users`/`auth.identities` 와 `public.terms_consents` 누락분을 스캔하고 동일한 Kakao service_terms 검증 후 insert 또는 audit 기록. | 사용자 탭 종료/네트워크 단절과 무관하게 eventually consistent. |

**PoC 성공 기준 (Kakao Sync).** Webhook 또는 reconciliation 경로가 Kakao 정상 콜백 후 Kakao `/v2/user/service_terms` 로 required term tags 를 검증하고 동일 `(user_id, term_id, version, source='kakao_sync')` UNIQUE 제약을 위반하지 않는다. Naver / Google 콜백에서는 kakao_sync row 가 생성되지 않으며, required tags 불충족 시에는 `consent_promotion_audit` 에 기록하고 conversion-only 라우트는 `require_permanent_user` 가 403 으로 막는다.

**금지 fallback.** 단순 “콜백 복귀 후 클라이언트 POST” 패턴은 **사용 금지**. 이유: Kakao OAuth 콜백 시점에 이미 `auth.users.is_anonymous=false` 가 commit 되어 영구 user 로 전이되는데, 그 직후 사용자가 탭을 닫거나 네트워크가 끊기면 `source='kakao_sync'` 동의 row 가 영구 누락되어 §2.3 의 “Kakao Sync 시 반드시 source='kakao_sync' row 존재” invariant 가 깨진다. fallback 도 반드시 server-side reconciliation 이어야 하며, Postgres trigger 가 provider=kakao 만 보고 `source='kakao_sync'` row 를 만드는 경로는 required term tags 검증이 불가능하므로 금지한다.
...
| 3 | `[SUPABASE][AUTH][MODEL] public.users / public.terms_consents / public.terms_consent_intents 재정의 + auth.users FK + Alembic vs Supabase CLI 결정 + AFTER UPDATE 트리거 (internal_signup only)` | DB / API | 본 ADR Accepted |
...
| 5 | `[SUPABASE][AUTH][POC] Naver Custom OAuth/OIDC provider (custom:naver) PoC + Kakao Sync Database Webhook/service_terms reconciliation PoC` | API / DEVOPS | 본 ADR Accepted |
...
| `docs/adr/0003-anon-user-and-sso.md` §봉인표 (Refresh Token TTL 등) | refresh TTL · OAuth start route · state store · localStorage 키 · 헤더 이름 | Supabase 정본으로 일괄 이동. **rev8 정정 (Codex P2 line 569)**: OAuth state store 는 Google/Kakao 만 Supabase 로 이동. Naver Custom OAuth PoC 실패 fallback 채택 시 Redis `naver_oauth_state:*` 는 **그대로 보존** (anonymous ownership 보존 필수). 환경변수 폐기 목록은 본 ADR §2.3 환경변수 표 (rev7 정정: `OAUTH_STATE_REDIS_URL` / `OAUTH_STATE_TTL_SECONDS` 폐기 보류). | 본 ADR §2.3 + §2.5 + §4.4 |
...
2. **`SUPABASE_URL` / `SUPABASE_ANON_KEY` / `SUPABASE_SERVICE_ROLE_KEY` / `SUPABASE_JWKS_URL` (1순위 — rev9 정정 Codex P2 line 540) / `SUPABASE_JWT_SECRET` (2순위 legacy fallback) / `SUPABASE_PROJECT_REF` / Supabase Postgres `DATABASE_URL` (Alembic direct, port 5432) / `DATABASE_POOL_URL` (request path, pooler port 6543 — rev9 정정 P2 line 276)** 발급 → 운영 시크릿 매니저 (or `.env`). **본 ADR / 자식 이슈 / PR 본문에는 변수명만 둔다.** `SUPABASE_JWKS_URL` 의 정본 위치는 `https://<project-ref>.supabase.co/auth/v1/.well-known/jwks.json` (Supabase asymmetric signing key flow). 신규 Supabase 프로젝트는 JWKS 우선 사용 권장. JWT_SECRET 은 신규 프로젝트에서 비어 있어도 무방하며, 그 경우 본 ADR §4.3 가드는 JWKS path 만으로 동작해야 한다 (자식 §5.3 #4 PR 봉인). JWKS 캐시·키 회전 처리는 표준 JWKS 라이브러리 (`PyJWT` 등) 위임 + `cache_keys=True` + Supabase 의 `kid` rotation header 자동 대응.
...
| 약관 동의 게이트 (Kakao) | Kakao Sync 약관 화면 자체가 게이트. **rev9 정정 (Codex P2 line 570)**: 후속 저장은 **Database Webhook + Kakao `/v2/user/service_terms` REST API + reconciliation 잡 단일 경로** (rev8 P2 line 258 봉인). `auth.users` AFTER UPDATE Postgres trigger 는 kakao_sync 정합에서 폐기 (`identity_data` 에 term tags 없음 → trigger 검증 불가). trigger 자체는 internal_signup 정합에만 잔존. | `before/after_user_created` Auth Hook 사용 금지. Postgres trigger 의 kakao_sync 분기 금지. |
...
| OAuth state store | Supabase (Google/Kakao) + Redis (Naver fallback 조건부) | **rev8 정정 (Codex P2 line 569)**: Google/Kakao 의 state 는 Supabase 가 관리. Naver Custom OAuth PoC 실패 fallback 채택 시 Redis `naver_oauth_state:<state> = anon_user_id` (TTL ≤ 10분) 가 anonymous ownership 보존을 위한 필수 정본 — 따라서 Redis `oauth_state:*` key prefix 의 폐기는 **PoC 성공 + Custom OAuth 채택 확정 시점에만**. Redis 자체는 채팅·세션 캐시 + Naver fallback state store 로 잔존. |
...
| 2026-06-01 | CTO (`4edca504-...`, CMP-573) | rev8 — rev7 push 직후 Codex 가 단 review threads 5건 (모두 P2) 처리. (1) **P2 line 258** — kakao_sync 검증 경로를 Postgres trigger 에서 Database Webhook + Kakao `/v2/user/service_terms` REST API + reconciliation 잡 단일 경로로 이동 (`identity_data` 에 term tags 가 없으므로 trigger 검증 불가). (2) **P2 line 569** — §0/§6/§8 봉인 표의 “Redis state store 폐기” 를 “Naver fallback 채택 시 보존” 으로 정정. (3) **P2 line 347** — rev5~rev7 의 `auth.identities` 직접 INSERT 패턴 (`fn_link_naver_identity`) 일괄 폐기. fallback 단일 경로를 Auth Admin API `createUser` + FK 재할당 단일 트랜잭션 + `deleteUser` 또는 `public.user_aliases` 매핑 두 supported 패턴으로 봉인. (4) **P2 line 398** — rev6 의 “connect as authenticated” 폐기. 신규 backend login role (`app_backend`, NOBYPASSRLS) 을 만들고 트랜잭션 별 `SET LOCAL ROLE authenticated` + claims 주입. service_role / supabase_auth_admin 같은 강한 권한 role 로 application pool 묶지 않음. (5) **P2 line 397** — RLS 가드를 `AS RESTRICTIVE` 정책 + permissive ownership companion 또는 모든 기존 정책에 fold 패턴으로 봉인 (RLS PERMISSIVE 의 OR 결합으로 가드가 우회되는 문제 차단). |
| 2026-06-01 | CTO (`4edca504-...`, CMP-573) | rev9 — rev8 push 직후 Codex 가 단 review threads 7건 (모두 P2) 처리. (1) **P2 line 570** — §8 봉인 표 Kakao 행에서 Postgres trigger 1순위 표기 제거, rev8 line 258 fix 와 정합되도록 Webhook+reconciliation 단일 경로 명시. (2) **P2 line 540** — §7.2.2 콘솔 작업 checklist 에 `SUPABASE_JWKS_URL` (1순위) + `DATABASE_POOL_URL` 추가 + JWKS 캐시/회전 처리 명시. (3) **P2 line 404** — RLS INSERT 정책의 `USING` 을 `WITH CHECK` 로 정정 + 자식 §5.3 #11 의 통합 테스트 의무 봉인. (4) **P2 line 281** — env block 에 `KAKAO_SERVICE_APP_ADMIN_KEY` 신설 (Kakao `/v2/user/service_terms` 호출용 server admin key, webhook 컨텍스트). (5) **P2 line 350** — §2.5 Naver fallback 의 `admin.createUser()` 직후 브라우저 세션 발급 경로를 `admin.generateLink({ type: 'magiclink', redirectTo: FRONTEND_AUTH_SUCCESS_URL })` 로 봉인. issuer/audience/subject 매핑은 `user_metadata` 에 봉인. (6) **P2 line 276** — `DATABASE_URL` (Alembic direct port 5432) vs `DATABASE_POOL_URL` (request path pooler port 6543) 분리 봉인 (Alembic 이 pooler 안 타도록). (7) **P2 line 297** — `FRONTEND_AUTH_SUCCESS_URL` / `FRONTEND_AUTH_FAILURE_URL` 의 폐기를 “Naver fallback 채택 시 보존” 으로 정정. |
| 2026-06-01 | CTO (`4edca504-...`, CMP-573) | rev10 — Codex PR #44 review thread 1건 (P2 line 356) + board 코멘트 `33cf7055` 처리. (1) rev9 가 봉인한 `admin.generateLink({ type: 'magiclink', email })` 세션 브리지 패턴을 **폐기** — magiclink 는 email/passwordless 흐름이라 세션이 `auth.identities.provider='email'` 로 발급되어 sealed `custom:naver` provider 정책 외 email-login path 를 우회적으로 여는 결과. (2) **Naver Custom OAuth PoC 성공 = MVP Naver 지원 hard 요건** 으로 봉인 — Supabase Auth admin SDK 가 임의 sealed provider 와 정합되는 세션 발급 API 를 노출하지 않으므로 fallback 의 anonymous → permanent 정합 path 가 architectural 으로 존재하지 않음. (3) PoC 실패 시 사용자 UX = Google/Kakao 전환 권유 또는 익명 유지 + PoC 재시도. (4) 자체 JWT 발급 회귀 (ADR-0003 §2.2 패턴) 는 본 ADR 봉인 무력화이므로 금지 — 채택 시 신규 ADR (`0005-naver-without-supabase-custom-oauth.md` 또는 다음 번호) 발행 선결. (5) §2.5 fallback 의 의미 재정의: 사용자 데이터 마이그레이션 약속이 아니라 “Naver MVP 지원 자체를 CEO 결정으로 escalation” + “그 동안 사용자에게 Google/Kakao 전환 또는 익명 유지 제안” 으로 좁힘. |
| _pending_ | CEO | §7.1.2 (리전) + §7.1.3 (Storage) 결정 + **§2.5 Naver Custom OAuth PoC 실패 시 정책 결정 (rev10 신설)**: (a) Naver provider 자체를 MVP 에서 제외하고 Google/Kakao 만 채택, (b) PoC 성공까지 Naver 지원 release 대기, (c) ADR-0005 (또는 다음 번호) 발행하여 self-OAuth + 자체 JWT 발급 패턴 회귀. |
