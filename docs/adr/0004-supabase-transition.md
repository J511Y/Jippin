...
**단일 fallback path (rev8) — FK 재할당 + anon row 삭제**: callback 핸들러가 (a) Naver userinfo + Naver verified email 로 Supabase 측에 새 permanent user 를 생성 (`supabase.auth.admin.createUser({ email, email_confirm: true, ... })` — Auth Admin API 의 supported 경로), (b) **anon `auth.users.id` 에 묶여 있던 모든 도메인 FK** (도면·리포트·약관 동의 intent 등 `public.*` 테이블의 `user_id` 컬럼) 를 새 permanent user.id 로 **단일 트랜잭션** 안에서 일괄 재할당, (c) 우리 측 영구화 로그 (`public.user_migration_audit` — 신설) 에 `(old_anon_id, new_user_id, migrated_at, source='naver_custom_fallback')` row 기록, (d) Supabase Auth Admin API 의 `deleteUser(anonId)` 로 anon `auth.users` row 삭제 (CASCADE 로 `auth.identities` 의 anon row 도 자동 정리). **`auth` 스키마에는 우리가 직접 쓰지 않는다 — 모든 변경은 Auth Admin API 또는 우리 측 `public.*` 테이블 경계 안에서만 일어난다.** 자식 §5.3 #5 (Naver PoC) 의 fallback PR 이 단일 트랜잭션 격리 수준·rollback 시 안전성·FK 컬럼 매핑 카탈로그를 봉인.
