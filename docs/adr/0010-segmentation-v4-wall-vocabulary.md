...
- **겹침 억제**: 병합 후처리에서 RC 와 **양(+)의 면적**이 겹치는 `wall_nonbearing` 영역은
  ...
  조각은 **원본 id 를 버리고** 전체 region 집합에서 유일한 `{id}:N` 을 새로 받는다 —
  아래 선택 프루닝이 기하가 달라진 벽의 저장된 선택을 자동 무효화해, 사용자가 바뀐
  철거 대상을 다시 확인하게 된다. RC 가 벽 안에 완전히 포함되면 difference 의 구멍을
...
- **선택 프루닝**: 재분석은 region id 를 새로 만들므로, 판단스키마에 저장된
  ...
  (`reopen_session_status`, 재개 이벤트 기록) — 사용자가 다시 골라야 하는 현실과
  SSE/퍼널 상태를 일치시킨다.
