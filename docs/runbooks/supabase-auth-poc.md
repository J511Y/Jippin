...
- 자체 비밀번호 가입은 금지한다.
- MVP에서는 manual identity linking을 우선한다. 익명 사전검토 사용자의 OAuth 전환은 로그인된 익명 세션에서 `linkIdentity()`로 검증한다.
- 동일 verified email 기반 automatic identity linking은 허용하지 않는다.
- 여러 provider 계정 통합은 후속 별도 이슈에서 계정 통합 UX, 감사 로그, 보안 검토를 마친 뒤 다룬다.
...
- Supabase는 같은 verified email을 가진 OAuth identity를 자동으로 같은 user에 연결하는 전략을 제공한다. 이 동작은 집핀 MVP 정책과 충돌하므로 채택 전제에서 제외한다. PoC는 automatic linking이 꺼져 있거나 차단 가능한지 확인해야 하며, 차단할 수 없으면 Supabase Auth 완전 대체를 보류한다. 참고: <https://supabase.com/docs/guides/auth/auth-identity-linking>
...
3. 동일 verified email 기반 automatic identity linking 설정이 있으면 disable한다. 끌 수 없으면 이 PoC는 "Supabase Auth 완전 대체 불가"로 판정하고 live OAuth 확장 테스트를 중단한다.
4. Google provider를 enable하고 redirect URL을 Supabase가 제시한 callback URL로 Google Console에 등록한다.
5. Kakao provider를 enable하고 Kakao developers callback URL을 Supabase가 제시한 callback URL로 등록한다.
6. Naver는 Custom OAuth/OIDC provider `custom:naver`로 만든다.
7. Naver가 OIDC discovery를 제공하지 않는 경우 OAuth2 provider로 만들고 Authorization URL, Token URL, UserInfo URL을 수동 입력한다.
8. Site URL과 additional redirect URLs에 `SUPABASE_AUTH_REDIRECT_URL`의 origin/path를 등록한다.
9. JWT signing key가 asymmetric인지 확인한다. JWKS endpoint가 빈 `keys` 배열을 반환하면 FastAPI adapter는 `/auth/v1/user` remote validation fallback이 필요하다.
...
### D. 동일 이메일 Google/Kakao automatic linking 금지 검증
...
- 같은 `auth.users.id`면 Supabase automatic linking이 집핀 정책과 충돌한다. 이 경우 Supabase Auth 완전 대체는 보류하고, 현재 first-party auth stack 유지 또는 부분 채택만 검토한다.

이 검증은 automatic linking을 허용하기 위한 단계가 아니다. automatic linking이 발생하지 않는다는 것을 확인하는 negative test다.
...
- Naver email verified 신호가 있더라도 MVP에서는 자동 계정 병합에 사용하지 않는다.
...
| 동일 이메일 Google/Kakao | provider별 별도 user 유지, automatic linking disable/차단 확인 | 자동으로 같은 user에 병합되거나 차단 방법 없음 | MVP 정책 충돌. Supabase Auth 완전 대체 불가 |
...
|---|---|---|
| `POST /auth/anonymous-users` | CMP-604 폐기 완료 — 즉시 `410 AUTH_LEGACY_FLOW_REMOVED` | Supabase `signInAnonymously()`가 `auth.users.id`와 JWT를 발급한다. |
| `GET /auth/{provider}/start` | CMP-604 폐기 완료 — 즉시 `410 AUTH_LEGACY_FLOW_REMOVED` | Supabase client `signInWithOAuth`/`linkIdentity`가 provider redirect를 소유한다. |
| `GET /auth/callback/{provider}` | CMP-604 폐기 완료 — 즉시 `410 AUTH_LEGACY_FLOW_REMOVED` | Supabase Auth callback이 OAuth code exchange를 처리한다. 앱은 frontend callback에서 Supabase session을 읽는다. |
| `POST /auth/sso-accounts/{provider}/link` | CMP-604 폐기 완료 — 즉시 `410 AUTH_LEGACY_FLOW_REMOVED` | 명시 계정 통합 UX는 Supabase `linkIdentity` 가 소유한다. 자동 email merge는 허용하지 않는다. |
...
- Supabase automatic linking이 켜진 상태로 같은 email OAuth를 병합하면 ADR-0003의 계정 탈취 방어 모델이 깨진다. MVP에서는 허용하지 않는다.
...
동일 이메일 automatic linking을 차단할 수 없거나 Naver/Kakao Sync 감사 요건이 충족되지 않으면 Supabase Auth 완전 대체는 보류하고, Supabase Postgres만 유지하면서 현재 first-party auth stack을 계속 쓴다. 여러 provider 계정 통합은 별도 계정 통합 UX/감사 로그/보안 검토 이슈에서 다룬다.
