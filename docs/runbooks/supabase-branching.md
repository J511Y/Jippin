...
| 단계 | 트리거 | 작업 범위 | 책임 이슈 | 상태 |
|---|---|---|---|---|
| **단계 0 (CMP-574)** | CMP-574 PR 머지 | `supabase/` 스캐폴드 + 본 런북 추가. Neon workflow 무변경. | CMP-574 | ✅ 완료 |
| **단계 1** | CMP-573 ADR-0004 Accepted + wrapper workflow 머지 | Neon workflow gating 또는 archive. wrapper required check 등록. | CMP-603 | ✅ 완료 (CMP-603 단일 cutover) |
| **단계 2** | 단계 1 직후 | `ci.yml::migrate-check` → Supabase SQL migration drift guard 재정의 (§6.3.2). | CMP-603 | ✅ 완료 |
| **단계 3** | 단계 2 후 + §3.1 U7 + §7.2 R5 + §7.3 C3 통과 | `neon-pr-branch.yml` archive, `deploy.yml::release-migrate` Neon 의존 제거, `docs/runbooks/neon-branches.md` archive 배너 강화. **운영 가드 (U7 / R5 / C3) 검증은 사용자 콘솔 연결 후 첫 dummy PR + 첫 `dev → main` 승격 PR 에서 수행** — CMP-603 PR 본문 §사용자 작업 체크리스트 참고. | CMP-603 | ✅ workflow 변경 완료 / 콘솔 검증은 사용자 작업 |

> CMP-603 cutover PR 이 단계 1~3 의 workflow 변경을 일괄 머지했다. 운영 가드 검증 (U7 = production deploy toggle ON, R5 = production Data API 봉인, C3 = connection-string 노출 0건) 은 Supabase 콘솔 연결이 완료된 시점에 사용자가 첫 PR/promotion 으로 검증한다.
