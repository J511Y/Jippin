...
| 게이트 | 조건 | 책임 | 본 PR 적용 |
|---|---|---|---|
| O1. **시크릿 등록 완료 (Vault 비의존 — wake R14 #1).** GitHub Secrets (`SUPABASE_AUTH_EXTERNAL_GOOGLE_CLIENT_ID`/`_SECRET`, `_KAKAO_*`, Naver custom OIDC) + **Supabase branch config env secrets (§7.4 W2 = `supabase secrets set --project-ref <REF> KEY=value` 또는 `--env-file <encrypted.env>`)** 로 등록. **Project Settings → Vault (Postgres `vault.secrets` 저장소) 사용 금지** — Vault 는 DB 안 application 시크릿용이지 `config.toml env()` 채널 아님 (§7.4 정본). 콘솔에서 provider 가 활성화돼 있고 client_id/secret 이 `auth.external.*.secret = env(...)` 의 참조 변수로 등록됨. PR 본문에 `supabase secrets list --project-ref <REF>` 출력 (key 이름만, 값 마스킹) 첨부 의무. **vault key 이름** 첨부 지시는 stale — 본 항목은 §7.4 W-B 와 동일 정본. | CMP-576 (PR 본문에 secrets list 출력 첨부, 값은 절대 첨부 금지) | 본 PR 미적용 |
