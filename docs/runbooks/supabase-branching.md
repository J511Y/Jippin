...
| R3. **PostgREST 토글 PR.** `supabase/config.toml` 의 `[api].enabled = true` + `schemas` 에 `"public"` 추가. PR 본문에 R1·R2 결과 첨부. CMP-575 머지 직후 또는 별도 후속 PR. | CMP-575 후속 |
| R4. **운영 검증.** preview branch 에서 anon key 로 `public` 의 민감 테이블 (`users`, `request_logs`, `consultations`) 직접 조회를 시도해 RLS 가 차단하는지 확인. 통과 결과를 PR 본문에 첨부. | CMP-575 후속 |

R1~R4 미통과 상태에서 `[api].enabled = true` 또는 `schemas` 에 `"public"` 추가를 시도하는 PR 은 머지 금지. 본 봉인은 §6.3.2 의 drift guard 와 독립이다 (drift guard 는 모델/마이그레이션 동행 가드, 본 게이트는 RLS 완성 가드).

본 PR (CMP-574, 단계 0) 의 `supabase/config.toml` 봉인 값: `[api].enabled = false`, `schemas = ["graphql_public"]` (public 미포함). CMP-575/후속 R3 PR 이 토글한다.
