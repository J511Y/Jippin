...
- [ ] **A3.** CMP-576 (Auth 트랙) 이 `supabase/config.toml` 의 `[auth.external.*]` 를 켜고 **같은 PR 에서 `[auth].enable_signup` 글로벌 gate 를 `true` 로 함께 바꾼다** (글로벌 gate 가 false 면 OAuth signup 도 막힘). `[auth.email].enable_signup` 은 영구 false 유지. **OAuth signup gate (MUST — PR 머지 직전 봉인).** A3 PR 머지 직전에 §7.1.4 (OAuth signup activation gate) 의 O1~O5 게이트가 모두 통과 상태여야 한다 (provider 시크릿 secrets/variables 등록 + §7.4 W-A~W-D Supabase-visible env wiring 통과 (root `.env`), redirect_uri 봉인 — 환경별 `env(SUPABASE_AUTH_EXTERNAL_REDIRECT_URI)` parameterize + 4환경 (prod/dev/staging/local) provider 콘솔 등록, §7.1.1 G1~G3 자동 linking 가드 완료 (§7.1.2 M-INTENT discriminator 봉인 동행 — insert 는 SECURITY DEFINER 함수만), §4.7 sealed terms flow 보장 — §7.1.5 H1~H4 + §7.1.6 (`private` schema + 마이그레이션 파일 ①~⑥ 봉인 순서 + RLS policy 명시 + Hook 은 **weak gate** + **gate-before-exchange + cookie-required + direct Postgres connection 패턴 정본** (terms 통과 후 cookie 발급된 callback 만 진입 — no-cookie callback 은 uninitiated OAuth flow 로 reject (exchange 미호출, user 생성 0건) → cookie 있음 + **direct Postgres connection** 으로 `validate_pending_terms_consent` (service_role 만 GRANT, PostgREST 비의존) → false 시 cookie 삭제 + redirect (exchange 미호출, user 생성 0건) → true 시 **`@supabase/ssr` cookie-backed server client** 의 `exchangeCodeForSession` (service_role admin 아님 — PKCE verifier 정본) → `is_new_user == true` 일 때만 direct Postgres 로 `consume_pending_terms_consent_internal` (service_role 만 + caller-supplied term_id/version 신뢰 금지) 호출) + consume false 시 **`app_metadata.terms_missing` marker + `auth.admin.signOut(session.access_token)` (session JWT 기준 — UUID 아님) + `@supabase/ssr` cookie clear**, **`auth.admin.deleteUser` 절대 호출 금지** + 미동의 user 는 §7.1.6 cron (marker user + `is_anonymous=false` 만) cleanup + **`Set-Cookie: __Host-jippin_consent_token; Path=/; Secure; HttpOnly; SameSite=Lax`** (`__Host-` prefix 규약) + `{}` allow / `{"error":{...}}` Supabase 정본 응답 형식 + Kakao Sync 분기 + 21-시나리오 PoC) 통과). 미통과 시 PR 머지 금지. CMP-577 트랙이 `[auth].enable_anonymous_sign_ins=true` 로 전환하며 §7.1.3 PoC 절차 (A5) 를 함께 수행.
...
| 게이트 | 조건 | 책임 |
|---|---|---|
| Cron1. **명시적 `terms_missing` marker user 비활성 cron (MUST — wake#5/PnPk).** Supabase cron 이 다음 SQL 을 N분 주기로 실행:<br/>```sql<br/>-- 명시적 terms_missing marker 가 있는 user (gate-before-exchange 의 backend handler 가<br/>-- consume false 시 set) 만 24h 후 ban. anonymous (auth.users.is_anonymous=true) +<br/>-- 정상 signup user 는 절대 대상이 아님 — marker 없으면 cron 영향 0건.<br/>UPDATE auth.users<br/>   SET banned_until = now() + interval '100 years',<br/>       app_metadata = app_metadata || jsonb_build_object('terms_disabled_at', now())<br/> WHERE (app_metadata->>'terms_missing')::boolean IS TRUE<br/>   AND COALESCE(is_anonymous, false) = false  -- anonymous (signInAnonymously) 제외<br/>   AND (app_metadata->>'terms_missing_set_at')::timestamptz < now() - interval '24 hours'<br/>   AND banned_until IS NULL;<br/>```<br/>**범위 봉인:**<br/>&nbsp;&nbsp;• cleanup 대상 = `app_metadata.terms_missing = true` (§7.1.5 H3 step (e) 의 backend handler 가 consume false 시 명시 set) **AND** `is_anonymous = false` (anonymous precheck 보호).<br/>&nbsp;&nbsp;• **정상 신규 signup user** (`is_new_user = true` + `consume_internal` true → `terms_consents` row 생성됨) → `terms_missing` marker 없음 → cron 무영향.<br/>&nbsp;&nbsp;• **Phase 2 anonymous user** (`signInAnonymously()` 정본 — §7.1.3) → `is_anonymous = true` 로 별도 흐름. terms_consents 가 없는 것이 정상이며 cron 무영향.<br/>&nbsp;&nbsp;• **returning user login** → `is_new_user = false` → marker set 안 됨 → cron 무영향.<br/>본 cron 은 `auth.admin.deleteUser` 가 아닌 ban 으로 처리해 audit trail 보존. 후속 별도 PR 에서 7일 후 deleteUser cron 추가 가능 (동일 marker 조건 유지). | DevOps + CMP-575 (cron SQL) |
