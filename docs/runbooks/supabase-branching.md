...
- [ ] **A3.** CMP-576 (Auth 트랙) 이 `supabase/config.toml` 의 `[auth.external.*]` 를 켜고 **같은 PR 에서 `[auth].enable_signup` 글로벌 gate 를 `true` 로 함께 바꾼다** (글로벌 gate 가 false 면 OAuth signup 도 막힘). `[auth.email].enable_signup` 은 영구 false 유지. **OAuth signup gate (MUST — PR 머지 직전 봉인).** A3 PR 머지 직전에 §7.1.4 (OAuth signup activation gate) 의 O1~O5 게이트가 모두 통과 상태여야 한다 (provider 시크릿 secrets/variables 등록 + §7.4 W-A~W-D Supabase-visible env wiring 통과 (root `.env`), redirect_uri 봉인 — 환경별 `env(SUPABASE_AUTH_EXTERNAL_REDIRECT_URI)` parameterize + 4환경 (prod/dev/staging/local) provider 콘솔 등록, §7.1.1 G1~G3 자동 linking 가드 완료 (§7.1.2 M-INTENT discriminator 봉인 동행 — insert 는 SECURITY DEFINER 함수만), §4.7 sealed terms flow 보장 — §7.1.5 H1~H4 (`private` schema + 마이그레이션 파일 ①~⑥ 봉인 순서 + REVOKE FROM PUBLIC/anon/authenticated/service_role + SECURITY DEFINER insert + `supabase_auth_admin` 만 EXECUTE + `signInWithOAuth({ options: { data: { consent_token } } })` 채널 + nested `event->'user'->'user_metadata'`/`event->'user'->'app_metadata'->>'provider'` 정본 경로 + `{}` allow / `{"error":{...}}` reject Supabase 정본 응답 형식 + Kakao Sync 분기 + 10-시나리오 PoC) 통과). 미통과 시 PR 머지 금지. CMP-577 트랙이 `[auth].enable_anonymous_sign_ins=true` 로 전환하며 §7.1.3 PoC 절차 (A5) 를 함께 수행.
...
| H2. **권한/역할 봉인 — Hook 전용 `private` schema + `supabase_auth_admin` 만 EXECUTE + SECURITY DEFINER insert 채널 (MUST).** Hook 함수와 데이터를 **`public` 이 아닌 `private` schema** (Supabase 정본 권장: https://supabase.com/docs/guides/auth/auth-hooks#developing) 에 둔다. `public` 에 두면 R3 (§7.2) 가 `[api].enabled=true` 로 바뀌는 순간 hook 함수가 PostgREST RPC 로 노출돼 클라이언트가 terms-gate 를 외부 실행/probe 할 수 있다. Postgres 는 default 로 `PUBLIC` role 에 `EXECUTE` 를 부여하므로 GRANT 만으로 충분하지 않고 **명시 REVOKE 동행 필수**.<br/>**마이그레이션 순서 봉인 (MUST — 객체 생성 → GRANT/REVOKE 순서).** Postgres GRANT/REVOKE 는 대상 객체가 이미 존재해야 실행 가능. 따라서 마이그레이션 파일은 다음 순서로 분리:<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ①** `_<ts>_private_schema.sql` — `CREATE SCHEMA IF NOT EXISTS private` + `private_owner` role 생성 + 스키마 default `REVOKE ALL ... FROM anon, authenticated, service_role, PUBLIC` + `ALTER DEFAULT PRIVILEGES`.<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ②** `_<ts>_pending_terms_consents.sql` — 테이블 정의 (§7.1.5 H3 컬럼) + RLS ON + (이 파일 내에서) `supabase_auth_admin` 용 SELECT/UPDATE GRANT + `PUBLIC`/`anon`/`authenticated`/`service_role` REVOKE.<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ③** `_<ts>_identity_link_intents.sql` — 동일 패턴 (테이블 + RLS + GRANT/REVOKE).<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ④** `_<ts>_create_pending_terms_consent_fn.sql` — `public.create_pending_terms_consent(p_provider text, p_terms_version text) SECURITY DEFINER LANGUAGE plpgsql AS $$...$$` 정의. function owner = `private_owner` (혹은 BYPASSRLS owner) — 본 함수가 `private` schema 접근 권한을 위임받는 **유일한 경로**. 함수 본문이 RETURN `consent_token`. `GRANT EXECUTE ... TO anon, authenticated` (terms 화면에서 호출 가능).<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ⑤** `_<ts>_create_identity_link_intent_fn.sql` — 동일 SECURITY DEFINER 패턴 (M-INTENT 발급).<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ⑥** `_<ts>_before_user_created_terms_gate.sql` — Hook 함수 정의 + 함수 자체 `REVOKE EXECUTE FROM PUBLIC, anon, authenticated, service_role` + `GRANT EXECUTE ... TO supabase_auth_admin`.<br/>위 순서를 깨서 파일 ①·② 의 GRANT 가 테이블 생성보다 먼저 실행되면 `relation "private.pending_terms_consents" does not exist` 로 fail. CMP-576 PR 본문에 본 파일 순서 캡처 (파일명 timestamp 정렬) 첨부 의무.<br/>**파일 ②~⑥ 의 표준 SQL 블록 (의미만 — 실 SQL 은 CMP-576 PR 정본).**<br/>```sql<br/>-- 파일 ① (private schema 봉인).<br/>CREATE SCHEMA IF NOT EXISTS private;<br/>CREATE ROLE private_owner NOLOGIN BYPASSRLS;  -- SECURITY DEFINER 함수 owner.<br/>REVOKE ALL ON SCHEMA private FROM PUBLIC, anon, authenticated, service_role;<br/>GRANT USAGE ON SCHEMA private TO private_owner, supabase_auth_admin;<br/>ALTER DEFAULT PRIVILEGES IN SCHEMA private REVOKE ALL ON TABLES   FROM PUBLIC, anon, authenticated, service_role;<br/>ALTER DEFAULT PRIVILEGES IN SCHEMA private REVOKE ALL ON FUNCTIONS FROM PUBLIC, anon, authenticated, service_role;<br/><br/>-- 파일 ② (테이블 + RLS + 권한).<br/>CREATE TABLE private.pending_terms_consents (...);<br/>ALTER TABLE  private.pending_terms_consents OWNER TO private_owner;<br/>ALTER TABLE  private.pending_terms_consents ENABLE ROW LEVEL SECURITY;<br/>REVOKE ALL ON TABLE private.pending_terms_consents FROM PUBLIC, anon, authenticated, service_role;<br/>GRANT SELECT, UPDATE (consumed_at, provider_subject) ON TABLE private.pending_terms_consents TO supabase_auth_admin;<br/><br/>-- 파일 ④ (insert SECURITY DEFINER — 유일한 정본 경로).<br/>CREATE OR REPLACE FUNCTION public.create_pending_terms_consent(p_provider text, p_terms_version text)<br/>  RETURNS uuid<br/>  LANGUAGE plpgsql<br/>  SECURITY DEFINER<br/>  SET search_path = private, public<br/>AS $$<br/>DECLARE new_token uuid;<br/>BEGIN<br/>  IF p_provider NOT IN ('google','naver') THEN  -- Kakao 제외 (AGENTS.md §4.7 #5).<br/>    RAISE EXCEPTION 'provider not eligible for internal terms gate: %', p_provider;<br/>  END IF;<br/>  INSERT INTO private.pending_terms_consents (provider, terms_version, expires_at)<br/>    VALUES (p_provider, p_terms_version, now() + interval '30 minutes')<br/>    RETURNING consent_token INTO new_token;<br/>  RETURN new_token;<br/>END $$;<br/>ALTER FUNCTION public.create_pending_terms_consent(text, text) OWNER TO private_owner;<br/>REVOKE EXECUTE ON FUNCTION public.create_pending_terms_consent(text, text) FROM PUBLIC;<br/>GRANT  EXECUTE ON FUNCTION public.create_pending_terms_consent(text, text) TO anon, authenticated;<br/><br/>-- 파일 ⑥ (Hook 함수 + REVOKE/GRANT — supabase_auth_admin 만).<br/>CREATE OR REPLACE FUNCTION private.before_user_created_terms_gate(event jsonb) RETURNS jsonb<br/>  LANGUAGE plpgsql<br/>AS $$ ... (H3 본문) $$;<br/>REVOKE EXECUTE ON FUNCTION private.before_user_created_terms_gate(jsonb) FROM PUBLIC, anon, authenticated, service_role;<br/>GRANT  EXECUTE ON FUNCTION private.before_user_created_terms_gate(jsonb) TO supabase_auth_admin;<br/>```<br/>**역할 분리 봉인 (MUST — 모순 제거).** API runtime role (`anon`/`authenticated`/`service_role`) 은 **`private` schema USAGE 권한 없음 + 테이블/함수 EXECUTE 권한 없음**. 따라서:<br/>&nbsp;&nbsp;• terms 화면의 `private.pending_terms_consents` row insert 는 **`public.create_pending_terms_consent(...)` SECURITY DEFINER 함수 호출이 유일한 정본 경로** (owner `private_owner` 가 schema 접근권 위임). client (anon) 또는 server (authenticated/service_role) 둘 다 본 함수만 호출.<br/>&nbsp;&nbsp;• **`service_role` 직접 `INSERT INTO private.pending_terms_consents` 는 비정본** — REVOKE 로 차단. 백엔드도 본 함수를 호출한다.<br/>&nbsp;&nbsp;• Hook 함수 실행은 Supabase Auth 가 `supabase_auth_admin` 으로 호출하는 경로만.<br/>**RLS 가 켜진 테이블 (§7.2 R1 이후) 에는 별도 policy** 를 `supabase_auth_admin` role 에 추가하지 않으면 SELECT 가 silent fail → hook 이 모든 signup 을 reject 한다. CMP-575 의 RLS 정본화에서 본 role 의 read policy 를 함께 봉인.<br/>**GRANT 누락 시 첫 OAuth signup 이 `permission denied` 로 fail**, REVOKE 누락 시 PostgREST 노출 시점에 hook 함수가 RPC 로 callable. staging PoC (§3.2 A1.2) 에서 H4 시나리오 ⑥ (GRANT 회수 후 permission denied) + ⑦ (RPC 우회 negative) + ⑧ (service_role 직접 SELECT/INSERT 실패) 모두 검증. | CMP-576 (Hook SQL) + CMP-575 (RLS policy) |
| H3. **pre-user terms consent intent 조회 메커니즘 (MUST — supported channel + nested payload + provider 별 분기).** `before-user-created` Hook 은 candidate user 가 `auth.users` 에 **아직 insert 되기 전** 에 실행되므로 `users.id` 를 키로 한 조회는 불가능하다 (정본 inputs 문서: https://supabase.com/docs/guides/auth/auth-hooks/before-user-created-hook). 그리고 **Supabase hook payload 는 `{ metadata, user }` 형태** 로 `user_metadata`/`app_metadata`/`identities` 가 **`user` 아래에 nested** 되어 있다 (top-level 에 `identity` 키 없음). top-level `event->'user_metadata'`/`event->'identity'` 를 읽으면 NULL → 모든 signup reject 한다. `signInWithOAuth({ queryParams })` 의 provider query 와 `redirectTo` 의 path 는 **Supabase 가 hook payload 로 자동 복사하지 않는다**. 따라서 본 PR 의 supported channel + payload 정본:<br/>**Supported channel = `signInWithOAuth({ options: { data: { consent_token } } })`.** `options.data` 는 Supabase JS 정본 (https://supabase.com/docs/reference/javascript/auth-signinwithoauth) 에서 **`auth.users.raw_user_meta_data` 로 자동 저장**되도록 지원되는 유일한 채널이다. hook 입력 정본 경로:<br/>&nbsp;&nbsp;• consent_token: **`event->'user'->'user_metadata'->>'consent_token'`** (또는 동일 의미의 `event->'user'->'raw_user_meta_data'->>'consent_token'`)<br/>&nbsp;&nbsp;• provider: **`event->'user'->'app_metadata'->>'provider'`** (single sign-up) 또는 `event->'user'->'identities'->0->>'provider'` (identities array — Supabase 가 채우는 정본 위치)<br/>&nbsp;&nbsp;• provider_id (provider 측 subject): `event->'user'->'identities'->0->>'id'` 또는 `event->'user'->'identities'->0->'identity_data'->>'sub'`<br/>PKCE state / queryParams 우회는 비정본 — 사용하지 않는다.<br/>**provider 별 분기 (MUST — AGENTS.md §4.7 #5 봉인).** Kakao 는 **Kakao Sync 자체 약관 화면을 제공**하므로 우리 internal terms 화면을 거치지 않으며 `consent_token` 발급 흐름이 없다 (정본: AGENTS.md §4.7 #5 — `source='kakao_sync'` 분리 저장 + 내부 약관 중복 노출 금지). 따라서 hook 은 provider 별 분기:<br/>&nbsp;&nbsp;• `provider = 'kakao'` → consent_token 검사 skip. 대신 Kakao Sync 가 제공한 약관 동의 fact 가 `event->'user'->'identities'->0->'identity_data'` 에 존재하는지 (Kakao API 응답의 `service_terms`/agreed_at 등) 만 확인. 누락 시 reject. CMP-576 가 Kakao Sync 응답 keys 의 정본 위치를 PoC 로 봉인.<br/>&nbsp;&nbsp;• `provider ∈ {'google', 'naver'}` (internal terms 흐름) → 아래 ①~④ 의 consent_token 검증 적용.<br/>본 PR 봉인 모델 (`internal terms 흐름 — google/naver`):<br/>① **테이블 + insert 경로.** `private.pending_terms_consents` 정의 (`private` schema 는 §7.1.5 H2 봉인). 컬럼: `consent_token uuid PRIMARY KEY DEFAULT gen_random_uuid()`, `provider text NOT NULL CHECK (provider IN ('google','naver'))` (Kakao 제외), `terms_version text NOT NULL`, `accepted_at timestamptz NOT NULL DEFAULT now()`, `expires_at timestamptz NOT NULL` (e.g. now() + 30 분), `provider_subject text NULL`, `consumed_at timestamptz NULL`. RLS ON. **Insert 경로는 SECURITY DEFINER 함수 `public.create_pending_terms_consent(p_provider text, p_terms_version text)` 만 — `service_role` / `anon` / `authenticated` 의 직접 INSERT 는 모두 §7.1.5 H2 봉인으로 차단된다.** 본 함수 owner 는 `private` schema 접근권을 가진 별도 owner role (예: `private_owner`) 로 두고, 함수 본문이 위 컬럼만 검증·INSERT 한 뒤 `consent_token` RETURN. 호출자는 anon JWT (terms 화면이 client 에서 직접 호출 가능) 또는 FastAPI 백엔드.<br/>② **흐름.** (1) terms 화면에서 사용자가 동의 → client 가 `supabase.rpc('create_pending_terms_consent', { p_provider: 'google', p_terms_version: '<ver>' })` 호출 → `consent_token` 반환. (2) web 이 토큰으로 OAuth signin: `supabase.auth.signInWithOAuth({ provider, options: { redirectTo, data: { consent_token } } })`. (3) Supabase Auth → provider → `/auth/v1/callback` → `before-user-created` hook 호출, `event.user.user_metadata.consent_token` 로 token 전달. (4) Hook 본문 (provider 별 분기 포함):<br/>```sql<br/>-- private.before_user_created_terms_gate(event jsonb) RETURNS jsonb<br/>DECLARE<br/>  provider text := COALESCE(event->'user'->'app_metadata'->>'provider', event->'user'->'identities'->0->>'provider');<br/>  token uuid := NULLIF(event->'user'->'user_metadata'->>'consent_token', '')::uuid;<br/>BEGIN<br/>  -- Kakao Sync: 자체 약관 화면 사용 (AGENTS.md §4.7 #5). consent_token 미요구.<br/>  IF provider = 'kakao' THEN<br/>    -- Kakao Sync 약관 동의 fact 확인 (CMP-576 가 Kakao API 응답 정본 keys 봉인).<br/>    IF event->'user'->'identities'->0->'identity_data'->'service_terms' IS NULL THEN<br/>      RETURN jsonb_build_object('error', jsonb_build_object('http_code', 403, 'message', 'kakao_terms_missing'));<br/>    END IF;<br/>    RETURN '{}'::jsonb;  -- allow<br/>  END IF;<br/>  -- internal terms 흐름 (google/naver).<br/>  IF token IS NULL OR NOT EXISTS (<br/>    SELECT 1 FROM private.pending_terms_consents<br/>    WHERE consent_token = token<br/>      AND pending_terms_consents.provider = provider<br/>      AND consumed_at IS NULL<br/>      AND expires_at > now()<br/>  ) THEN<br/>    RETURN jsonb_build_object('error', jsonb_build_object('http_code', 403, 'message', 'terms_not_accepted'));<br/>  END IF;<br/>  UPDATE private.pending_terms_consents<br/>    SET consumed_at = now(),<br/>        provider_subject = event->'user'->'identities'->0->>'id'<br/>    WHERE consent_token = token;<br/>  RETURN '{}'::jsonb;  -- allow<br/>END;<br/>```<br/>③ **expiry/회수.** 30분 expiry + consumed_at 단방향 update + cron 으로 7일 이상 row 삭제 (`pg_cron` extension 또는 별도 cleanup job). consent_token 재사용 시 같은 user 가 같은 provider 로 재시도해도 reject.<br/>④ **Naver custom OAuth/OIDC** 도 internal terms 흐름. `provider_subject` 는 (4) 의 `event->'user'->'identities'->0->>'id'` 또는 `identity_data->>'sub'` 로 채움.<br/>**비정본 (절대 사용 금지):** (a) `signInWithOAuth({ options: { queryParams: { state: ... } } })` 의 state 우회. PKCE state 는 Supabase 가 자체 관리하며 hook payload 에 노출하지 않는다. (b) top-level `event->'user_metadata'`/`event->'identity'` 읽기 — 위 nested 경로만 정본. (c) Kakao 에 internal `consent_token` 요구. | CMP-576 + CMP-577 |
| H4. **Hook 활성화 self-check (CMP-576 PR).** 다음 PoC 결과를 staging branch 에서 캡처해 CMP-576 PR 본문에 첨부 — **Hook 응답은 Supabase 정본 contract** (allow = `{}`, reject = `{"error":{"http_code":<int>,"message":"<id>"}}`) 로 검증한다. `{"decision":"reject"}` 류는 Supabase 가 `error` 필드 부재로 **fail-open 처리해 user 가 생성된다** — 본 시나리오는 negative 로 함께 검증한다.<br/>① consent_token 없이 google/naver OAuth signup → Hook 응답 `{"error":{"http_code":403,"message":"terms_not_accepted"}}` + `auth.users` row 0건<br/>② 만료된 consent_token → 동일 error + row 0건<br/>③ 유효 consent_token (google/naver) → Hook 응답 `{}` (allow) + `auth.users` row 생성 + `private.pending_terms_consents.consumed_at` 채워짐 + `provider_subject` 채워짐<br/>④ 같은 consent_token 재사용 → Hook reject error<br/>⑤ provider 불일치 (google token 으로 naver signup) → Hook reject error<br/>⑥ `supabase_auth_admin` 의 GRANT 회수 후 ③ 재실행 → permission denied 로 fail<br/>⑦ **RPC 우회 negative.** `anon` JWT 로 PostgREST `/rest/v1/rpc/before_user_created_terms_gate` 호출 시도 → 404 / `permission denied for function` / schema 노출 안 됨<br/>⑧ **`service_role` 직접 write 차단 negative.** `service_role` JWT 로 `SELECT *` / `INSERT INTO private.pending_terms_consents` 시도 → `permission denied for schema private` (모두 실패)<br/>⑨ **Kakao Sync 흐름 — `consent_token` 미요구.** `provider = 'kakao'` 로 signup 시 `consent_token` 없이도 Kakao Sync `identity_data.service_terms` 가 있으면 Hook 응답 `{}` → `auth.users` 생성. Kakao Sync `service_terms` 누락 시 reject error `{"http_code":403,"message":"kakao_terms_missing"}` + row 0건. CMP-576 가 Kakao API 응답 정본 keys 봉인 (필요 시 CMP-577 PoC 캡처).<br/>⑩ **error 형식 negative.** 임시로 Hook 본문을 `RETURN jsonb_build_object('decision','reject','message','x')` 로 바꾸고 ① 재실행 → `error` 필드 부재로 Supabase 가 fail-open → `auth.users` row 생성됨을 캡처. 본 캡처는 "왜 `error` object 형식이 필수인지" 의 증거로 PR 본문에 포함 (원상 복구 후 H4 통과).<br/>위 10개 시나리오 중 1개라도 누락된 PR 은 §3.2 A3 + 본 게이트로 reject. | CMP-576 + CMP-577 |

본 PR (CMP-574, 단계 0) 은 Hook 자체를 도입하지 않는다. CMP-576 PR 이 본 §7.1.5 H1~H4 를 정본화해 다음 6개 SQL 마이그레이션을 H2 봉인 순서대로 추가한다:<br/>① `_<ts1>_private_schema.sql`, ② `_<ts2>_pending_terms_consents.sql`, ③ `_<ts3>_identity_link_intents.sql`, ④ `_<ts4>_create_pending_terms_consent_fn.sql`, ⑤ `_<ts5>_create_identity_link_intent_fn.sql`, ⑥ `_<ts6>_before_user_created_terms_gate.sql`. timestamp `<ts1> < <ts2> < ... < <ts6>` 이 자동으로 강제되도록 PR 작성 시 동일 타임라인 사용. web 측 token 전파 (`signInWithOAuth({ options: { data: { consent_token } } })`) 와 terms 화면의 `supabase.rpc('create_pending_terms_consent', ...)` 호출은 CMP-577 이 담당. §7.1.4 O4 (a) 의 PoC 첨부 의무는 본 §7.1.5 H4 결과 (10 시나리오) 로 만족한다.
