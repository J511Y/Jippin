...
- [ ] **A3.** CMP-576 (Auth 트랙) 이 `supabase/config.toml` 의 `[auth.external.*]` 를 켜고 **같은 PR 에서 `[auth].enable_signup` 글로벌 gate 를 `true` 로 함께 바꾼다** (글로벌 gate 가 false 면 OAuth signup 도 막힘). `[auth.email].enable_signup` 은 영구 false 유지. **OAuth signup gate (MUST — PR 머지 직전 봉인).** A3 PR 머지 직전에 §7.1.4 (OAuth signup activation gate) 의 O1~O5 게이트가 모두 통과 상태여야 한다 (provider 시크릿 secrets/variables 등록 + §7.4 W-A~W-D Supabase-visible env wiring 통과 (root `.env`), redirect_uri 봉인 — 환경별 parameterize + 4환경 provider 콘솔 등록, §7.1.1 G1~G3 자동 linking 가드 완료 + §7.1.2 M-INTENT discriminator, §4.7 sealed terms flow 보장 — §7.1.5 H1~H4 + §7.1.6 (`private` schema + cross-schema GRANT (`private_owner` 가 `public.terms` SELECT + `public.terms_consents` INSERT + `auth.users` UPDATE on `raw_app_meta_data` 권한 보유 — wake R8 #4) + 마이그레이션 파일 ①~⑥ 봉인 순서 + RLS policy 명시 + Hook weak gate + **gate-before-exchange + cookie-required + direct Postgres connection 패턴 정본** (terms 화면 → backend 가 **`create_pending_terms_consent(provider, version, term_ids[])` server-only direct Postgres** 호출 — anon/authenticated GRANT 없음, **다중 required term_id array** 보존 + cookie 발급 → no-cookie callback uninitiated reject → cookie 있음 + direct PG `validate_pending_terms_consent` false → cookie 삭제 + redirect (exchange 미호출) → true → **`@supabase/ssr` cookie-backed exchangeCodeForSession** → `is_new_user == true` 면 direct PG `consume_pending_terms_consent_internal` 가 **각 required term_id 별로 terms_consents row INSERT** (wake R8 #2) + consume false 시 **`raw_app_meta_data` (DB column — `app_metadata` 가 아님, wake R8 #1) 에 marker UPDATE + `auth.admin.signOut(session.access_token)` (session JWT) + `@supabase/ssr` cookie clear**, **`auth.admin.deleteUser` 절대 호출 금지** + 미동의 user 는 §7.1.6 cron (`raw_app_meta_data->>'terms_missing'=true` AND `is_anonymous=false`) cleanup + `Set-Cookie: __Host-jippin_consent_token; Path=/; Secure; HttpOnly; SameSite=Lax` + `{}` allow / `{"error":{...}}` reject 정본 형식 + Kakao Sync 분기 + 25-시나리오 PoC) 통과). 미통과 시 PR 머지 금지. CMP-577 트랙이 `[auth].enable_anonymous_sign_ins=true` 로 전환하며 §7.1.3 PoC 절차 (A5) 를 함께 수행.
