...
- [ ] **A3.** CMP-576 (Auth 트랙) 이 `supabase/config.toml` 의 `[auth.external.*]` 를 켜고 **같은 PR 에서 `[auth].enable_signup` 글로벌 gate 를 `true` 로 함께 바꾼다** (글로벌 gate 가 false 면 OAuth signup 도 막힘). `[auth.email].enable_signup` 은 영구 false 유지. **OAuth signup gate (MUST — PR 머지 직전 봉인).** A3 PR 머지 직전에 §7.1.4 (OAuth signup activation gate) 의 O1~O5 게이트가 모두 통과 상태여야 한다 (provider 시크릿 secrets/variables 등록 + §7.4 W-A~W-D Supabase-visible env wiring 통과 (root `.env`), redirect_uri 봉인 — 환경별 `env(SUPABASE_AUTH_EXTERNAL_REDIRECT_URI)` parameterize + 4환경 (prod/dev/staging/local) provider 콘솔 등록, §7.1.1 G1~G3 자동 linking 가드 완료 (§7.1.2 M-INTENT discriminator 봉인 동행 — insert 는 SECURITY DEFINER 함수만), §4.7 sealed terms flow 보장 — §7.1.5 H1~H4 + §7.1.6 (`private` schema + 마이그레이션 파일 ①~⑥ 봉인 순서 + RLS policy 명시 + Hook 은 **weak gate** + **gate-before-exchange 패턴 정본** (Next.js `/auth/callback` Route Handler 가 cookie 검사 → `validate_pending_terms_consent` (service_role 만 GRANT) → false 시 cookie 삭제 + redirect (exchange 미호출, user 생성 0건) → true 시 `exchangeCodeForSession` → `is_new_user == true` 일 때만 `consume_pending_terms_consent_internal` (service_role 만 + caller-supplied term_id/version 신뢰 금지 — 함수가 token row 에서 도출) 호출) + `auth.admin.deleteUser` **절대 호출 금지** (returning user 사고 차단 — wake#3) + 미동의 user 는 §7.1.6 cron cleanup + **`Set-Cookie: __Host-jippin_consent_token; Path=/; Secure; HttpOnly; SameSite=Lax`** (`__Host-` prefix 규약 — wake#1) + `{}` allow / `{"error":{...}}` reject Supabase 정본 응답 형식 + Kakao Sync 분기 + 16-시나리오 PoC) 통과). 미통과 시 PR 머지 금지. CMP-577 트랙이 `[auth].enable_anonymous_sign_ins=true` 로 전환하며 §7.1.3 PoC 절차 (A5) 를 함께 수행.
...
| H2. **권한/역할 봉인 — Hook 전용 `private` schema + `supabase_auth_admin` 만 EXECUTE + SECURITY DEFINER insert 채널 (MUST).** Hook 함수와 데이터를 **`public` 이 아닌 `private` schema** (Supabase 정본 권장: https://supabase.com/docs/guides/auth/auth-hooks#developing) 에 둔다. `public` 에 두면 R3 (§7.2) 가 `[api].enabled=true` 로 바뀌는 순간 hook 함수가 PostgREST RPC 로 노출돼 클라이언트가 terms-gate 를 외부 실행/probe 할 수 있다. Postgres 는 default 로 `PUBLIC` role 에 `EXECUTE` 를 부여하므로 GRANT 만으로 충분하지 않고 **명시 REVOKE 동행 필수**.<br/>**마이그레이션 순서 봉인 (MUST — 객체 생성 → GRANT/REVOKE 순서).** Postgres GRANT/REVOKE/POLICY 는 대상 객체가 이미 존재해야 실행 가능. 따라서 마이그레이션 파일은 다음 순서로 분리:<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ①** `_<ts>_private_schema.sql` — `CREATE SCHEMA IF NOT EXISTS private` + `private_owner` role (`BYPASSRLS NOLOGIN`) 생성 + 스키마 default `REVOKE ALL ... FROM anon, authenticated, service_role, PUBLIC` + `ALTER DEFAULT PRIVILEGES`.<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ②** `_<ts>_pending_terms_consents.sql` — 테이블 정의 (§7.1.5 H3 컬럼) + `ALTER TABLE ... OWNER TO private_owner` + `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` + `ALTER TABLE ... FORCE ROW LEVEL SECURITY` + REVOKE/GRANT + **RLS policy 명시** (`supabase_auth_admin` 용 SELECT/UPDATE policy — wake#3 봉인).<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ③** `_<ts>_identity_link_intents.sql` — 동일 패턴 (테이블 + RLS + RLS policy + GRANT/REVOKE).<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ④** `_<ts>_create_pending_terms_consent_fn.sql` + `consume_pending_terms_consent_atomic_fn.sql` — SECURITY DEFINER 함수 2개:<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;• `public.create_pending_terms_consent(p_provider text, p_terms_version text) RETURNS uuid` — terms 화면이 호출, `consent_token` 발급.<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;• `public.consume_pending_terms_consent_atomic(p_token uuid, p_provider text, p_provider_subject text, p_user_id uuid, p_term_id uuid, p_terms_version text) RETURNS boolean` — app `/auth/callback` 이 호출. 본 함수가 **단일 conditional UPDATE + terms_consents INSERT 를 동일 transaction 으로 atomic 수행** (wake#2 봉인). owner = `private_owner` (`private` schema 접근권 + RLS bypass).<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ⑤** `_<ts>_create_identity_link_intent_fn.sql` — M-INTENT 발급 SECURITY DEFINER 함수.<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ⑥** `_<ts>_before_user_created_terms_gate.sql` — Hook 함수 (weak gate — Kakao 분기 + google/naver allow) + 함수 자체 `REVOKE EXECUTE FROM PUBLIC, anon, authenticated, service_role` + `GRANT EXECUTE ... TO supabase_auth_admin`.<br/>위 순서를 깨서 파일 ①·② 의 GRANT/POLICY 가 테이블 생성보다 먼저 실행되면 `relation "private.pending_terms_consents" does not exist` 로 fail. CMP-576 PR 본문에 본 파일 순서 캡처 (파일명 timestamp 정렬) 첨부 의무.<br/>**파일 ②~⑥ 의 표준 SQL 블록 (의미만 — 실 SQL 은 CMP-576 PR 정본).**<br/>```sql<br/>-- 파일 ① (private schema 봉인).<br/>CREATE SCHEMA IF NOT EXISTS private;<br/>CREATE ROLE private_owner NOLOGIN BYPASSRLS;  -- SECURITY DEFINER 함수 owner + RLS bypass.<br/>REVOKE ALL ON SCHEMA private FROM PUBLIC, anon, authenticated, service_role;<br/>GRANT USAGE ON SCHEMA private TO private_owner, supabase_auth_admin;<br/>ALTER DEFAULT PRIVILEGES IN SCHEMA private REVOKE ALL ON TABLES   FROM PUBLIC, anon, authenticated, service_role;<br/>ALTER DEFAULT PRIVILEGES IN SCHEMA private REVOKE ALL ON FUNCTIONS FROM PUBLIC, anon, authenticated, service_role;<br/><br/>-- 파일 ② (테이블 + RLS + policy + 권한).<br/>CREATE TABLE private.pending_terms_consents (...);<br/>ALTER TABLE  private.pending_terms_consents OWNER TO private_owner;<br/>ALTER TABLE  private.pending_terms_consents ENABLE ROW LEVEL SECURITY;<br/>ALTER TABLE  private.pending_terms_consents FORCE ROW LEVEL SECURITY;<br/>REVOKE ALL   ON TABLE private.pending_terms_consents FROM PUBLIC, anon, authenticated, service_role;<br/>GRANT  SELECT, UPDATE (consumed_at, provider_subject) ON TABLE private.pending_terms_consents TO supabase_auth_admin;<br/>-- RLS policy 명시 (wake#3 — GRANT 만으로 RLS bypass 안 됨).<br/>CREATE POLICY pending_terms_consents_auth_admin_select<br/>  ON private.pending_terms_consents FOR SELECT TO supabase_auth_admin USING (true);<br/>CREATE POLICY pending_terms_consents_auth_admin_update<br/>  ON private.pending_terms_consents FOR UPDATE TO supabase_auth_admin USING (true) WITH CHECK (true);<br/>-- private_owner (SECURITY DEFINER 함수 owner) 는 BYPASSRLS 로 별도 policy 불요.<br/>-- anon/authenticated/service_role 는 policy 없음 — 모든 row 차단.<br/><br/>-- 파일 ④ (insert SECURITY DEFINER + atomic consume SECURITY DEFINER).<br/>CREATE OR REPLACE FUNCTION public.create_pending_terms_consent(p_provider text, p_terms_version text)<br/>  RETURNS uuid<br/>  LANGUAGE plpgsql<br/>  SECURITY DEFINER<br/>  SET search_path = private, public<br/>AS $$<br/>DECLARE new_token uuid;<br/>BEGIN<br/>  IF p_provider NOT IN ('google','naver') THEN  -- Kakao 제외 (AGENTS.md §4.7 #5).<br/>    RAISE EXCEPTION 'provider not eligible for internal terms gate: %', p_provider;<br/>  END IF;<br/>  INSERT INTO private.pending_terms_consents (provider, terms_version, expires_at)<br/>    VALUES (p_provider, p_terms_version, now() + interval '30 minutes')<br/>    RETURNING consent_token INTO new_token;<br/>  RETURN new_token;<br/>END $$;<br/>ALTER FUNCTION public.create_pending_terms_consent(text, text) OWNER TO private_owner;<br/>REVOKE EXECUTE ON FUNCTION public.create_pending_terms_consent(text, text) FROM PUBLIC;<br/>GRANT  EXECUTE ON FUNCTION public.create_pending_terms_consent(text, text) TO anon, authenticated;<br/><br/>-- validate (read-only pre-exchange gate — wake#JMb gate-before-exchange).<br/>CREATE OR REPLACE FUNCTION public.validate_pending_terms_consent(p_token uuid, p_expected_provider text)<br/>  RETURNS boolean<br/>  LANGUAGE plpgsql<br/>  SECURITY DEFINER<br/>  SET search_path = private, public<br/>AS $$<br/>BEGIN<br/>  RETURN EXISTS (<br/>    SELECT 1 FROM private.pending_terms_consents<br/>     WHERE consent_token = p_token<br/>       AND provider = p_expected_provider<br/>       AND consumed_at IS NULL<br/>       AND expires_at > now()<br/>  );<br/>END $$;<br/>ALTER FUNCTION public.validate_pending_terms_consent(uuid, text) OWNER TO private_owner;<br/>REVOKE EXECUTE ON FUNCTION public.validate_pending_terms_consent(uuid, text) FROM PUBLIC, anon, authenticated;<br/>GRANT  EXECUTE ON FUNCTION public.validate_pending_terms_consent(uuid, text) TO service_role;<br/>-- service_role 만 — Next.js callback Route Handler 가 서버에서 호출 (wake#JMY).<br/><br/>-- consume_internal (post-exchange atomic — wake#2 race-free + wake#4 terms_consents 영속화).<br/>-- caller-supplied term_id/terms_version 신뢰 금지 — token row 에서 도출 (wake#JMY).<br/>CREATE OR REPLACE FUNCTION public.consume_pending_terms_consent_internal(<br/>  p_token uuid, p_provider text, p_user_id uuid<br/>) RETURNS boolean<br/>  LANGUAGE plpgsql<br/>  SECURITY DEFINER<br/>  SET search_path = private, public<br/>AS $$<br/>DECLARE<br/>  consumed_count int;<br/>  row_terms_version text;<br/>  row_term_id uuid;<br/>BEGIN<br/>  -- 단일 conditional UPDATE + RETURNING — race-free atomic consume (wake#2).<br/>  UPDATE private.pending_terms_consents<br/>     SET consumed_at = now()<br/>   WHERE consent_token = p_token<br/>     AND provider = p_provider<br/>     AND consumed_at IS NULL<br/>     AND expires_at > now()<br/>  RETURNING terms_version INTO row_terms_version;<br/>  GET DIAGNOSTICS consumed_count = ROW_COUNT;<br/>  IF consumed_count <> 1 THEN<br/>    RETURN false;  -- caller 가 signOut + 에러. deleteUser 안 함 (wake#3).<br/>  END IF;<br/>  -- term_id 도 caller 가 아닌 server-side resolve (active terms 의 id).<br/>  SELECT term_id INTO row_term_id FROM public.terms WHERE is_active LIMIT 1;<br/>  -- 같은 transaction — terms_consents 영속화 (wake#4 — AGENTS.md §4.7 정본).<br/>  INSERT INTO public.terms_consents (user_id, term_id, version, source, agreed_at)<br/>    VALUES (p_user_id, row_term_id, row_terms_version, 'internal_signup', now());<br/>  RETURN true;<br/>END $$;<br/>ALTER FUNCTION public.consume_pending_terms_consent_internal(uuid, text, uuid) OWNER TO private_owner;<br/>REVOKE EXECUTE ON FUNCTION public.consume_pending_terms_consent_internal(uuid, text, uuid) FROM PUBLIC, anon, authenticated;<br/>GRANT  EXECUTE ON FUNCTION public.consume_pending_terms_consent_internal(uuid, text, uuid) TO service_role;<br/>-- service_role 만 — Next.js callback Route Handler 가 서버에서 호출 (wake#JMY).<br/><br/>-- 파일 ⑥ (Hook 함수 weak gate + REVOKE/GRANT).<br/>CREATE OR REPLACE FUNCTION private.before_user_created_terms_gate(event jsonb) RETURNS jsonb<br/>  LANGUAGE plpgsql<br/>AS $$ ... (H3 본문 — Kakao 분기 + google/naver allow) $$;<br/>REVOKE EXECUTE ON FUNCTION private.before_user_created_terms_gate(jsonb) FROM PUBLIC, anon, authenticated, service_role;<br/>GRANT  EXECUTE ON FUNCTION private.before_user_created_terms_gate(jsonb) TO supabase_auth_admin;<br/>```<br/>**역할 분리 봉인 (MUST — 모순 제거).** API runtime role (`anon`/`authenticated`/`service_role`) 은 **`private` schema USAGE 권한 없음 + 테이블/함수 EXECUTE 권한 없음**. 따라서:<br/>&nbsp;&nbsp;• terms 화면의 `private.pending_terms_consents` row insert 는 **`public.create_pending_terms_consent(...)` SECURITY DEFINER 함수 호출이 유일한 정본 경로** (owner `private_owner` 가 schema 접근권 위임). client (anon) 또는 server (authenticated/service_role) 둘 다 본 함수만 호출.<br/>&nbsp;&nbsp;• **`service_role` 직접 `INSERT INTO private.pending_terms_consents` 는 비정본** — REVOKE 로 차단. 백엔드도 본 함수를 호출한다.<br/>&nbsp;&nbsp;• Hook 함수 실행은 Supabase Auth 가 `supabase_auth_admin` 으로 호출하는 경로만.<br/>**RLS 가 켜진 테이블 (§7.2 R1 이후) 에는 별도 policy** 를 `supabase_auth_admin` role 에 추가하지 않으면 SELECT 가 silent fail → hook 이 모든 signup 을 reject 한다. CMP-575 의 RLS 정본화에서 본 role 의 read policy 를 함께 봉인.<br/>**GRANT 누락 시 첫 OAuth signup 이 `permission denied` 로 fail**, REVOKE 누락 시 PostgREST 노출 시점에 hook 함수가 RPC 로 callable. staging PoC (§3.2 A1.2) 에서 H4 시나리오 ⑥ (GRANT 회수 후 permission denied) + ⑦ (RPC 우회 negative) + ⑧ (service_role 직접 SELECT/INSERT 실패) 모두 검증. | CMP-576 (Hook SQL) + CMP-575 (RLS policy) |
| H3. **pre-user terms consent intent 조회 메커니즘 (MUST — gate-before-exchange 패턴).** Supabase OAuth flow 는 Hook payload 로 client-side consent_token 을 전달할 supported channel 이 없다 (정본: https://supabase.com/docs/reference/javascript/auth-signinwithoauth — 정본 options 는 `queryParams`, `redirectTo`, `scopes` 만, `data` 는 unsupported, PKCE state 는 Supabase 자체 관리). **그리고 callback-after-insert 패턴은 (a) `auth.users` 행이 잠시 영속하면서 §4.7 sealed flow (internal terms 동의 전 영속 user row 금지) 를 위반하고, (b) returning user 의 login callback 에서 잘못 deleteUser 호출 사고를 낼 수 있어 비정본** 이다 (wake#3, #4). 본 PR 봉인 정본은 **gate-before-exchange** 패턴 — `exchangeCodeForSession` 호출 자체를 terms gate 가 통과한 신규 signup 흐름에서만 수행하여 user 가 생성되는 순간을 지연시킨다.<br/>**정본 흐름 (Google/Naver internal terms).**<br/>① **terms 동의.** terms 화면에서 사용자 동의 → backend (Next.js Route Handler `/api/terms/accept` 또는 FastAPI) 가 `supabase.rpc('create_pending_terms_consent', { p_provider: 'google', p_terms_version: '<ver>' })` 호출 → `consent_token` 반환. backend 가 다음 **명시 Set-Cookie** 헤더로 cookie 설정 (`__Host-` prefix 규약 — MDN: https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Set-Cookie 그대로 따른다):<br/>&nbsp;&nbsp;`Set-Cookie: __Host-jippin_consent_token=<token>; Path=/; Secure; HttpOnly; SameSite=Lax; Max-Age=1800`<br/>**`Path=/` 명시 + `Domain` 미지정 + `Secure` 가 `__Host-` prefix 규약 (MUST 3종 모두)** — 하나라도 빠지면 브라우저가 cookie 자체를 reject 한다. `Path=/` 가 없으면 `/auth/callback` route 가 cookie 를 받지 못해 모든 신규 signup 이 gate fail 한다 (wake#1).<br/>② **PKCE flow 시작.** Frontend `signInWithOAuth({ flowType: 'pkce', provider, options: { redirectTo: '<app>/auth/callback', queryParams, scopes } })` 호출. PKCE code_verifier 는 Supabase SDK 가 별도 cookie 에 저장.<br/>③ **provider OAuth → Supabase 단발성 code → app callback.** provider 인증 → Supabase Auth `/auth/v1/callback` → **이 시점에는 `auth.users` 가 생성되지 않는다** (PKCE flow 에서는 `exchangeCodeForSession` 호출 시점에만 user 생성. 정본: Supabase Server-Side Auth + PKCE) → 302 to `<app>/auth/callback?code=<supabase_code>`.<br/>④ **Next.js server-side Route Handler `/auth/callback` (gate-before-exchange — strong gate).** SSR route handler 가 다음을 차례로 수행 (서버에서만 실행):<br/>&nbsp;&nbsp;(a) Cookie 에서 `__Host-jippin_consent_token` 추출. **cookie 없음 → returning user login 흐름** 으로 간주 → 그대로 `exchangeCodeForSession(code)` 호출 → 기존 user session 발급 → 정상 redirect. **deleteUser 호출 안 함.**<br/>&nbsp;&nbsp;(b) Cookie 있음 → **신규 signup 흐름** → backend RPC `public.validate_pending_terms_consent(p_token uuid, p_expected_provider text)` 호출 (read-only check, service_role 전용). false → cookie 삭제 + redirect to `/auth/error?reason=terms_missing` — **`exchangeCodeForSession` 호출 안 함 → `auth.users` 생성 안 함.** wake#4 의 "user 생성 후 사후 검증" 사고 차단.<br/>&nbsp;&nbsp;(c) validate true → `supabaseAdmin.auth.exchangeCodeForSession(code)` 호출 → 이 시점에만 `auth.users` row 생성. `is_new_user` 판정 = 응답의 `user.created_at` 와 직전 시도 / `identities[0].created_at` 비교 또는 Supabase 응답의 `is_new_user` 플래그.<br/>&nbsp;&nbsp;(d) `is_new_user == true` 일 때만 backend RPC `public.consume_pending_terms_consent_internal(p_token uuid, p_provider text, p_user_id uuid)` 호출 (**service_role 전용, caller-supplied term_id/terms_version 신뢰 금지** — 함수가 token 의 row 에서 term_id/terms_version 도출). 함수가 단일 transaction 으로:<br/>&nbsp;&nbsp;&nbsp;&nbsp;• 단일 conditional UPDATE 로 race-free atomic consume (`consumed_at IS NULL + expires_at > now() + provider 매칭` 조건). `GET DIAGNOSTICS ROW_COUNT = 1` 검증.<br/>&nbsp;&nbsp;&nbsp;&nbsp;• 같은 transaction 에서 `INSERT INTO public.terms_consents (user_id, term_id, version, source, agreed_at) VALUES (p_user_id, row.term_id, row.terms_version, 'internal_signup', now())`. user_id 는 `exchangeCodeForSession` 결과의 user.id (backend 가 제공 — caller-supplied param 이지만 같은 server-side handler 안에서만 사용).<br/>&nbsp;&nbsp;&nbsp;&nbsp;• 둘 중 한쪽 fail (RLS, FK, race lose) → 둘 다 rollback → 함수 false 반환. **backend 는 `auth.admin.signOut(user.id)` 만 호출 (세션 invalidate). `auth.admin.deleteUser` 는 호출하지 않는다** (wake#3 — non-signup 흐름과의 분리). 미동의 user 는 `terms_consents` 가 없는 상태로 잔존 → §7.1.6 cron cleanup 가 24h 후 비활성/삭제 정본화.<br/>&nbsp;&nbsp;(e) `is_new_user == false` (existing user) + cookie 있음 → 일반적이지 않은 케이스 (e.g. stale cookie). cookie 삭제 + 정상 login 처리. **deleteUser 호출 안 함**.<br/>**비정본 (절대 사용 금지):**<br/>(a) `signInWithOAuth({ options: { data: { consent_token } } })` — `data` 는 Supabase JS 정본 옵션 아님.<br/>(b) PKCE state 우회 — Supabase 자체 관리.<br/>(c) Hook 단계 client token 검증 — payload 에 도달 안 함.<br/>(d) **callback 의 `auth.admin.deleteUser` 호출** — wake#3 정본 위반. signup 흐름이든 login 흐름이든 deleteUser 안 함. 미동의 user 는 cron 책임.<br/>(e) **`consume_pending_terms_consent_atomic` 류의 `authenticated` GRANT** — wake#2/#JMY. caller-supplied `p_user_id/p_term_id/p_terms_version` 신뢰 금지. **service_role 전용 + 함수 내부에서 token row 에서 term_id/terms_version 도출**.<br/>(f) `exchangeCodeForSession` 호출 후 사후 검증 — wake#4 위반. **검증은 반드시 exchange 호출 전에**.<br/>**Hook 본문 (weak gate — Kakao 분기만 의미).**<br/>```sql<br/>-- private.before_user_created_terms_gate(event jsonb) RETURNS jsonb<br/>DECLARE<br/>  provider text := COALESCE(event->'user'->'app_metadata'->>'provider', event->'user'->'identities'->0->>'provider');<br/>BEGIN<br/>  IF provider = 'kakao' THEN<br/>    IF event->'user'->'identities'->0->'identity_data'->'service_terms' IS NULL THEN<br/>      RETURN jsonb_build_object('error', jsonb_build_object('http_code', 403, 'message', 'kakao_terms_missing'));<br/>    END IF;<br/>  END IF;<br/>  -- google/naver: gate-before-exchange (④) 가 strong gate. Hook 은 allow.<br/>  RETURN '{}'::jsonb;<br/>END;<br/>```<br/>**테이블 + RLS policy + RPC 권한 (wake#JMV + #JMY + #JMa + #JMb 결합 봉인).** `private.pending_terms_consents` 정의 (위 §7.1.5 H3 본문 컬럼). RLS ON + FORCE + `supabase_auth_admin` 용 SELECT/UPDATE policy 명시 (Hook 보조용). **`validate_pending_terms_consent` + `consume_pending_terms_consent_internal` 두 RPC 모두 `REVOKE EXECUTE ... FROM PUBLIC, anon, authenticated` + `GRANT EXECUTE ... TO service_role` 만** — 콜백 서버 (Next.js Route Handler 가 service_role 키로 호출) 외 channel 차단. `create_pending_terms_consent` 만 `anon`/`authenticated` 호출 허용 (terms 화면). | CMP-576 + CMP-577 + CMP-575 (RLS policy) |
| H4. **Hook + gate-before-exchange 활성화 self-check (CMP-576 PR).** 다음 PoC 결과를 staging branch 에서 캡처해 CMP-576 PR 본문에 첨부 — **모든 시나리오에서 `auth.admin.deleteUser` 호출 0건** 이어야 한다 (wake#3).<br/>**Hook (weak gate) 시나리오.**<br/>① google/naver signup (Hook 만) → Hook 응답 `{}` allow. **strong gate 는 ⑥~⑫ 의 callback 시나리오** — Hook 자체는 user 생성 시점이 아님 (PKCE: `exchangeCodeForSession` 호출 시점에 user 생성).<br/>② Kakao signup `service_terms` 있음 → Hook 응답 `{}` + Kakao 흐름은 internal terms gate 우회 (§4.7 #5) → `auth.users` row + Kakao `terms_consents (source='kakao_sync')` row 생성.<br/>③ Kakao signup `service_terms` 누락 → Hook 응답 `{"error":{"http_code":403,"message":"kakao_terms_missing"}}` + `auth.users` row 0건.<br/>④ **error 형식 negative.** 임시로 Hook 본문을 `RETURN jsonb_build_object('decision','reject','message','x')` 로 바꾸고 ③ 재실행 → `error` 필드 부재로 Supabase fail-open → `auth.users` row 생성됨 캡처 (원상복구 후 H4 통과).<br/>**Cookie `__Host-` 시나리오 (wake#1).**<br/>⑤ Cookie 설정 시 `Set-Cookie: __Host-jippin_consent_token=<token>; Path=/; Secure; HttpOnly; SameSite=Lax; Max-Age=1800` 응답 헤더 캡처. `Path=/` / `Domain` 미지정 / `Secure` 3종 모두 충족 확인. 임시로 `Path=/` 를 제거한 응답으로 PoC → 브라우저가 cookie 자체를 reject (DevTools application > cookies 에 안 나타남) 캡처 (원상복구).<br/>**Gate-before-exchange 시나리오 (google/naver `/auth/callback` Route Handler).**<br/>⑥ consent_token cookie 없음 → handler 가 returning user login 으로 간주 → `exchangeCodeForSession` 호출 → 기존 user session 발급 + redirect. `auth.users` 행 수 불변. **deleteUser 호출 0건** (wake#3).<br/>⑦ Cookie 있음 + `validate_pending_terms_consent` false (만료/provider 불일치/이미 consume) → cookie 삭제 + redirect to `/auth/error?reason=terms_missing`. **`exchangeCodeForSession` 호출 0건 → `auth.users` row 생성 0건** (wake#4 — exchange 자체를 안 함 → 미동의 user 가 만들어지는 순간 없음).<br/>⑧ Cookie 있음 + validate true → `exchangeCodeForSession(code)` 호출 → `auth.users` row 생성 + user_id 받음. `is_new_user == true` → `consume_pending_terms_consent_internal(token, provider, user_id)` 호출:<br/>&nbsp;&nbsp;• `private.pending_terms_consents.consumed_at` 채워짐<br/>&nbsp;&nbsp;• 같은 transaction 의 `public.terms_consents` 에 `(user_id, term_id, version, source='internal_signup', agreed_at)` row 1건 (wake#4)<br/>&nbsp;&nbsp;• `auth.users` 보존 (deleteUser 안 함)<br/>⑨ Cookie 있음 + validate true → exchange → `is_new_user == false` (existing user 가 어쩌다 cookie 갖고 다시 callback) → cookie 삭제 + 정상 login 진행. `consume_pending_terms_consent_internal` 호출 0건. **deleteUser 호출 0건** (wake#3).<br/>⑩ **Race-free atomic consume (wake#2).** 같은 consent_token 으로 동시 2개 signup flow 시도 — validate 가 둘 다 true 봐도 `exchangeCodeForSession` 후 `consume_pending_terms_consent_internal` 의 단일 conditional UPDATE 가 정확히 1개만 통과. 한 쪽은 false → backend 가 `auth.admin.signOut(user.id)` 만 (deleteUser 안 함, wake#3) + cron cleanup 으로 24h 후 정리. `terms_consents` row 1건만 생성됨 확인.<br/>**RPC 권한 negative (wake#JMY).**<br/>⑪ `anon` JWT 로 PostgREST `/rest/v1/rpc/validate_pending_terms_consent` / `/rpc/consume_pending_terms_consent_internal` 호출 → `permission denied for function` (service_role 만 GRANT).<br/>⑫ `authenticated` JWT 로 동일 호출 → 동일 fail. **callback 서버 (Next.js Route Handler 의 service_role) 외 channel 에서는 호출 불가**.<br/>⑬ `service_role` JWT 로 `SELECT *` / `INSERT INTO private.pending_terms_consents` 시도 → `permission denied for schema private` (RLS + GRANT 둘 다 차단).<br/>⑭ **RLS policy 누락 negative.** 임시로 `DROP POLICY pending_terms_consents_auth_admin_select` 후 PoC. Hook (weak gate) 는 `private` 접근 안 하므로 영향 없음. 미래에 Hook 본문이 SELECT 추가 시 silent 0 rows 가 됨을 입증 (PoC 시 임시 SELECT + 0 rows 캡처). 원상복구.<br/>**Sealed flow + 미동의 user cleanup (wake#JMb + #3 / §4.7 정본).**<br/>⑮ ⑦ 시나리오 (cookie validate false) 직후 `auth.users` 와 `public.terms_consents` 둘 다 row 0건 확인. 신규 user 가 생성되지 않음 = §4.7 sealed flow 충족.<br/>⑯ ⑧ 시나리오 직후 `auth.users` 1건 + `public.terms_consents` 1건 (source=`internal_signup`) 확인. user 와 terms_consents 가 같은 backend handler 안에서 생성됨 (gap < 1초). cron cleanup 미동의 user 비활성 PoC (§7.1.6 참조) 는 24h `terms_consents IS NULL` user 0건 검증.<br/>위 16개 시나리오 중 1개라도 누락된 PR 은 §3.2 A3 + 본 게이트로 reject. | CMP-576 + CMP-577 + CMP-575 (RLS policy) |

본 PR (CMP-574, 단계 0) 은 Hook + gate-before-exchange 를 직접 도입하지 않는다. CMP-576 PR 이 본 §7.1.5 H1~H4 + §7.1.6 cron 정본화를 추가한다. 마이그레이션 파일 6개를 H2 봉인 순서로 추가:<br/>① `_<ts1>_private_schema.sql`, ② `_<ts2>_pending_terms_consents.sql` (RLS policy 포함), ③ `_<ts3>_identity_link_intents.sql` (RLS policy 포함), ④ `_<ts4>_pending_terms_consent_fns.sql` (`create_pending_terms_consent` + `validate_pending_terms_consent` + `consume_pending_terms_consent_internal` 3개 함수 — service_role 만 GRANT for validate/consume), ⑤ `_<ts5>_create_identity_link_intent_fn.sql`, ⑥ `_<ts6>_before_user_created_terms_gate.sql` (weak gate — Kakao 분기 + google/naver allow). timestamp `<ts1> < <ts2> < ... < <ts6>` 강제. web 측은:<br/>&nbsp;&nbsp;• terms 화면 → server-side handler 가 `supabase.rpc('create_pending_terms_consent', ...)` 호출 + **`Set-Cookie: __Host-jippin_consent_token=<token>; Path=/; Secure; HttpOnly; SameSite=Lax; Max-Age=1800`** 응답 헤더 (wake#1 — `__Host-` prefix 규약).<br/>&nbsp;&nbsp;• `signInWithOAuth({ flowType: 'pkce', options: { redirectTo: '<app>/auth/callback', queryParams, scopes } })` 호출 (PKCE flow).<br/>&nbsp;&nbsp;• **Next.js `/auth/callback` server-side Route Handler 가 gate-before-exchange 수행** — cookie 검사 → `validate_pending_terms_consent` (service_role) → false 시 cookie 삭제 + redirect to `/auth/error` (exchange 미호출) → true 시 `exchangeCodeForSession` → `is_new_user == true` 일 때만 `consume_pending_terms_consent_internal` (service_role) 호출. **`auth.admin.deleteUser` 절대 호출 안 함** (wake#3 — 미동의 user 는 §7.1.6 cron cleanup). validate/consume 실패 시 `auth.admin.signOut` (세션 invalidate) + 에러 페이지.<br/>를 CMP-577 이 담당. §7.1.4 O4 (a) 의 PoC 첨부 의무는 본 §7.1.5 H4 결과 (16 시나리오) 로 만족한다.
...
| 게이트 | 조건 | 책임 |
|---|---|---|
| Cron1. **미동의 user 비활성/삭제 cron (MUST).** Supabase cron (`pg_cron` 또는 별도 Edge Function scheduled job) 이 다음 SQL 을 N분 주기로 실행:<br/>```sql<br/>-- 24h 이상 'internal_signup' / 'kakao_sync' terms_consents 가 없는 user 비활성.<br/>UPDATE auth.users<br/>   SET banned_until = now() + interval '100 years',<br/>       app_metadata = COALESCE(app_metadata, '{}'::jsonb) || jsonb_build_object('terms_missing_disabled_at', now())<br/> WHERE created_at < now() - interval '24 hours'<br/>   AND id NOT IN (SELECT user_id FROM public.terms_consents);<br/>```<br/>본 cron 은 `auth.admin.deleteUser` 가 아닌 ban 으로 처리해 audit trail 보존. 후속 별도 PR 에서 7일 후 deleteUser 호출 cron 추가 가능. | DevOps + CMP-575 (cron SQL) |
| Cron2. **PoC 시나리오.** staging branch 에서 §7.1.5 H4 ⑦ (validate false → exchange 미호출) 와 ⑩ (race lose → signOut) 로 미동의 user 시뮬레이션 후 cron 24h 시뮬레이션 (timestamp 강제 조정) → user `banned_until` 채워짐 + 일반 로그인 시도 시 거부 확인. CMP-576/CMP-577 PR 본문에 본 PoC 캡처 첨부. | CMP-576 + CMP-577 |
| Cron3. **monitoring alert.** 24h 미동의 user 가 N건 이상 발생 시 (cron 실행 시) Slack/email 알림. backend handler 의 race lose 또는 crash 빈도 모니터링. | DevOps |
...
git grep -nE '7\.1\.6 미동의 user|banned_until|terms_missing_disabled_at|Cron1\.' docs/runbooks/supabase-branching.md
# → §7.1.6 표 + SQL 매칭.
```
