...
- [ ] **A3.** CMP-576 (Auth 트랙) 이 `supabase/config.toml` 의 `[auth.external.*]` 를 켜고 **같은 PR 에서 `[auth].enable_signup` 글로벌 gate 를 `true` 로 함께 바꾼다** (글로벌 gate 가 false 면 OAuth signup 도 막힘). `[auth.email].enable_signup` 은 영구 false 유지. **OAuth signup gate (MUST — PR 머지 직전 봉인).** A3 PR 머지 직전에 §7.1.4 (OAuth signup activation gate) 의 O1~O5 게이트가 모두 통과 상태여야 한다 (provider 시크릿 secrets/variables 등록 + §7.4 W-A~W-D Supabase-visible env wiring 통과 (root `.env`), redirect_uri 봉인 — 환경별 `env(SUPABASE_AUTH_EXTERNAL_REDIRECT_URI)` parameterize + 4환경 (prod/dev/staging/local) provider 콘솔 등록, §7.1.1 G1~G3 자동 linking 가드 완료 (§7.1.2 M-INTENT discriminator 봉인 동행), §4.7 sealed terms flow 보장 — §7.1.5 H1~H4 (`private` schema + REVOKE FROM PUBLIC/anon/authenticated/service_role + `supabase_auth_admin` 만 EXECUTE + `signInWithOAuth({ options: { data: { consent_token } } })` 채널 + 8-시나리오 PoC) 통과). 미통과 시 PR 머지 금지. CMP-577 트랙이 `[auth].enable_anonymous_sign_ins=true` 로 전환하며 §7.1.3 PoC 절차 (A5) 를 함께 수행.
...
| H2. **권한/역할 봉인 — Hook 전용 `private` schema + `supabase_auth_admin` 만 EXECUTE (MUST).** Hook 함수와 데이터를 **`public` 이 아닌 `private` schema** (또는 동등한 hook 전용 schema — Supabase 정본 권장: https://supabase.com/docs/guides/auth/auth-hooks#developing) 에 둔다. `public` 에 두면 R3 (§7.2) 가 `[api].enabled=true` 로 바뀌는 순간 본 hook 함수가 PostgREST RPC 로 노출돼 클라이언트가 terms-gate 를 외부에서 실행/probe 할 수 있다 (`auth.users` 미생성 상태로 `consent_token` 을 consume 하거나 expiry/존재 여부 탐지 가능). Postgres 는 default 로 `PUBLIC` role 에 `EXECUTE` 를 부여하므로 GRANT 만으로 충분하지 않고 **명시 REVOKE 동행 필수**. SQL 마이그레이션 정본:<br/>```sql<br/>-- 1. Hook 전용 schema + 함수 위치.<br/>CREATE SCHEMA IF NOT EXISTS private;<br/>CREATE OR REPLACE FUNCTION private.before_user_created_terms_gate(event jsonb) RETURNS jsonb AS $$ ... $$ LANGUAGE plpgsql;<br/><br/>-- 2. 기본 PUBLIC 권한 박탈 — RPC 우회 차단.<br/>REVOKE EXECUTE ON FUNCTION private.before_user_created_terms_gate(jsonb) FROM PUBLIC;<br/>REVOKE EXECUTE ON FUNCTION private.before_user_created_terms_gate(jsonb) FROM anon, authenticated, service_role;<br/><br/>-- 3. supabase_auth_admin 만 EXECUTE.<br/>GRANT USAGE ON SCHEMA private TO supabase_auth_admin;<br/>GRANT EXECUTE ON FUNCTION private.before_user_created_terms_gate(jsonb) TO supabase_auth_admin;<br/>GRANT SELECT, UPDATE (consumed_at, provider_subject) ON TABLE private.pending_terms_consents TO supabase_auth_admin;<br/><br/>-- 4. API runtime role (anon/authenticated/service_role) 의 schema 접근 차단.<br/>REVOKE ALL ON SCHEMA private FROM anon, authenticated, service_role, PUBLIC;<br/>REVOKE ALL ON ALL TABLES IN SCHEMA private FROM anon, authenticated, service_role, PUBLIC;<br/>REVOKE ALL ON ALL FUNCTIONS IN SCHEMA private FROM anon, authenticated, service_role, PUBLIC;<br/>ALTER DEFAULT PRIVILEGES IN SCHEMA private REVOKE ALL ON TABLES FROM anon, authenticated, service_role, PUBLIC;<br/>ALTER DEFAULT PRIVILEGES IN SCHEMA private REVOKE ALL ON FUNCTIONS FROM anon, authenticated, service_role, PUBLIC;<br/>```<br/>**역할 분리 봉인.** `anon`/`authenticated`/`service_role` (API runtime) 은 **Hook 함수에 EXECUTE 권한 없음**, `private` schema 자체에 USAGE 권한 없음. terms 화면의 `INSERT INTO private.pending_terms_consents` 는 `anon` 권한이 아닌 별도 SECURITY DEFINER 함수 (`public.create_pending_terms_consent(provider, terms_version)`) 또는 FastAPI 백엔드가 service_role 로 호출하는 형태로만 허용한다 (즉 클라이언트 직접 SQL 불가). Hook 실행은 오직 Supabase Auth 가 `supabase_auth_admin` 으로 호출.<br/>**RLS 가 켜진 테이블 (§7.2 R1 이후) 에는 별도 policy** 를 `supabase_auth_admin` role 에 추가하지 않으면 SELECT 가 silent fail → hook 이 모든 signup 을 reject 한다. CMP-575 의 RLS 정본화에서 본 role 의 read policy 를 함께 봉인.<br/>**GRANT 누락 시 첫 OAuth signup 이 `permission denied` 로 fail**, REVOKE 누락 시 PostgREST 노출 시점에 hook 함수가 RPC 로 callable 이 됨 — staging PoC (§3.2 A1.2) 에서 H4 시나리오 ⑥ (GRANT 회수 후 permission denied) + ⑦ 신설 (`anon` JWT 로 PostgREST `/rpc/before_user_created_terms_gate` 호출 시도 → 404/permission denied) 양쪽 검증. | CMP-576 (Hook SQL) + CMP-575 (RLS policy) |
| H3. **pre-user terms consent intent 조회 메커니즘 (MUST — supported channel 만).** `before-user-created` Hook 은 candidate user 가 `auth.users` 에 **아직 insert 되기 전** 에 실행되므로 `users.id` 를 키로 한 조회는 불가능하다 (정본 inputs 문서: https://supabase.com/docs/guides/auth/auth-hooks/before-user-created-hook). 그리고 **Supabase 가 자동으로 hook payload 에 넘기는 metadata 는 한정** 돼 있다 — request metadata (IP, user_agent), candidate `user` payload, `user.raw_user_meta_data`/`user.app_metadata` 만 정본이다. `signInWithOAuth({ queryParams })` 의 provider query 와 `redirectTo` 의 path 는 **Supabase 가 hook payload 로 자동 복사하지 않는다** — 그대로 두면 hook 이 token 을 읽을 수 없어 모든 OAuth signup 을 reject 한다. 따라서 본 PR 의 supported channel 정본은:<br/>**Supported channel = `signInWithOAuth({ options: { data: { consent_token } } })`.** `options.data` 는 Supabase JS 정본 (https://supabase.com/docs/reference/javascript/auth-signinwithoauth) 에서 **`auth.users.raw_user_meta_data` 로 자동 저장**되도록 지원되는 유일한 채널이다. `before-user-created` hook 은 `event.user_metadata` (= `raw_user_meta_data`) 에서 본 token 을 그대로 읽을 수 있다. PKCE state / queryParams 우회는 비정본 — 사용하지 않는다.<br/>본 PR 봉인 모델:<br/>① **테이블.** CMP-576 가 `private.pending_terms_consents` 를 SQL 마이그레이션으로 정의 (`private` schema 는 본 PR 의 §7.1.5 H2-revised 봉인 — `public` 노출 회피). 컬럼: `consent_token uuid PRIMARY KEY DEFAULT gen_random_uuid()`, `provider text NOT NULL` (google/kakao/naver), `terms_version text NOT NULL`, `accepted_at timestamptz NOT NULL DEFAULT now()`, `expires_at timestamptz NOT NULL` (e.g. now() + 30 분), `provider_subject text NULL` (OAuth 콜백 시 채워짐), `consumed_at timestamptz NULL`. RLS ON, anon 은 자기 row INSERT 만, `supabase_auth_admin` (Hook 실행 role) 은 SELECT/UPDATE.<br/>② **흐름.** (1) app 의 internal terms 화면에서 사용자가 동의 누르면 `INSERT INTO private.pending_terms_consents (provider, terms_version) RETURNING consent_token`. (2) web 이 `consent_token` 을 받아 OAuth signin 시 **`supabase.auth.signInWithOAuth({ provider, options: { redirectTo, data: { consent_token: <token> } } })`** 로 호출. Supabase Auth 가 본 `data` 를 candidate user 의 `raw_user_meta_data` 로 보존한다 (CMP-577 web 책임 — 명시적 `data:` 옵션 사용, queryParams/state 우회 금지). (3) Supabase 가 OAuth 콜백 처리 후 `before-user-created` hook 을 부르며 `event.user_metadata.consent_token` 으로 token 이 들어온다. (4) Hook 본문:<br/>```sql<br/>-- private.before_user_created_terms_gate(event jsonb)<br/>DECLARE token uuid := (event->'user_metadata'->>'consent_token')::uuid;<br/>BEGIN<br/>  IF NOT EXISTS (<br/>    SELECT 1 FROM private.pending_terms_consents<br/>    WHERE consent_token = token<br/>      AND provider = event->'identity'->>'provider'<br/>      AND consumed_at IS NULL<br/>      AND expires_at > now()<br/>  ) THEN RETURN jsonb_build_object('decision', 'reject', 'message', 'terms_not_accepted');<br/>  END IF;<br/>  UPDATE private.pending_terms_consents SET consumed_at = now(), provider_subject = event->'identity'->>'provider_id' WHERE consent_token = token;<br/>  RETURN jsonb_build_object('decision', 'continue');<br/>END;<br/>```<br/>③ **expiry/회수.** 30분 expiry + consumed_at 단방향 update + cron 으로 7일 이상 row 삭제 (`pg_cron` extension 또는 별도 cleanup job). consent_token 재사용 시 같은 user 가 같은 provider 로 재시도해도 reject.<br/>④ **Naver custom OAuth/OIDC** 처럼 provider_subject 가 콜백 시점에야 결정되는 흐름은 (4) 의 token 검증과 동시에 `provider_subject` 를 row 에 UPDATE 해 audit trail 보존.<br/>**비정본 (절대 사용 금지):** `signInWithOAuth({ options: { queryParams: { state: ... } } })` 의 state 가 user_metadata 로 자동 복사된다고 가정하는 구현. PKCE state 는 Supabase 가 자체 관리하며 hook payload 에 노출하지 않는다. | CMP-576 + CMP-577 |
| H4. **Hook 활성화 self-check (CMP-576 PR).** 다음 PoC 결과를 staging branch 에서 캡처해 CMP-576 PR 본문에 첨부:<br/>① consent_token 없이 OAuth signup → Hook reject + `auth.users` row 0건 생성<br/>② 만료된 consent_token → Hook reject + row 0건<br/>③ 유효 consent_token → Hook continue + `auth.users` row 생성 + `private.pending_terms_consents.consumed_at` 채워짐<br/>④ 같은 consent_token 재사용 → Hook reject (consumed_at IS NOT NULL)<br/>⑤ provider 불일치 (google token 으로 kakao signup) → Hook reject<br/>⑥ `supabase_auth_admin` 의 GRANT 회수 후 ③ 재실행 → permission denied 로 fail (negative 검증)<br/>⑦ **RPC 우회 negative.** `anon` JWT 로 PostgREST `/rest/v1/rpc/before_user_created_terms_gate` 호출 시도 (또는 `private` schema 노출 후) → 404 / `permission denied for function` / schema 노출 안 됨 — 본 시나리오 통과 = Hook 함수가 외부 RPC 로 callable 하지 않음을 입증<br/>⑧ **API runtime role 차단 negative.** `service_role` JWT 로 `SELECT * FROM private.pending_terms_consents` 시도 → `permission denied for schema private` — schema 자체에 USAGE 없음을 입증<br/>위 8개 시나리오 중 1개라도 누락된 PR 은 §3.2 A3 + 본 게이트로 reject. | CMP-576 + CMP-577 |

본 PR (CMP-574, 단계 0) 은 Hook 자체를 도입하지 않는다. CMP-576 PR 이 본 §7.1.5 H1~H4 를 정본화해 `supabase/migrations/<ts>_private_schema_terms_gate.sql` (schema + REVOKE + GRANT 봉인) + `supabase/migrations/<ts>_pending_terms_consents.sql` + `supabase/migrations/<ts>_before_user_created_hook.sql` 을 추가하고, web 측 token 전파 (`signInWithOAuth({ options: { data: { consent_token } } })`) 는 CMP-577 이 담당. §7.1.4 O4 (a) 의 PoC 첨부 의무는 본 §7.1.5 H4 결과 (8 시나리오) 로 만족한다.
