...
- [ ] **A3.** CMP-576 (Auth 트랙) 이 `supabase/config.toml` 의 `[auth.external.*]` 를 켜고 **같은 PR 에서 `[auth].enable_signup` 글로벌 gate 를 `true` 로 함께 바꾼다** (글로벌 gate 가 false 면 OAuth signup 도 막힘). `[auth.email].enable_signup` 은 영구 false 유지. **OAuth signup gate (MUST — PR 머지 직전 봉인).** A3 PR 머지 직전에 §7.1.4 (OAuth signup activation gate) 의 O1~O5 게이트가 모두 통과 상태여야 한다 (provider 시크릿 secrets/variables 등록 + §7.4 W-A~W-D Supabase-visible env wiring 통과 (root `.env`), redirect_uri 봉인 — 환경별 `env(SUPABASE_AUTH_EXTERNAL_REDIRECT_URI)` parameterize + 4환경 (prod/dev/staging/local) provider 콘솔 등록, §7.1.1 G1~G3 자동 linking 가드 완료 (§7.1.2 M-INTENT discriminator 봉인 동행 — insert 는 SECURITY DEFINER 함수만), §4.7 sealed terms flow 보장 — §7.1.5 H1~H4 (`private` schema + 마이그레이션 파일 ①~⑥ 봉인 순서 + RLS policy 명시 (`supabase_auth_admin` 용 SELECT/UPDATE) + Hook 은 **weak gate** (Kakao 분기 + google/naver allow) + strong gate 는 **app `/auth/callback` 의 `consume_pending_terms_consent_atomic` 단일 conditional UPDATE 가 race-free atomic consume + 같은 transaction 의 `terms_consents` (`source='internal_signup'`) 영속화 + false 시 즉시 `auth.admin.deleteUser`** + `signInWithOAuth({ options: { redirectTo: ... + queryParams } })` 정본 옵션 사용 (data/state 우회 금지) + `__Host-jippin_consent_token` cookie CSRF 매칭 + `{}` allow / `{"error":{...}}` reject Supabase 정본 응답 형식 + Kakao Sync 분기 + 12-시나리오 PoC) 통과). 미통과 시 PR 머지 금지. CMP-577 트랙이 `[auth].enable_anonymous_sign_ins=true` 로 전환하며 §7.1.3 PoC 절차 (A5) 를 함께 수행.
...
| H2. **권한/역할 봉인 — Hook 전용 `private` schema + `supabase_auth_admin` 만 EXECUTE + SECURITY DEFINER insert 채널 (MUST).** Hook 함수와 데이터를 **`public` 이 아닌 `private` schema** (Supabase 정본 권장: https://supabase.com/docs/guides/auth/auth-hooks#developing) 에 둔다. `public` 에 두면 R3 (§7.2) 가 `[api].enabled=true` 로 바뀌는 순간 hook 함수가 PostgREST RPC 로 노출돼 클라이언트가 terms-gate 를 외부 실행/probe 할 수 있다. Postgres 는 default 로 `PUBLIC` role 에 `EXECUTE` 를 부여하므로 GRANT 만으로 충분하지 않고 **명시 REVOKE 동행 필수**.<br/>**마이그레이션 순서 봉인 (MUST — 객체 생성 → GRANT/REVOKE 순서).** Postgres GRANT/REVOKE/POLICY 는 대상 객체가 이미 존재해야 실행 가능. 따라서 마이그레이션 파일은 다음 순서로 분리:<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ①** `_<ts>_private_schema.sql` — `CREATE SCHEMA IF NOT EXISTS private` + `private_owner` role (`BYPASSRLS NOLOGIN`) 생성 + 스키마 default `REVOKE ALL ... FROM anon, authenticated, service_role, PUBLIC` + `ALTER DEFAULT PRIVILEGES`.<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ②** `_<ts>_pending_terms_consents.sql` — 테이블 정의 (§7.1.5 H3 컬럼) + `ALTER TABLE ... OWNER TO private_owner` + `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` + `ALTER TABLE ... FORCE ROW LEVEL SECURITY` + REVOKE/GRANT + **RLS policy 명시** (`supabase_auth_admin` 용 SELECT/UPDATE policy — wake#3 봉인).<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ③** `_<ts>_identity_link_intents.sql` — 동일 패턴 (테이블 + RLS + RLS policy + GRANT/REVOKE).<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ④** `_<ts>_create_pending_terms_consent_fn.sql` + `consume_pending_terms_consent_atomic_fn.sql` — SECURITY DEFINER 함수 2개:<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;• `public.create_pending_terms_consent(p_provider text, p_terms_version text) RETURNS uuid` — terms 화면이 호출, `consent_token` 발급.<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;• `public.consume_pending_terms_consent_atomic(p_token uuid, p_provider text, p_provider_subject text, p_user_id uuid, p_term_id uuid, p_terms_version text) RETURNS boolean` — app `/auth/callback` 이 호출. 본 함수가 **단일 conditional UPDATE + terms_consents INSERT 를 동일 transaction 으로 atomic 수행** (wake#2 봉인). owner = `private_owner` (`private` schema 접근권 + RLS bypass).<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ⑤** `_<ts>_create_identity_link_intent_fn.sql` — M-INTENT 발급 SECURITY DEFINER 함수.<br/>&nbsp;&nbsp;&nbsp;&nbsp;**파일 ⑥** `_<ts>_before_user_created_terms_gate.sql` — Hook 함수 (weak gate — Kakao 분기 + google/naver allow) + 함수 자체 `REVOKE EXECUTE FROM PUBLIC, anon, authenticated, service_role` + `GRANT EXECUTE ... TO supabase_auth_admin`.<br/>위 순서를 깨서 파일 ①·② 의 GRANT/POLICY 가 테이블 생성보다 먼저 실행되면 `relation "private.pending_terms_consents" does not exist` 로 fail. CMP-576 PR 본문에 본 파일 순서 캡처 (파일명 timestamp 정렬) 첨부 의무.<br/>**파일 ②~⑥ 의 표준 SQL 블록 (의미만 — 실 SQL 은 CMP-576 PR 정본).**<br/>```sql<br/>-- 파일 ① (private schema 봉인).<br/>CREATE SCHEMA IF NOT EXISTS private;<br/>CREATE ROLE private_owner NOLOGIN BYPASSRLS;  -- SECURITY DEFINER 함수 owner + RLS bypass.<br/>REVOKE ALL ON SCHEMA private FROM PUBLIC, anon, authenticated, service_role;<br/>GRANT USAGE ON SCHEMA private TO private_owner, supabase_auth_admin;<br/>ALTER DEFAULT PRIVILEGES IN SCHEMA private REVOKE ALL ON TABLES   FROM PUBLIC, anon, authenticated, service_role;<br/>ALTER DEFAULT PRIVILEGES IN SCHEMA private REVOKE ALL ON FUNCTIONS FROM PUBLIC, anon, authenticated, service_role;<br/><br/>-- 파일 ② (테이블 + RLS + policy + 권한).<br/>CREATE TABLE private.pending_terms_consents (...);<br/>ALTER TABLE  private.pending_terms_consents OWNER TO private_owner;<br/>ALTER TABLE  private.pending_terms_consents ENABLE ROW LEVEL SECURITY;<br/>ALTER TABLE  private.pending_terms_consents FORCE ROW LEVEL SECURITY;<br/>REVOKE ALL   ON TABLE private.pending_terms_consents FROM PUBLIC, anon, authenticated, service_role;<br/>GRANT  SELECT, UPDATE (consumed_at, provider_subject) ON TABLE private.pending_terms_consents TO supabase_auth_admin;<br/>-- RLS policy 명시 (wake#3 — GRANT 만으로 RLS bypass 안 됨).<br/>CREATE POLICY pending_terms_consents_auth_admin_select<br/>  ON private.pending_terms_consents FOR SELECT TO supabase_auth_admin USING (true);<br/>CREATE POLICY pending_terms_consents_auth_admin_update<br/>  ON private.pending_terms_consents FOR UPDATE TO supabase_auth_admin USING (true) WITH CHECK (true);<br/>-- private_owner (SECURITY DEFINER 함수 owner) 는 BYPASSRLS 로 별도 policy 불요.<br/>-- anon/authenticated/service_role 는 policy 없음 — 모든 row 차단.<br/><br/>-- 파일 ④ (insert SECURITY DEFINER + atomic consume SECURITY DEFINER).<br/>CREATE OR REPLACE FUNCTION public.create_pending_terms_consent(p_provider text, p_terms_version text)<br/>  RETURNS uuid<br/>  LANGUAGE plpgsql<br/>  SECURITY DEFINER<br/>  SET search_path = private, public<br/>AS $$<br/>DECLARE new_token uuid;<br/>BEGIN<br/>  IF p_provider NOT IN ('google','naver') THEN  -- Kakao 제외 (AGENTS.md §4.7 #5).<br/>    RAISE EXCEPTION 'provider not eligible for internal terms gate: %', p_provider;<br/>  END IF;<br/>  INSERT INTO private.pending_terms_consents (provider, terms_version, expires_at)<br/>    VALUES (p_provider, p_terms_version, now() + interval '30 minutes')<br/>    RETURNING consent_token INTO new_token;<br/>  RETURN new_token;<br/>END $$;<br/>ALTER FUNCTION public.create_pending_terms_consent(text, text) OWNER TO private_owner;<br/>REVOKE EXECUTE ON FUNCTION public.create_pending_terms_consent(text, text) FROM PUBLIC;<br/>GRANT  EXECUTE ON FUNCTION public.create_pending_terms_consent(text, text) TO anon, authenticated;<br/><br/>-- atomic consume — race-free + terms_consents 영속화 같은 transaction (wake#2 + wake#4).<br/>CREATE OR REPLACE FUNCTION public.consume_pending_terms_consent_atomic(<br/>  p_token uuid, p_provider text, p_provider_subject text,<br/>  p_user_id uuid, p_term_id uuid, p_terms_version text<br/>) RETURNS boolean<br/>  LANGUAGE plpgsql<br/>  SECURITY DEFINER<br/>  SET search_path = private, public<br/>AS $$<br/>DECLARE consumed_count int;<br/>BEGIN<br/>  -- 단일 conditional UPDATE — race-free atomic consume (wake#2 봉인).<br/>  UPDATE private.pending_terms_consents<br/>     SET consumed_at = now(), provider_subject = p_provider_subject<br/>   WHERE consent_token = p_token<br/>     AND provider = p_provider<br/>     AND consumed_at IS NULL<br/>     AND expires_at > now();<br/>  GET DIAGNOSTICS consumed_count = ROW_COUNT;<br/>  IF consumed_count <> 1 THEN<br/>    RETURN false;  -- caller (Next callback) 가 deleteUser + 에러 처리.<br/>  END IF;<br/>  -- 같은 transaction — terms_consents 영속화 (wake#4 봉인 — AGENTS.md §4.7 정본).<br/>  INSERT INTO public.terms_consents (user_id, term_id, version, source, agreed_at)<br/>    VALUES (p_user_id, p_term_id, p_terms_version, 'internal_signup', now());<br/>  RETURN true;<br/>END $$;<br/>ALTER FUNCTION public.consume_pending_terms_consent_atomic(uuid, text, text, uuid, uuid, text) OWNER TO private_owner;<br/>REVOKE EXECUTE ON FUNCTION public.consume_pending_terms_consent_atomic(uuid, text, text, uuid, uuid, text) FROM PUBLIC;<br/>GRANT  EXECUTE ON FUNCTION public.consume_pending_terms_consent_atomic(uuid, text, text, uuid, uuid, text) TO authenticated, service_role;<br/>-- (Next callback 은 authenticated 또는 service_role JWT 로 호출.)<br/><br/>-- 파일 ⑥ (Hook 함수 weak gate + REVOKE/GRANT).<br/>CREATE OR REPLACE FUNCTION private.before_user_created_terms_gate(event jsonb) RETURNS jsonb<br/>  LANGUAGE plpgsql<br/>AS $$ ... (H3 본문 — Kakao 분기 + google/naver allow) $$;<br/>REVOKE EXECUTE ON FUNCTION private.before_user_created_terms_gate(jsonb) FROM PUBLIC, anon, authenticated, service_role;<br/>GRANT  EXECUTE ON FUNCTION private.before_user_created_terms_gate(jsonb) TO supabase_auth_admin;<br/>```<br/>**역할 분리 봉인 (MUST — 모순 제거).** API runtime role (`anon`/`authenticated`/`service_role`) 은 **`private` schema USAGE 권한 없음 + 테이블/함수 EXECUTE 권한 없음**. 따라서:<br/>&nbsp;&nbsp;• terms 화면의 `private.pending_terms_consents` row insert 는 **`public.create_pending_terms_consent(...)` SECURITY DEFINER 함수 호출이 유일한 정본 경로** (owner `private_owner` 가 schema 접근권 위임). client (anon) 또는 server (authenticated/service_role) 둘 다 본 함수만 호출.<br/>&nbsp;&nbsp;• **`service_role` 직접 `INSERT INTO private.pending_terms_consents` 는 비정본** — REVOKE 로 차단. 백엔드도 본 함수를 호출한다.<br/>&nbsp;&nbsp;• Hook 함수 실행은 Supabase Auth 가 `supabase_auth_admin` 으로 호출하는 경로만.<br/>**RLS 가 켜진 테이블 (§7.2 R1 이후) 에는 별도 policy** 를 `supabase_auth_admin` role 에 추가하지 않으면 SELECT 가 silent fail → hook 이 모든 signup 을 reject 한다. CMP-575 의 RLS 정본화에서 본 role 의 read policy 를 함께 봉인.<br/>**GRANT 누락 시 첫 OAuth signup 이 `permission denied` 로 fail**, REVOKE 누락 시 PostgREST 노출 시점에 hook 함수가 RPC 로 callable. staging PoC (§3.2 A1.2) 에서 H4 시나리오 ⑥ (GRANT 회수 후 permission denied) + ⑦ (RPC 우회 negative) + ⑧ (service_role 직접 SELECT/INSERT 실패) 모두 검증. | CMP-576 (Hook SQL) + CMP-575 (RLS policy) |
| H3. **pre-user terms consent intent 조회 메커니즘 (MUST — supported channel: callback-after-insert 패턴).** `before-user-created` Hook 은 candidate user 가 `auth.users` 에 **아직 insert 되기 전** 에 실행되지만, **Supabase OAuth flow 는 Hook payload 로 client-side consent_token 을 전달할 supported channel 이 없다** (정본: https://supabase.com/docs/reference/javascript/auth-signinwithoauth — `signInWithOAuth` 의 정본 options 는 `queryParams`, `redirectTo`, `scopes` 만이며 `data` 는 unsupported. PKCE state 는 Supabase 가 자체 관리하고 hook payload 에 노출하지 않는다). 따라서 Hook 안에서 client 가 발급받은 `consent_token` 을 검증하려는 시도는 모두 NULL 을 읽거나 옵션이 무시되어 hook 이 모든 OAuth signup 을 reject 한다. 본 PR 봉인 정본은 **callback-after-insert + 즉시 정리** 패턴이다:<br/>**정본 흐름 (Google/Naver internal terms).**<br/>① **terms 동의.** terms 화면에서 사용자 동의 → client 가 `supabase.rpc('create_pending_terms_consent', { p_provider: 'google', p_terms_version: '<ver>' })` 호출 → `consent_token` 반환.<br/>② **consent_token 전파 — `redirectTo` query (supported channel).** web 이 `signInWithOAuth({ provider, options: { redirectTo: 'http://localhost:3000/auth/callback?consent_token=' + token + '&csrf=' + csrfNonce, queryParams: { ... }, scopes: '...' } })` 호출. `redirectTo` 는 Supabase 정본 옵션이고 Supabase 가 OAuth 완료 후 본 URL 로 사용자를 redirect 한다 — 즉 **app callback URL 의 query string 으로 token 이 brower 로 돌아온다**. 동시에 web 은 **HttpOnly Secure SameSite=Lax cookie `__Host-jippin_consent_token=<token>`** 도 설정해서 CSRF 검증용 이중 채널을 확보.<br/>③ **OAuth 진행 + auth.users 영속 (Hook 통과).** provider → Supabase Auth `/auth/v1/callback` → Supabase 가 `auth.users` row insert → 302 to `redirectTo` (= app `/auth/callback?consent_token=...`). 이 시점의 `before-user-created` Hook (H3-HOOK) 은 **약한 weak gate** 만 수행:<br/>&nbsp;&nbsp;• `provider = 'kakao'` → §4.7 #5 분기 — Kakao Sync `identity_data.service_terms` 존재 확인. 누락 시 reject.<br/>&nbsp;&nbsp;• `provider ∈ {'google','naver'}` → 항상 allow (`{}`). 실제 차단은 ④ 의 app callback 에서.<br/>④ **app callback — terms_consents 영속화 + 미동의 시 즉시 삭제 (강한 strong gate).** Next.js `/auth/callback` route 가 다음을 **단일 DB transaction** 으로 수행:<br/>```sql<br/>-- atomic consume (race-free single UPDATE — wake#2 봉인).<br/>UPDATE private.pending_terms_consents<br/>   SET consumed_at = now(),<br/>       provider_subject = $3<br/> WHERE consent_token = $1<br/>   AND provider = $2<br/>   AND consumed_at IS NULL<br/>   AND expires_at > now()<br/>RETURNING consent_token, terms_version;<br/>-- IF NOT FOUND → callback 이 즉시 auth.admin.deleteUser($user_id) + cookie 정리 + 에러 페이지.<br/>-- IF FOUND → 같은 transaction 에서:<br/>INSERT INTO public.terms_consents (user_id, term_id, version, source, agreed_at)<br/>SELECT $user_id, t.term_id, $terms_version, 'internal_signup', now()<br/>FROM public.terms t WHERE t.is_active;<br/>```<br/>app callback 은 `consent_token` 을 **(a) URL query** 와 **(b) `__Host-jippin_consent_token` cookie** 둘 다에서 읽어 매치해야 통과 (CSRF). 매치 실패도 즉시 deleteUser + 에러.<br/>**callback 실패 = user 삭제 + 에러** 가 strong gate. terms_consents 영속 + atomic consume 이 같은 transaction 이므로 한쪽이 fail 하면 둘 다 rollback (terms_consents 미작성 user 가 잔존하지 않음). consumed_at IS NULL + expires_at > now() 조건이 단일 UPDATE 의 WHERE 절에 있으므로 concurrent replay 시 **둘 중 하나만 RETURNING row 를 받고 다른 하나는 0 rows → 두 번째 callback 이 deleteUser 호출**.<br/>**비정본 (절대 사용 금지):**<br/>(a) `signInWithOAuth({ options: { data: { consent_token } } })` — `data` 는 Supabase JS 정본 옵션 아님. 무시되거나 ESLint/타입 에러.<br/>(b) `signInWithOAuth({ options: { queryParams: { state: ... } } })` 의 state — PKCE state 는 Supabase 자체 관리, hook payload 에 노출 안 됨.<br/>(c) Hook 단계에서 `event->'user'->'user_metadata'->>'consent_token'` 으로 token 읽기 — 위 (a)/(b) 의 channel 이 hook payload 에 도달하지 않으므로 NULL. Hook 은 Kakao 분기와 weak gate 만 담당.<br/>(d) callback 에서 EXISTS+UPDATE 분리 패턴 — race condition. 위 ④ 의 단일 conditional UPDATE 만 정본.<br/>(e) `terms_consents` 영속화 누락 — strong gate fail. AGENTS.md §4.7 정본의 `terms_consents(user_id, term_id, version, source='internal_signup', agreed_at)` row 가 같은 callback transaction 안에서 반드시 작성된다.<br/>**Hook 본문 (H3-HOOK — weak gate 만).**<br/>```sql<br/>-- private.before_user_created_terms_gate(event jsonb) RETURNS jsonb<br/>DECLARE<br/>  provider text := COALESCE(event->'user'->'app_metadata'->>'provider', event->'user'->'identities'->0->>'provider');<br/>BEGIN<br/>  -- Kakao Sync: 자체 약관 화면 사용 (AGENTS.md §4.7 #5).<br/>  IF provider = 'kakao' THEN<br/>    IF event->'user'->'identities'->0->'identity_data'->'service_terms' IS NULL THEN<br/>      RETURN jsonb_build_object('error', jsonb_build_object('http_code', 403, 'message', 'kakao_terms_missing'));<br/>    END IF;<br/>  END IF;<br/>  -- google/naver: strong gate 는 app /auth/callback. Hook 은 항상 allow.<br/>  RETURN '{}'::jsonb;<br/>END;<br/>```<br/>**테이블 + insert 경로.** `private.pending_terms_consents` 정의: `consent_token uuid PRIMARY KEY DEFAULT gen_random_uuid()`, `provider text NOT NULL CHECK (provider IN ('google','naver'))` (Kakao 제외), `terms_version text NOT NULL`, `accepted_at timestamptz NOT NULL DEFAULT now()`, `expires_at timestamptz NOT NULL` (e.g. now() + 30 분), `provider_subject text NULL`, `consumed_at timestamptz NULL`, `nonce text NOT NULL DEFAULT encode(gen_random_bytes(16),'hex')` (CSRF cookie 매칭용 추가 marker). Insert 경로는 §7.1.5 H2 봉인 — SECURITY DEFINER 함수만 (`service_role` 직접 INSERT 차단).<br/>**RLS policy (필수 — §7.2 R1 RLS ON 이후 정본).** `private.pending_terms_consents` 의 RLS ON 상태에서 GRANT 만으로는 RLS bypass 안 됨. CMP-575 RLS 정본화에서 다음 policy 를 함께 봉인 (§7.1.5 H2 마이그레이션 ②/③ 파일에 동행):<br/>```sql<br/>ALTER TABLE private.pending_terms_consents FORCE ROW LEVEL SECURITY;<br/>-- (1) SECURITY DEFINER 함수의 owner role (private_owner) 은 ALTER TABLE OWNER + BYPASSRLS 로 우회.<br/>-- (2) supabase_auth_admin 은 hook 에서 SELECT/UPDATE 필요 — 명시 policy:<br/>CREATE POLICY pending_terms_consents_auth_admin_select<br/>  ON private.pending_terms_consents<br/>  FOR SELECT TO supabase_auth_admin USING (true);<br/>CREATE POLICY pending_terms_consents_auth_admin_update<br/>  ON private.pending_terms_consents<br/>  FOR UPDATE TO supabase_auth_admin USING (true) WITH CHECK (true);<br/>-- (3) anon/authenticated/service_role 은 policy 없음 — RLS 가 모든 row 차단.<br/>```<br/>app `/auth/callback` 의 atomic UPDATE 는 FastAPI server 가 `private_owner` BYPASSRLS owner 또는 별도 SECURITY DEFINER 함수 (`public.consume_pending_terms_consent_atomic(token, provider, provider_subject)`) 로 호출한다 (§7.1.5 H2 정본 — service_role 직접 SQL 차단). | CMP-576 + CMP-577 + CMP-575 (RLS policy) |
| H4. **Hook + callback strong gate 활성화 self-check (CMP-576 PR).** 다음 PoC 결과를 staging branch 에서 캡처해 CMP-576 PR 본문에 첨부.<br/>**Hook (weak gate) 시나리오.**<br/>① google/naver signup (Hook 만) → Hook 응답 `{}` (allow) + `auth.users` row 생성. **strong gate 는 ⑤~⑪ 의 callback 시나리오**.<br/>② Kakao signup `service_terms` 있음 → Hook 응답 `{}` + `auth.users` row 생성.<br/>③ Kakao signup `service_terms` 누락 → Hook 응답 `{"error":{"http_code":403,"message":"kakao_terms_missing"}}` + row 0건.<br/>④ **error 형식 negative.** 임시로 Hook 본문을 `RETURN jsonb_build_object('decision','reject','message','x')` 로 바꾸고 ③ 재실행 → `error` 필드 부재로 Supabase 가 fail-open → `auth.users` row 생성됨을 캡처 (원상 복구 후 H4 통과).<br/>**Callback strong gate 시나리오 (google/naver — `/auth/callback` 라우트 진입 후).**<br/>⑤ consent_token 없이 / cookie mismatch / 만료 → callback 이 `consume_pending_terms_consent_atomic` 호출 → false 반환 → callback 이 `auth.admin.deleteUser($user_id)` 호출 → `auth.users` row 0건 (Hook 통과 후 즉시 삭제) + `terms_consents` 0건 + 사용자에게 에러 페이지.<br/>⑥ 유효 consent_token + cookie 일치 → `consume_pending_terms_consent_atomic` 단일 transaction 으로:<br/>&nbsp;&nbsp;• `private.pending_terms_consents.consumed_at` + `provider_subject` 채워짐<br/>&nbsp;&nbsp;• `public.terms_consents` 에 `(user_id, term_id, version, source='internal_signup', agreed_at)` row 생성 (wake#4 봉인 정본)<br/>&nbsp;&nbsp;• `auth.users` row 보존<br/>⑦ **Atomic consume race-free (wake#2 negative).** 같은 consent_token 으로 동시 2개 callback request 발사 (pgbench 또는 async ×2). 둘 중 정확히 1개만 `consume_pending_terms_consent_atomic` 이 true 반환 + terms_consents row 1건. 다른 1개는 false 반환 + 즉시 deleteUser. concurrent 두 user 가 모두 생성되거나 terms_consents 2건 생성되면 H4 fail.<br/>⑧ provider 불일치 → false + deleteUser.<br/>⑨ 같은 consent_token 재사용 (sequential 두 번째 호출) → false + deleteUser.<br/>**권한/RPC negative.**<br/>⑩ **RPC 우회.** `anon` JWT 로 PostgREST `/rest/v1/rpc/before_user_created_terms_gate` 호출 → 404 / permission denied. `anon` JWT 로 `consume_pending_terms_consent_atomic` 호출 → permission denied (authenticated/service_role 만 GRANT).<br/>⑪ **`service_role` 직접 write 차단.** `service_role` JWT 로 `SELECT *` / `INSERT INTO private.pending_terms_consents` 시도 → `permission denied for schema private` (모두 실패) — RLS policy 가 supabase_auth_admin 만 허용 + `private` schema USAGE 없음.<br/>⑫ **RLS policy 누락 negative (wake#3).** 임시로 `DROP POLICY pending_terms_consents_auth_admin_select` 후 google signup → Hook 자체는 `{}` 응답하지만 callback 의 `consume_pending_terms_consent_atomic` 이 SECURITY DEFINER owner=private_owner BYPASSRLS 로 통과. 그러나 만약 Hook 본문이 향후 변경되어 SELECT 를 시도하면 silent 0 rows 가 됨을 입증 (PoC 시 임시 SELECT 추가 후 0 rows 캡처). 원상복구 후 H4 통과.<br/>위 12개 시나리오 중 1개라도 누락된 PR 은 §3.2 A3 + 본 게이트로 reject. | CMP-576 + CMP-577 + CMP-575 (RLS policy) |

본 PR (CMP-574, 단계 0) 은 Hook + callback strong gate 를 직접 도입하지 않는다. CMP-576 PR 이 본 §7.1.5 H1~H4 를 정본화해 다음 6개 SQL 마이그레이션을 H2 봉인 순서대로 추가한다:<br/>① `_<ts1>_private_schema.sql`, ② `_<ts2>_pending_terms_consents.sql` (RLS policy 포함), ③ `_<ts3>_identity_link_intents.sql` (RLS policy 포함), ④ `_<ts4>_pending_terms_consent_fns.sql` (`create_pending_terms_consent` + `consume_pending_terms_consent_atomic` 2개 함수), ⑤ `_<ts5>_create_identity_link_intent_fn.sql`, ⑥ `_<ts6>_before_user_created_terms_gate.sql` (weak gate — Kakao 분기 + google/naver allow). timestamp `<ts1> < <ts2> < ... < <ts6>` 이 자동으로 강제되도록 PR 작성 시 동일 타임라인 사용. web 측은:<br/>&nbsp;&nbsp;• terms 화면 `supabase.rpc('create_pending_terms_consent', ...)` 호출,<br/>&nbsp;&nbsp;• `signInWithOAuth({ options: { redirectTo: '<app callback>?consent_token=...&csrf=...', queryParams, scopes } })` 호출 + `__Host-jippin_consent_token` cookie 설정,<br/>&nbsp;&nbsp;• `/auth/callback` route 가 query+cookie 매치 확인 후 `supabase.rpc('consume_pending_terms_consent_atomic', ...)` 호출 → false 시 `auth.admin.deleteUser` + 에러,<br/>를 CMP-577 이 담당. §7.1.4 O4 (a) 의 PoC 첨부 의무는 본 §7.1.5 H4 결과 (12 시나리오) 로 만족한다.
