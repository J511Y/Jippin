...
| **M-INTENT. Manual link intent marker (MUST — §7.1.1 G2 discriminator).** Manual `linkIdentity()` (M1) / anonymous→OAuth upgrade (§7.1.3 A2) 호출 **직전 N초** (예: 5분) 안에 `private.identity_link_intents (intent_id uuid PK DEFAULT gen_random_uuid(), user_id uuid NOT NULL REFERENCES auth.users(id), provider text NOT NULL, created_at timestamptz NOT NULL DEFAULT now(), expires_at timestamptz NOT NULL DEFAULT now() + interval '5 minutes', consumed_at timestamptz NULL)` 에 row INSERT (현재 인증된 user 의 user_id + 추가할 provider).<br/>**관측 가능한 discriminator (wake R11 #2 — nonce 비의존).** §7.1.1 G2 트리거가 `auth.identities` BEFORE INSERT 시점에 `auth.uid()` (현재 session user — manual link 시 인증된 user) + `NEW.provider` 매칭의 active intent (consumed_at IS NULL + expires_at > now()) 가 존재하는지 검사. 매칭 있으면 manual link → 허용 + `UPDATE consumed_at`. 매칭 없으면 automatic attach 로 간주 → reject. **nonce 검증은 사용하지 않는다** — Supabase OAuth flow 는 client-supplied nonce 를 `auth.identities` row 에 전파하는 supported channel 이 없으므로 (PKCE state 는 Supabase 자체 관리, hook payload 에 노출 안 됨). 따라서 정본 discriminator 는 `(auth.uid(), NEW.provider, active intent 존재)` 3종 매칭. 관측 가능한 이유:<br/>&nbsp;&nbsp;• `auth.uid()` = 트리거 컨텍스트의 현재 session user 가 `linkIdentity()` 호출자와 일치 (인증된 user 가 자기 자신에게 identity 추가).<br/>&nbsp;&nbsp;• `NEW.provider` = `auth.identities` insert row 의 provider 컬럼 — 트리거가 직접 접근.<br/>&nbsp;&nbsp;• `expires_at > now()` 시간 윈도우 (5분) 가 stale provider intent 의 자동 attach 우회 차단 — sufficiently small window.<br/>**한계**: 같은 user 가 5분 안에 manual link + automatic attach 시도 시 트리거가 두 경로를 구분 못 함 (둘 다 같은 user_id+provider). 본 케이스는 §7.1.1 G3 PoC (Google + Kakao 같은 verified email → 별도 user) 가 별도 가드 — automatic attach 는 다른 user_id 에서 일어나므로 본 case 에선 `auth.uid()` 매칭 실패.<br/>RLS ON, `private` schema (§7.1.5 H2 동일 권한 모델). **insert 는 `public.create_identity_link_intent(p_provider text)` SECURITY DEFINER 함수 호출만 허용** (owner = `private_owner`). 함수 본문이 `user_id = auth.uid()` 자체 도출 (caller-supplied 신뢰 제거 — wake#JMY 동일 패턴). client (anon JWT, 인증된 user 자신) 와 FastAPI 양쪽 모두 본 함수만 호출. `service_role` 직접 INSERT 는 비정본 (REVOKE). | CMP-576 (intent 테이블 + SECURITY DEFINER 함수) + CMP-577 (호출) |
...
| **A-AUTH-2b. Production app redirect URL gate (MUST — A-AUTH-3 전에 통과, wake R10 #4).** `supabase/config.toml` 의 `[auth].site_url` / `additional_redirect_urls` 가 local-only (예: `http://localhost:3000`, `http://127.0.0.1:3000`) 상태에서 그대로 `supabase config push` 하면 **production OAuth callback (`redirectTo=https://<prod-app>/auth/callback`) 이 Supabase redirect allow-list 에서 빠짐** → callback reject 또는 localhost fallback. 정정 옵션 중 하나:<br/>&nbsp;&nbsp;① **`[remotes.production.auth]` override 블록 추가** — `supabase/config.toml` 에 `[remotes.production.auth] site_url = "https://<prod-app>"` + `additional_redirect_urls = ["https://<prod-app>", "https://<prod-app>/auth/callback", "https://<prod-app>/**"]` 명시 (Supabase 정본 `[remotes.<branch>]` 패턴 — §3.2 A1.1 동일 형식).<br/>&nbsp;&nbsp;② **Dashboard direct edit** — Console (production) → Authentication → URL Configuration 에서 `Site URL` 과 `Redirect URLs` 에 production app callback 추가. CLI 가 push 한 local-only 값을 dashboard 가 override.<br/>옵션 ① 권장 (config-as-code). 옵션 ② 사용 시 후속 `supabase config push` 가 dashboard 값을 덮어쓸 위험 → 옵션 ① 로 보완.<br/>**검증 PoC**: production project 에서 OAuth callback redirect URL 4개 (origin / `/auth/callback` / `/**`) 가 모두 redirect allow-list 에 들어있는지 dashboard 캡처. `https://<prod-app>` 의 sample callback URL 도 reject 안 됨 확인. **A-AUTH-3 의 `supabase config push` 실행 시점에 본 게이트가 이미 통과 상태여야 한다** — 그렇지 않으면 A-AUTH-4 smoke 가 redirect rejection 으로 fail. | CMP-576 + DevOps |
...
| HK-REG-3. **검증 PoC (MUST).** staging branch 에 §7.1.5 파일 ①~⑦ 적용 + HK-REG-1 또는 HK-REG-2 등록 직후 다음 시나리오 캡처:<br/>&nbsp;&nbsp;• Kakao signup `service_terms` 누락 시도 → Hook 응답 `{"error":{"http_code":403,"message":"kakao_terms_missing"}}` + `auth.users` 0건 (Hook 이 실제로 호출됐다는 입증).<br/>&nbsp;&nbsp;• Hook 등록 비활성 (config.toml 의 `enabled = false` 또는 Dashboard Disable) negative — 위 Kakao signup 이 reject 없이 user 생성됨 캡처 (Hook 미호출 입증). 본 negative 후 다시 활성화 + smoke 통과 확인.<br/>**HK-REG-1 / HK-REG-2 등록 누락 시** `private.before_user_created_terms_gate` 함수가 DB 에 있어도 Supabase Auth 가 호출하지 않으므로 H4 시나리오 ①~④ (Hook weak gate) 가 silent skip — Kakao signup 의 `service_terms` 검증이 실행 안 됨. §7.1.5 H4 의 정상 통과는 본 HK-REG 통과 전제. | CMP-576 |
...
| W-B. **W2/W3 CLI secrets 등록 (MUST — Vault 사용 금지).** Production + Development + Staging persistent branch project 각각에 **`supabase secrets set --project-ref <REF> KEY=value`** (개별) 또는 **`supabase secrets set --project-ref <REF> --env-file <encrypted.env>`** (encrypted dotenv) 실행. **Project Settings → Vault 등록은 `env()` 해석 채널 아님 — 사용 시 W-B 미통과** (wake R10 #2). PoC: `supabase secrets list --project-ref <REF>` 출력 (값 마스킹) PR 본문에 첨부. | CMP-576 (DevOps + Auth Lead) |
