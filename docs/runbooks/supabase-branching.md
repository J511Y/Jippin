...
- [ ] **A3.** CMP-576 (Auth 트랙) 이 `supabase/config.toml` 의 `[auth.external.*]` 를 켜고 **같은 PR 에서 `[auth].enable_signup` 글로벌 gate 를 `true` 로 함께 바꾼다** (글로벌 gate 가 false 면 OAuth signup 도 막힘). `[auth.email].enable_signup` 은 영구 false 유지. **OAuth signup gate (MUST — PR 머지 직전 봉인).** A3 PR 머지 직전에 §7.1.4 (OAuth signup activation gate) 의 O1~O5 게이트가 모두 통과 상태여야 한다 (provider 시크릿 secrets/variables 등록 + §7.4 W-A~W-D Supabase-visible env wiring 통과 (root `.env`), redirect_uri 봉인 — Supabase Auth callback `https://<PROD_REF>.supabase.co/auth/v1/callback` 만, 콘솔 provider 활성화, §7.1.1 G1~G3 자동 linking 가드 완료, §4.7 sealed terms flow 보장 — §7.1.5 H1~H4 (Hook 가용성 + GRANT 봉인 + pending_terms_consents 모델 + 6-시나리오 PoC) 통과). 미통과 시 PR 머지 금지. CMP-577 트랙이 `[auth].enable_anonymous_sign_ins=true` 로 전환하며 §7.1.3 PoC 절차 (A5) 를 함께 수행.
...
| H2. **권한/역할 봉인 — `supabase_auth_admin` GRANT (SQL Hook MUST).** SQL Hook 함수와 함수가 읽는 스키마/테이블에 다음 권한을 명시 SQL 마이그레이션으로 부여한다 (Supabase 공식 가이드: https://supabase.com/docs/guides/auth/auth-hooks#developing):<br/>```sql<br/>-- Hook 함수 자체 EXECUTE 권한.<br/>GRANT EXECUTE ON FUNCTION public.before_user_created_terms_gate(jsonb) TO supabase_auth_admin;<br/>-- Hook 이 위치한 schema USAGE.<br/>GRANT USAGE ON SCHEMA public TO supabase_auth_admin;<br/>-- Hook 이 조회하는 pending_terms_consents 테이블 SELECT.<br/>GRANT SELECT ON TABLE public.pending_terms_consents TO supabase_auth_admin;<br/>-- (선택) Hook 이 본 row 를 consume 표시 하려면 UPDATE.<br/>GRANT UPDATE (consumed_at) ON TABLE public.pending_terms_consents TO supabase_auth_admin;<br/>```<br/>**RLS 가 켜진 테이블 (§7.2 R1 이후) 에는 별도 policy** 를 `supabase_auth_admin` role 에 추가하지 않으면 SELECT 가 silent fail → hook 이 모든 signup 을 reject 한다. CMP-575 의 RLS 정본화에서 본 role 의 read policy 를 함께 봉인. **GRANT 누락 시 첫 OAuth signup 이 `permission denied` 로 fail** 하므로 staging PoC (§3.2 A1.2) 에서 본 시나리오를 명시 검증한다. | CMP-576 (Hook SQL) + CMP-575 (RLS policy) |
| H3. **pre-user terms consent intent 조회 메커니즘 (MUST — Hook 입력 정본).** `before-user-created` Hook 은 candidate user 가 `auth.users` 에 **아직 insert 되기 전** 에 실행되므로 `users.id` 를 키로 한 어떤 조회도 불가능하다 (정본 inputs 문서: https://supabase.com/docs/guides/auth/auth-hooks/before-user-created-hook). 따라서 internal terms 동의를 user_id 가 아닌 **사전 발급된 pending consent record** 로 조회해야 한다. 본 PR 봉인 모델:<br/>① **테이블.** CMP-576 가 `public.pending_terms_consents` 를 SQL 마이그레이션으로 정의. 컬럼: `consent_token uuid PRIMARY KEY`, `provider text NOT NULL` (google/kakao/naver), `terms_version text NOT NULL`, `accepted_at timestamptz NOT NULL DEFAULT now()`, `expires_at timestamptz NOT NULL` (e.g. now() + 30 분), `provider_subject text NULL` (OAuth 콜백 시 채워짐), `consumed_at timestamptz NULL`. RLS ON, anon 은 INSERT 만 (자기 row), `supabase_auth_admin` 은 SELECT/UPDATE.<br/>② **흐름.** (1) app 의 internal terms 화면에서 사용자가 동의 누르면 `consent_token = gen_random_uuid()`, `provider`, `terms_version` 으로 row INSERT. (2) app 이 OAuth provider 로 redirect 할 때 `supabase.auth.signInWithOAuth({ options: { queryParams: { state: <consent_token> } } })` 또는 `redirectTo` query 에 `consent_token` 을 포함. (3) Supabase 가 OAuth 콜백 처리 시 `state`/`redirectTo` 의 token 을 user metadata (`raw_user_meta_data.consent_token` 또는 `app_metadata.consent_token`) 로 보존하도록 Edge Function 또는 callback redirect rewrite 로 봉인 (CMP-577 web 트랙 책임). (4) Hook 입력 payload (`event.user_metadata` 또는 `event.app_metadata` 의 `consent_token`) 를 가지고 다음 SQL 검증:<br/>```sql<br/>-- Hook 함수 본문 (pseudo).<br/>DECLARE token uuid := (event->'user_metadata'->>'consent_token')::uuid;<br/>BEGIN<br/>  IF NOT EXISTS (<br/>    SELECT 1 FROM public.pending_terms_consents<br/>    WHERE consent_token = token<br/>      AND provider = event->'identity'->>'provider'<br/>      AND consumed_at IS NULL<br/>      AND expires_at > now()<br/>  ) THEN RETURN jsonb_build_object('decision', 'reject', 'message', 'terms_not_accepted');<br/>  END IF;<br/>  UPDATE public.pending_terms_consents SET consumed_at = now() WHERE consent_token = token;<br/>  RETURN jsonb_build_object('decision', 'continue');<br/>END;<br/>```<br/>③ **expiry/회수.** 30분 expiry + consumed_at 단방향 update + cron 으로 7일 이상 row 삭제 (`pg_cron` extension 또는 별도 cleanup job). consent_token 재사용 시 같은 user 가 같은 provider 로 재시도해도 reject.<br/>④ **Naver custom OAuth/OIDC** 처럼 provider_subject 가 콜백 시점에야 결정되는 흐름은 (4) 의 token 검증 후 `provider_subject` 를 row 에 UPDATE 해 audit trail 보존. | CMP-576 + CMP-577 |
| H4. **Hook 활성화 self-check (CMP-576 PR).** 다음 PoC 결과를 staging branch 에서 캡처해 CMP-576 PR 본문에 첨부:<br/>① consent_token 없이 OAuth signup → Hook reject + `auth.users` row 0건 생성<br/>② 만료된 consent_token → Hook reject + row 0건<br/>③ 유효 consent_token → Hook continue + `auth.users` row 생성 + pending_terms_consents.consumed_at 채워짐<br/>④ 같은 consent_token 재사용 → Hook reject (consumed_at IS NOT NULL)<br/>⑤ provider 불일치 (google token 으로 kakao signup) → Hook reject<br/>⑥ `supabase_auth_admin` 의 GRANT 회수 후 ③ 재실행 → permission denied 로 fail (negative 검증)<br/>위 6개 시나리오 중 1개라도 누락된 PR 은 §3.2 A3 + 본 게이트로 reject. | CMP-576 + CMP-577 |

본 PR (CMP-574, 단계 0) 은 Hook 자체를 도입하지 않는다. CMP-576 PR 이 본 §7.1.5 H1~H4 를 정본화해 `supabase/migrations/<ts>_before_user_created_hook.sql` + `supabase/migrations/<ts>_pending_terms_consents.sql` 을 추가하고, web 측 token 전파는 CMP-577 이 담당. §7.1.4 O4 (a) 의 PoC 첨부 의무는 본 §7.1.5 H4 결과로 만족한다.
