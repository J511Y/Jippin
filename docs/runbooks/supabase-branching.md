...
> **읽는 법 (MUST).** "Repo-side 완료" 는 워크플로우/문서 변경이 PR 머지로 끝났다는 뜻이며, 단계 1 원래 트리거였던 **ADR-0004 Accepted 결정 자체는 본 PR 시점에 여전히 Pending** 이다 (`docs/adr/0004-supabase-transition.md` 상태: Proposed). 단계 3 의 operational gate (U7 / R5 / C3) 도 Supabase 콘솔 연결 후에 검증된다. 본 표를 근거로 "ADR-0004 가 Accepted 됐다" / "U7·R5·C3 가 통과됐다" / "Neon project 가 폐기됐다" 고 후속 트랙이 가정하면 안 된다. 다만 CMP-604 이후 web/auth cleanup 은 별도 완료되었으므로 [`supabase-web-auth.md`](supabase-web-auth.md) 의 current-state guidance 를 따른다.
...
> **CMP-604 이후 현재 정본.** Anonymous → OAuth upgrade 는 Supabase Anonymous Sign-In 이 만든 `auth.users.id` 를 그대로 유지하고, 사용자가 전환 CTA 를 누른 시점에 Supabase `linkIdentity()` 로 OAuth identity 를 연결하는 흐름이 정본이다. Legacy `public.anonymous_users`, `localStorage.jippin_anonymous_user_id`, `x-jippin-anon-id`, `converted_user_id` claim 경로는 forward 정본이 아니며, API route 는 accidental call 을 410 으로 막는다. 아래 절은 현재 운영 체크리스트이며, pre-CMP-604 dual-write/migration gate 는 historical context 로만 남긴다.

**현재 흐름.**

| 단계 | 동작 | 책임 |
|---|---|---|
| A1. anonymous sign-in | `supabase.auth.signInAnonymously()` 로 anonymous `auth.users` row 와 access token 을 만든다. 도면/리포트 ownership 은 이 `auth.users.id` 를 기준으로 저장한다. | Web |
| A2. manual upgrade | 사용자가 상담/저장/공유 등 전환 CTA 를 누르면 anonymous session 을 유지한 채 `linkIdentity({ provider })` 를 호출한다. 새 user 를 만들면 안 된다. | Web |
| A3. callback/session | Web callback 은 Supabase `exchangeCodeForSession()` 으로 세션 cookie 를 확정하고, FastAPI `/auth/supabase/session` 은 non-anonymous token 과 허용 provider(Kakao/custom:kakao 현재 UI 기준)를 검증한 뒤 `public.users(id = auth.users.id)` profile 을 안전하게 `ON CONFLICT DO NOTHING` upsert 한다. | Web + Backend/Auth |
| A4. conflict | provider identity 가 이미 다른 user 에 연결된 경우 자동 병합하지 않는다. 사용자는 기존 계정 로그인 + 명시적 데이터 이관 흐름으로 보낸다. | Web + Backend/Auth |
| A5. audit / terms | Kakao Sync 약관 audit 은 `terms_consents(source='kakao_sync')` 로, 일반 내부 약관은 `source='internal_signup'` 로 기록한다. `auth.users` / `auth.identities` 는 Supabase managed schema 이므로 직접 mutate 하지 않는다. | Backend/Auth + DB |

**운영 체크리스트.**

| 체크 | 기준 |
|---|---|---|
| Anonymous Sign-In | Supabase branch config 에서 anonymous sign-in 이 켜져 있어야 한다. 단 abuse control(CAPTCHA/rate limit/cleanup/MAU monitoring)은 별도 운영 게이트로 계속 추적한다. |
| Manual linking | `enable_manual_linking = true`, automatic verified-email linking 은 OFF. §7.1.1 / §7.1.2 가드와 함께 운영한다. |
| Legacy API | `/auth/anonymous-users`, `/auth/{provider}/start`, `/auth/callback/{provider}`, `/auth/sso-accounts/{provider}/link` 는 현재 정본 경로가 아니며 410 상태를 유지한다. |
| Legacy schema | `public.anonymous_users`, `public.external_sso_accounts`, `public.auth_identities` 는 forward auth SSOT 가 아니다. 새 마이그레이션/코드는 `auth.users.id` 와 `auth.identities` 기준으로 작성한다. |
| Smoke | staging/preview 에서 anonymous → Kakao `linkIdentity()` conversion, callback session bridge, provider mismatch rejection, identity conflict fallback 을 검증한다. |

**Historical pre-CMP-604 note.** CMP-574/CMP-577 초기 설계에는 `anonymous_users.id` + `localStorage.jippin_anonymous_user_id` dual-write 와 "Supabase anonymous sign-in 은 Phase 2 전 호출 금지" gate 가 있었다. CMP-603/CMP-604 cutover 이후 이 gate 는 종료되었고 현재 운영 지시로 사용하지 않는다.
...
| O5. **anonymous → OAuth upgrade 흐름 분리.** CMP-604 이후 `signInAnonymously()` + `linkIdentity()` 가 current path 다. provider 활성화 전에는 §7.1.3 의 current checklist(anonymous sign-in toggle, manual linking, legacy route 410, provider/session smoke)를 통과해야 한다. | CMP-577/CMP-604 | CMP-604 반영 |
...
Supabase 는 default 로 `public` 스키마의 모든 테이블/뷰를 PostgREST + GraphQL 로 외부 노출한다. Forward schema SSOT 는 `supabase/migrations/*.sql` 이며, public app tables(`users` profile, `terms_consents`, request/consultation/report tables 등)는 API gateway/RLS 정책과 함께 검토해야 한다. `[api]` 가 켜진 채 RLS 없는 public table 이 남으면 anon key 보유자가 PostgREST/GraphQL 로 직접 조회할 수 있어 FastAPI 게이트웨이를 우회한다.
