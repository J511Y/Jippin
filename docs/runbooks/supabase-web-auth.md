...
├── client.ts        # createBrowserClient — 클라이언트 컴포넌트 / hooks 용 singleton
...
- **Browser client** (`client.ts`) — 매 호출마다 새 인스턴스를 만들지 말고 lazily 한 번만 생성. React Query 와 동일한 lifecycle.
- **Server client** (`server.ts`) — 매 요청에서 `cookies()` 핸들을 받아 새 인스턴스를 만든다. `setAll()` 은 항상 제공하되 Server Component 쓰기 제한에서는 예외를 삼킨다. 토큰을 모듈 스코프에 캐시하면 요청 간 누설 위험.
- **Proxy / Edge client** (`proxy.ts`) — Next.js 16 proxy 의 `request.cookies` / `response.cookies` 시그니처에 맞춘 어댑터. `@supabase/ssr` v0.5+ 표준 `getAll()` / `setAll()` 패턴 그대로.
...
| `SUPABASE_FLOW_COOKIE_SECRET` | `apps/web/.env.example` | OAuth flow-cookie HMAC 서명 키. | `jippin_oauth_provider` / `jippin_merge_intent` 등 Web BFF 발급 flow-cookie 에 사용. 실값은 운영 시크릿으로만 주입하고 코드/이슈/PR 본문에 쓰지 않는다. |
...
- **OAuth 진입 BFF — flow context cookie + intent dispatch + PKCE verifier.** 위 `linkIdentity` / `signInWithOAuth` 호출은 **클라이언트가 직접 부르지 않고** `apps/web/app/auth/oauth/start/route.ts` (GET, `?provider=<ui>&intent=link|signin|link-merge`) 를 거친다. 이 BFF 가 다음을 단일 응답으로 묶어 처리한다 — **모든 Set-Cookie 가 같은 `NextResponse` 객체에 부착되어야 PKCE / flow context / merge intent 가 함께 전달**된다 (review item 5).
  ...
  3. **PKCE verifier cookie (필수, review item 2·5 — `getAll`/`setAll` 패턴).** `@supabase/ssr` v0.5+ 는 cookie adapter 를 `get`/`set`/`remove` 개별 패턴에서 **`getAll` + `setAll` 배치 패턴**으로 옮겼다. server-side `linkIdentity` / `signInWithOAuth` 호출이 발급하는 모든 cookie (`sb-<ref>-auth-token-code-verifier` 등) 가 단일 `setAll(cookiesToSet)` 호출로 한 번에 도착하므로, 그 콜백이 같은 `NextResponse` 인스턴스에 모두 부착해야 PKCE 교환이 완료된다. 응답 객체를 만들고 나서 SDK 를 호출하거나, 새 `NextResponse.redirect()` 를 또 만들어 반환하면 verifier 가 손실되어 callback 이 `auth/missing-code-verifier` 에러로 실패한다.
      ...
      export async function GET(request: NextRequest) {
        ...
        if (urlResult.error || !urlResult.data?.url) {
          // URL 생성 실패/throw 는 fresh 500 으로 fail-fast.
          // 누적 response 를 버려 PKCE verifier / flow-cookie 가 빈 redirect 로 남지 않게 한다.
          return NextResponse.json({ error: { code: 'oauth_init_failed' } }, { status: 500 });
        }
      ...
      위 패턴의 invariant: **`response` 객체 하나가 모든 Set-Cookie 의 owner** 이며, 마지막 단계가 그 객체의 headers 를 그대로 사용하여 302 를 만든다. `getAll`/`setAll` 은 v0.5+ 의 표준이며 deprecated `get`/`set`/`remove` 개별 콜백을 사용하면 cookie 누락 / 동시 set race 가 발생할 수 있어 금지. 본 패턴은 `lib/supabase/server.ts` / `proxy.ts` 의 모든 `createServerClient` 호출에 일관 적용하고, browser singleton 은 `lib/supabase/client.ts` 로 분리한다.
  4. **자체 cookie 발급 — Web origin 필수 (review item 4).** 같은 `NextResponse` 에 다음 cookie 를 부착한다. 모두 `HttpOnly; Secure; SameSite=Lax; Path=/auth/callback; Max-Age=600`. API 가 다른 host/subdomain (예: `api.jippin.com`) 에 있으면 backend 의 `Set-Cookie` 는 `www.jippin.com/auth/callback` 에 도달하지 않으므로 **반드시 Web BFF 가 같은 origin 에서 발급**해야 한다.
     - `jippin_oauth_provider` — `SUPABASE_FLOW_COOKIE_SECRET` 으로 HMAC 서명된 payload 토큰. payload 는 `provider`, `supabase_provider`, `intent`, 16-byte random hex `nonce`, 발급 시각(`iat`), 만료(`exp`) 를 포함한다. callback §4.5.2.1 Kakao 감지가 1차 신호로 사용하며, callback consumer 는 1회 검증 후 즉시 expire 한다.
  ...
  6. **OAuth URL generation fail-fast (Round 11).** `signInWithOAuth` / `linkIdentity` 가 throw 하거나 `data.url` 을 반환하지 않으면 BFF 는 **새 `NextResponse.json(..., { status: 500 })`** 를 반환한다. 이 fresh response 는 의도적으로 accumulator 의 `Set-Cookie` 헤더를 재사용하지 않으므로 `jippin_oauth_in_progress`, `jippin_oauth_provider`, PKCE verifier 가 실패 응답에 남지 않는다.
...
| R2 (`PRRT_kwDOSp2wlM6F_CSD`) | §4.2.1 / line ~99 | OAuth start 302 응답에 Supabase 가 set 한 PKCE cookie 가 brower 까지 전달되도록 Set-Cookie 위임 | CMP-580/CMP-585 — PR [#47](https://github.com/J511Y/Jippin/pull/47) `fd1c85dd` + `7ab706b` (`apps/web/app/auth/oauth/start/route.ts`, callback/proxy flush tests) |
...
| R10 (`PRRT_kwDOSp2wlM6F_SzM`) | §4.2.1 / line ~308 | `@supabase/ssr` v0.5+ 표준 `getAll`/`setAll` 패턴 SSR cookie adapter | CMP-580/CMP-585 — PR [#47](https://github.com/J511Y/Jippin/pull/47) `fd1c85dd` + `7ab706b` (`apps/web/lib/supabase/{server,proxy,client}.ts`, `apps/web/proxy.ts`) |
