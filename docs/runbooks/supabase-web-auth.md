...
| **Kakao Sync 동의 audit** | Supabase hosted OAuth 가 사용자 동의 화면 owner. **Kakao 감지는 `app_metadata.provider` 단독 의존 금지** (익명→link 케이스 누락) — `lib/supabase/identities.ts` 의 `detectNewlyLinkedProvider(user, intendedProviderCookie)` 가 (1) `jippin_oauth_provider` flow context 쿠키 + (2) `user.identities[]` 두 신호로 판정 (§4.5.2.1). `POST /auth/terms/kakao-sync` backend route 구현 전 web callback 은 `NEXT_PUBLIC_AUTH_KAKAO_SYNC_AUDIT_ENABLED=false` 로 fail closed (`/auth/failure?reason=kakao_sync_unavailable`) 처리한다. 활성화 시 payload 는 **`provider_access_token: session.provider_token`** (provider OAuth access token — `id_token` 아님) 이며, 짧은 timeout 을 둬 OAuth callback 을 오래 붙잡지 않는다. backend 는 이 토큰으로 Kakao `/v2/user/scopes` 재호출 후 `terms_consents.source='kakao_sync'` insert (§4.5.2.2). 실패 시 reconcile 잡 fallback. |
...
- **clear 시점.** (1) callback Route Handler 의 redirect 목적지가 항상 `/auth/callback-done?next=<safeNext>` (small client page) 로 들어가고, 이 page 가 `sessionStorage.removeItem('jippin_oauth_in_progress')` **한 키만** 제거한 뒤 `next` 로 navigate. (2) 안전망으로 SessionProvider mount 시 flag 의 timestamp 가 5분 초과면 강제 clear.
...
- guard 가 5분을 넘기면 stale 로 간주하고 강제 clear — 사용자가 OAuth provider 화면에서 멈춰 있는 동안 익명 흐름이 영구 차단되지 않도록 한다.
...
- **OAuth 진입 BFF — flow context cookie + intent dispatch + PKCE verifier.** 위 `linkIdentity` / `signInWithOAuth` 호출은 **클라이언트가 직접 부르지 않고** `apps/web/app/auth/oauth/start/route.ts` (GET, `?provider=<ui>&intent=link|signin|link-merge`) 를 거친다. 이 BFF 가 다음을 단일 응답으로 묶어 처리한다 — **모든 Set-Cookie 가 같은 `NextResponse` 객체에 부착되어야 PKCE / flow context / merge intent 가 함께 전달**된다 (review item 5).
  ...
  3. **PKCE verifier cookie (필수, review item 2·5 — `getAll`/`setAll` 패턴).** `@supabase/ssr` v0.5+ 는 cookie adapter 를 `get`/`set`/`remove` 개별 패턴에서 **`getAll` + `setAll` 배치 패턴**으로 옮겼다. server-side `linkIdentity` / `signInWithOAuth` 호출이 발급하는 모든 cookie (`sb-<ref>-auth-token-code-verifier` 등) 가 단일 `setAll(cookiesToSet)` 호출로 한 번에 도착하므로, 그 콜백이 같은 `NextResponse` 인스턴스에 모두 부착해야 PKCE 교환이 완료된다. 응답 객체를 만들고 나서 SDK 를 호출하거나, 새 `NextResponse.redirect()` 를 또 만들어 반환하면 verifier 가 손실되어 callback 이 `auth/missing-code-verifier` 에러로 실패한다.
      ...
      export async function GET(request: NextRequest) {
        ...
        // ★ Step 1 — Route Handler accumulator 를 먼저 만든다. setAll 콜백이 이 객체에
        // cookie 를 적용한다. NextResponse.next() 는 middleware-only 이므로 금지.
        ...
        response.cookies.set('jippin_oauth_provider',
          encodeOAuthFlowContext(sbProv, Date.now()), COOKIE_OPTS);
      ...
      위 패턴의 invariant: **`response` 객체 하나가 모든 Set-Cookie 의 owner** 이며, 마지막 단계가 그 객체의 headers 를 그대로 사용하여 실제 `302` 응답을 만든다. Route Handler 에서는 `NextResponse.next()` 를 쓰지 않는다. `NextResponse.next()` 는 middleware-only 패턴이라 App Router Route Handler 에서 반환하면 예상 밖의 동작이 발생할 수 있다. `getAll`/`setAll` 은 v0.5+ 의 표준이며 deprecated `get`/`set`/`remove` 개별 콜백을 사용하면 cookie 누락 / 동시 set race 가 발생할 수 있어 금지. 본 패턴은 `lib/supabase/server.ts` / `proxy.ts` / `browser.ts` 의 모든 `createServerClient` 호출에 일관 적용한다.
  4. **자체 cookie 발급 — Web origin 필수 (review item 4).** 같은 `NextResponse` 에 다음 cookie 를 부착한다. 모두 `HttpOnly; Secure; SameSite=Lax; Path=/auth/callback; Max-Age=300`. API 가 다른 host/subdomain (예: `api.jippin.com`) 에 있으면 backend 의 `Set-Cookie` 는 `www.jippin.com/auth/callback` 에 도달하지 않으므로 **반드시 Web BFF 가 같은 origin 에서 발급**해야 한다. stale 정책은 단순하다: 새 OAuth start 는 같은 이름/path cookie 를 overwrite 하고, callback 성공/실패 및 OAuth URL 생성 실패 시 callback-scoped cookie 를 즉시 expire 한다.
     - `jippin_oauth_provider` — Supabase provider id + 생성 timestamp 힌트 (`<provider>|<createdAtEpochMs>`). callback §4.5.2.1 Kakao 감지가 1차 신호로 사용하지만 인증/인가 결정에는 쓰지 않는다. callback 진입 시 createdAt 이 5분을 넘었거나 malformed 이면 `/auth/failure?reason=oauth_guard_stale` 로 거부하고 callback-scoped cookie 를 expire 한다. PKCE `state` / `code_verifier` 무결성은 `@supabase/ssr` 가 발급하는 Supabase cookie 와 `exchangeCodeForSession()` 에 의존한다. 본 web BFF 는 해당 Supabase PKCE cookie 를 재서명하지 않는다.
     - `intent=link-merge` 인 경우 추가로 `jippin_merge_intent` — BFF 가 server-side 로 `POST /auth/anon-merge-intents` 를 먼저 호출해 받은 서명 토큰 (`{ intent_id, nonce, exp, sig }`) 을 Set-Cookie 로 부착한다. **signed token 은 URL query 에 싣지 않는다.** backend route 가 아직 없는 환경에서는 `NEXT_PUBLIC_AUTH_MERGE_INTENTS_ENABLED=false` 로 UI CTA 를 비활성화하고, 강제 GET 진입도 `/auth/failure?reason=merge_unavailable` 로 돌려보낸다.
     - `jippin_pending_anonymous_user_id` — legacy `localStorage.jippin_anonymous_user_id` 만 가진 사용자의 최초 OAuth 전환 ownership 을 잃지 않도록 OAuth start 시 `/auth` path 의 HttpOnly short-lived cookie 로 보존한다. Google/Naver 는 internal terms gate 로 이동한 뒤 backend 가 이 pending id 를 claim owner 로 소비한다.
  5. 응답 status `302`, `Location: <OAuth provider URL>` (`signInWithOAuth` / `linkIdentity` 가 반환). 이 302 는 guard-setting hop 이다: `jippin_oauth_provider`, `jippin_merge_intent`, Supabase PKCE verifier/state cookie 가 같은 실제 응답에 먼저 누적되고, 그 응답이 OAuth provider 로 이동한다. 재사용 방지는 callback 종료 시 `jippin_oauth_provider` / `jippin_merge_intent` 를 즉시 expire 하는 방식으로 처리한다. OAuth URL 생성 실패 시 JSON 502 로 방치하지 않고 `/auth/failure?reason=oauth_init_failed` 로 redirect 하며, callback-scoped flow cookie 를 즉시 expire 한다. `signOut()` 은 server-created merge intent cookie 가 정상 생성된 뒤에만 호출하며, GET query 값만으로 destructive action 을 수행하지 않는다.

  **callback URL origin 산출.** `apps/web/app/auth/oauth/start/route.ts` 는 `NEXT_PUBLIC_FRONTEND_AUTH_CALLBACK_URL` 이 있으면 그 값을 최우선으로 사용한다. 없으면 `NEXT_PUBLIC_SITE_URL` 의 origin 으로 `/auth/callback` 을 합성하고, 그것도 없거나 invalid 면 `request.nextUrl.origin` 으로 fallback 한다. 브라우저가 보낸 `Origin` header 는 OAuth redirect URL 산출에 사용하지 않는다. local 은 `.env.example` 의 `http://localhost:3000`, staging/prod 는 배포 origin 을 `NEXT_PUBLIC_SITE_URL` 또는 callback URL 로 명시한다.

  **redirect allowlist.** callback 의 `next` 는 `lib/safe-redirect.ts` 의 `isSafeNext()` 단일 함수로 검증한다. 허용은 `/` 로 시작하는 상대 경로뿐이며 `//`, `\`, CR/LF/NUL, absolute URL 은 모두 거부한다. 실패 fallback 역시 `resolveSafeNext()` 를 거쳐 absolute env 값이 open redirect 가 되지 않게 한다.
...
linkIdentity({ provider }) 실패
   ...
   │            HttpOnly + Secure + SameSite=Lax + Path=/auth/callback + Max-Age=300.
...
| `kakao` | **`'custom:kakao'`** | 현재 콘솔 결정은 Kakao Custom OAuth 이다. Kakao Sync 동의 화면을 OAuth 화면에서 표시하려면 콘솔에서 `scope=profile_nickname,profile_image,account_email,gender,birthyear,birthday,terms_of_service` 등 명시적 입력 필요. |
...
- 현재 Kakao 는 Custom OAuth (`custom:kakao`) 로 봉인한다. native provider 로 바꾸려면 콘솔 결정 + runbook + 코드 매핑을 같은 PR 에서 함께 바꾼다.
...
export function detectNewlyLinkedProvider(
  ...
  // 1) UI 의도 — flow context cookie (`provider|createdAt`, httpOnly·Secure·Path=/auth/callback·Max-Age=300).
...
- `intendedProviderCookie` 는 §4.2.1 OAuth start BFF 가 OAuth 진입 직전에 발급한다: 값 = `provider|createdAtEpochMs`. 쿠키 이름 `jippin_oauth_provider`, scope = `Path=/auth/callback; Max-Age=300; HttpOnly; Secure; SameSite=Lax`. 콜백이 끝나면 즉시 expire (callback 의사 코드 마지막 블록 참조). 값은 Kakao audit 힌트와 stale 판단에만 쓰며, PKCE state/verifier 검증은 Supabase SSR cookie + `exchangeCodeForSession()` 이 owner 이다.
...
export async function GET(request: NextRequest) {
  ...
  //     httpOnly·Secure·Path=/auth/callback·Max-Age=300) → UI 가 의도한 provider.
...
| `/auth/callback-done` | `?next=<relative>` | 위 (a) 의 `isSafeNext(next)` 재검증. merge 실패는 callback-done 으로 숨기지 않고 `/auth/failure?reason=merge_commit_failed&provider=<ui>&next=<safeNext>` 로 보낸다. |
...
| 4xx (`410 expired` / `409 already_consumed` / `422 signature_invalid`) | `mergeStatus='commit_failed'` + log warn | `/auth/failure?reason=merge_commit_failed&provider=<ui>&next=<safeNext>` 로 이동해 사용자가 실패를 인지하고 ladder 를 재시도할 수 있게 한다. backend reconcile 잡이 사후 처리하지 않으면 사용자는 새 ladder 재진입으로 복구. |
...
- **JS 비활성 / 페이지 에러** — 위 client page 의 `useEffect` 가 fire 하지 못한다. SessionProvider 의 5분 stale 안전망 (§4.1.1 마지막 bullet) 이 유일 복구 경로. 5분 후 다음 페이지 로드에서 자동 clear.
- **사용자가 OAuth provider 화면에서 탭 이탈** — callback / failure 둘 다 통과하지 않으므로 guard 는 5분 stale 까지 활성. 동일 탭 재진입 시 SessionProvider 가 stale 검사로 정리.
...
| **Backend/Auth — `POST /auth/terms/kakao-sync` 라우트** | §4.5.2 경로 (a) 의 callback-side audit insert. 백엔드가 카카오 user-info 재조회 (또는 id_token 파싱) 후 `terms_consents(source='kakao_sync')` 단일 트랜잭션 insert. | route 구현 전 web 은 `NEXT_PUBLIC_AUTH_KAKAO_SYNC_AUDIT_ENABLED=false` 로 호출을 비활성화한다. 활성화 후에도 callback fetch 는 짧은 timeout 을 둬 OAuth redirect 를 오래 붙잡지 않는다. 콜백 실패 대비 reconcile 잡도 owner. |
...
| **약관 동의 흐름 (Google · Naver)** | `terms_consents(source='internal_signup')` 데이터는 백엔드가 owner. Supabase user metadata 에는 약관 동의를 저장하지 않는다. | callback 은 Google/Naver 최초 가입 추정 시점에만 `/auth/terms?next=<safeNext>` 로 gate 한다. 기존 가입자는 반복 gate 하지 않는다. `POST /auth/terms/accept` 는 backend 가 Supabase access token 검증을 지원할 때 `NEXT_PUBLIC_AUTH_TERMS_ACCEPT_ENABLED=true` 로 활성화한다. |
...
|---|---|---|---|
| R1 (`PRRT_kwDOSp2wlM6F_CSA`) | §4.7.4 / line ~760 | callback 실패 분기에서 `jippin_merge_intent` / `jippin_oauth_provider` 쿠키 즉시 삭제 | CMP-579 → PR [#49](https://github.com/J511Y/Jippin/pull/49) `1354d290` |
...
| R5 (`PRRT_kwDOSp2wlM6F_Jq_`) | §4.2.1 / line ~380 | merge `signed_token` 을 web BFF 가 받아 `jippin_merge_intent` 쿠키에 set | CMP-579 → PR [#49](https://github.com/J511Y/Jippin/pull/49) `1354d290` |
| R6 (`PRRT_kwDOSp2wlM6F_JrA`) | §4.6 / line ~1032 | `/auth/redirect?to=` open redirect 차단 (nonce 또는 origin allow-list) | CMP-582 |
| R7 (`PRRT_kwDOSp2wlM6F_JrC`) | §4.7.4 / line ~760 | callback 실패 분기에서 `sessionStorage.jippin_oauth_in_progress` 명시 정리 | CMP-579 → PR [#49](https://github.com/J511Y/Jippin/pull/49) `1354d290` |
| R8 (`PRRT_kwDOSp2wlM6F_Ntr`) | §4.7.4 / line ~759 | `error=identity_already_exists` 분기 → §4.2.2 migration ladder 진입점 | CMP-579 → PR [#49](https://github.com/J511Y/Jippin/pull/49) `1354d290` |
...
| R12 (`PRRT_kwDOSp2wlM6F_SzR`) | §4.2.2 / line ~392 | merge commit endpoint 정본은 cookie-only path 단일 (`POST /auth/anon-merge-intents/commit`) | CMP-579 → PR [#49](https://github.com/J511Y/Jippin/pull/49) `1354d290` |
