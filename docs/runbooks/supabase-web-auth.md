...
- **OAuth 진입 BFF — flow context cookie.** 위 `linkIdentity` / `signInWithOAuth` 호출은 **클라이언트가 직접 부르지 않고** `apps/web/app/auth/oauth/start/route.ts` (GET, `?provider=<ui>&intent=link|signin`) 를 거친다. 이 BFF 가:
  1. `toSupabaseProviderId(uiProvider)` 검증 (§4.2.3 매핑).
  2. **httpOnly + Secure + SameSite=Lax + Path=/auth/callback + Max-Age=600** 쿠키 `jippin_oauth_provider` 발급 — 값 = HMAC 서명된 `<supabase_provider>|<nonce>|<exp>` 토큰. callback 의 §4.5.2.1 Kakao 감지가 이 쿠키를 1차 신호로 사용한다.
  3. 익명 fallback ladder 의 "예, 옮기고 로그인" 분기에서 진입한 경우 추가로 `jippin_merge_intent` 쿠키도 같은 path 로 발급 (§4.2.2 step b).
  4. server-side `supabase.auth.signInWithOAuth({ provider, options: { redirectTo, skipBrowserRedirect: true } })` 로 받은 URL 로 302.
  클라이언트 JS 가 직접 Supabase SDK 의 OAuth 함수를 호출하면 httpOnly 쿠키를 발급할 수 없다 — BFF 경유는 review item 1·3 대응의 필수 조건.
...
linkIdentity({ provider }) 실패
   ...
   │            응답: { intent_id, nonce, expires_at }  // 서버가 발급.
   │         b) `POST /auth/anon-merge-intents/{intent_id}/bind-cookie` 호출 →
   │            서버가 httpOnly + Secure + SameSite=Lax 쿠키
   │            `jippin_merge_intent` 를 Set-Cookie (값 = 서명된 토큰 = intent_id|nonce|sig,
   │            Max-Age=600, Path=/auth/callback). 클라이언트 JS 는 토큰 내용에 접근 못함.
   │         c) supabase.auth.signOut()  // 익명 세션 폐기
   │            (signOut 이 Supabase 자체 쿠키만 정리하므로 jippin_merge_intent 는 살아남음).
   │         d) supabase.auth.signInWithOAuth({ provider,
   │              options: { redirectTo: callbackUrl({ next: SUCCESS_URL }) } }) 로 OAuth 진입.
...
- **`POST /auth/anon-merge-intents`, `/bind-cookie`, `/commit` 모두 Backend/Auth 트랙 신설 라우트** (본 트랙은 web 측 호출 계약과 payload 만 봉인한다). 백엔드가 ADR-0003 의 anonymous_users + 신규 supabase user 매핑을 검증하고 ownership 이전을 단일 트랜잭션으로 처리한다.
...
export async function GET(request: NextRequest) {
  ...
  let mergeOutcome: 'none' | 'committed' | 'failed' = 'none';
  let clearMergeCookie = false;
  if (mergeIntentCookie) {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/anon-merge-intents/commit`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      mergeOutcome = res.ok ? 'committed' : 'failed';
    } catch {
      mergeOutcome = 'failed';
    } finally {
      clearMergeCookie = true;     // 성공/실패 무관 1회용.
    }
  }
  ...
  const response = NextResponse.redirect(new URL(safeNext, ORIGIN(request)));
  if (clearMergeCookie) {
    response.cookies.set('jippin_merge_intent', '', { path: '/auth/callback', maxAge: 0 });
  }
  if (intendedProviderCookie) {
    response.cookies.set('jippin_oauth_provider', '', { path: '/auth/callback', maxAge: 0 });
  }
  return response;
}
```
