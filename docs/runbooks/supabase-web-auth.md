...
|---|---|---|---|
| R1 (`PRRT_kwDOSp2wlM6F_CSA`) | §4.7.4 / line ~760 | callback 실패 분기에서 `jippin_merge_intent` / `jippin_oauth_provider` 쿠키 즉시 삭제 | CMP-579 → CMP-583 branch `feat/CMP-579-oauth-callback-ladder` |
...
| R5 (`PRRT_kwDOSp2wlM6F_Jq_`) | §4.2.1 / line ~380 | merge `signed_token` 을 web BFF 가 받아 `jippin_merge_intent` 쿠키에 set | CMP-579 → CMP-583 branch `feat/CMP-579-oauth-callback-ladder` |
| R6 (`PRRT_kwDOSp2wlM6F_JrA`) | §4.6 / line ~1032 | `/auth/redirect?to=` open redirect 차단 (nonce 또는 origin allow-list) | CMP-582 |
| R7 (`PRRT_kwDOSp2wlM6F_JrC`) | §4.7.4 / line ~760 | callback 실패 분기에서 `sessionStorage.jippin_oauth_in_progress` 명시 정리 | CMP-579 → CMP-583 branch `feat/CMP-579-oauth-callback-ladder` |
| R8 (`PRRT_kwDOSp2wlM6F_Ntr`) | §4.7.4 / line ~759 | `error=identity_already_exists` 분기 → §4.2.2 migration ladder 진입점 | CMP-579 → CMP-583 branch `feat/CMP-579-oauth-callback-ladder` |
...
| R12 (`PRRT_kwDOSp2wlM6F_SzR`) | §4.2.2 / line ~392 | merge commit endpoint 정본은 cookie-only path 단일 (`POST /auth/anon-merge-intents/commit`) | CMP-579 → CMP-583 branch `feat/CMP-579-oauth-callback-ladder` |
