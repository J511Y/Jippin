...
- **OAuth 진입 BFF — flow context cookie + intent dispatch + PKCE verifier.** 위 `linkIdentity` / `signInWithOAuth` 호출은 **클라이언트가 직접 부르지 않고** `apps/web/app/auth/oauth/start/route.ts` (GET, `?provider=<ui>&intent=link|signin|link-merge`) 를 거친다. 이 BFF 가 다음을 단일 응답으로 묶어 처리한다 — **모든 Set-Cookie 가 같은 `NextResponse` 객체에 부착되어야 PKCE / flow context / merge intent 가 함께 전달**된다 (review item 5).
  ...
  3. **PKCE verifier cookie (필수, review item 2·5).** server-side `linkIdentity` / `signInWithOAuth` 호출은 `@supabase/ssr` `createServerClient({ cookies: { get, set, remove } })` 의 cookie adapter 를 통해 `sb-<project-ref>-auth-token-code-verifier` 쿠키를 발급한다. **BFF 의 응답은 이 Set-Cookie 를 그대로 운반해야** callback 의 `exchangeCodeForSession(code)` 가 verifier 를 읽어 PKCE 교환을 완료할 수 있다. Route Handler 패턴에서 cookie adapter 는 동일 `NextResponse` 인스턴스에 바인딩하므로, **응답 객체를 먼저 만들어 SDK 에 cookie adapter 로 넘기고 마지막에 redirect 시킨다**. 응답 객체를 만들고 나서 SDK 를 호출하거나, 새 `NextResponse.redirect()` 를 또 만들어 반환하면 verifier 가 손실되어 callback 이 `auth/missing-code-verifier` 에러로 실패한다.
      ...
      export async function GET(request: NextRequest) {
        ...
        // ★ Step 1 — 응답 객체를 먼저 만든다. cookie adapter 가 이 객체에 바인딩되어
        //   이후 SDK 가 set 하는 모든 Set-Cookie 가 본 response 에 누적된다.
        ...
        //   이 시점에는 response 가 아직 없으므로 placeholder 를 만들어둔다.
        ...
        // ★ Step 3 — intent dispatch (item 1). SDK 호출이 PKCE verifier 를 response 에 set.
        let urlResult;
        if (intent === 'link') {
          ...
          if (intent === 'link-merge') await supabase.auth.signOut();   // 익명 폐기.
        ...
        // ★ Step 4 — 새 NextResponse.redirect 를 만들지 말고, 누적된 response 의 헤더만
        //   교체하여 302 로 변환. Set-Cookie 헤더가 모두 보존됨.
      ...
      위 패턴의 invariant: **`response` 객체 하나가 모든 Set-Cookie 의 owner** 이며, 마지막 단계가 그 객체의 headers 를 그대로 사용하여 302 를 만든다. 이를 어기면 PKCE verifier 가 손실되고 callback 이 실패한다.
...
- **정상 경로** — callback-done page 의 `useEffect` 첫 줄에서 `sessionStorage.removeItem`. JS 가 fire 하기 전에 사용자가 brute back-button 으로 빠져나가도 SessionProvider 의 10분 stale 안전망이 보완.
- **JS 비활성 / 페이지 에러** — 위 stale 안전망 (§4.1.1 마지막 bullet) 이 유일 복구 경로. 10분 후 다음 페이지 로드에서 자동 clear.
- **callback Route Handler 가 callback-done 으로 가지 않는 케이스 (failureRedirect)** — `expireCallbackCookies` 는 callback path cookie 만 비우고 sessionStorage 는 못 만진다. 사용자가 failure 후 새 진입 시 SessionProvider 가 10분 stale 안전망으로 clear.
