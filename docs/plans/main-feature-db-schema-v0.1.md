...
- `floorplan_assets` 처럼 catalog/upload/session 중 하나 이상에 연결되어야 하는 테이블은 nullable FK 만 나열하지 말고 check constraint 로 최소 1개 parent 를 강제한다.
...
- unique `(user_id, term_id, version, source, target_provider) where promoted_at is null`
- `(user_id, expires_at)`
- partial index: `(expires_at) where promoted_at is null`

비고:

- Kakao Sync 는 이 테이블을 쓰지 않는다. Kakao 약관 검증은 Database Webhook + Kakao API reconciliation 경로가 정본이다.
- 만료 intent 는 scheduled task 로 삭제해도 되고, 보존이 필요하면 `promoted_at is null and expires_at < now()` 기준으로 archive 한다.
...
- `address_id` 는 `session_addresses.session_id` 와 FK cycle 이므로 초기 migration 에서는 nullable 로 만들고, 주소 row 생성 이후 update 하거나 DEFERRABLE FK 로 처리한다.
...
- `verdict`, `required_facilities`, `permit_required`, `ruleset_version`, `evaluated_at` 은 `packages/contracts/schemas/rule-eval-result.schema.json` 의 `RuleEvalResult` 정본 필드와 1:1 로 맞춘다.
...
- `legal_notice_included boolean not null default true`
...
- `legal_notice_included` 기본값은 반드시 `true` 로 둔다. 리포트 생성 기본값이 고지 누락 상태가 되면 AGENTS §4.6 봉인 정책을 깨므로 `false` default 는 금지한다.
...
- `floorplan_assets`: owner upload 또는 public catalog floorplan 에 연결된 asset 만 읽기 허용. signed URL 발급은 서버 API 가 담당.
- `chat_messages`, `chat_tool_calls`, `analysis_runs`, `overlay_selections`, `flow_guard_decisions`, `rule_evaluations`, `reports`, `estimate_items`: `sessions.user_id` 를 통해 owner 확인. `estimate_items` 는 `reports.session_id -> sessions.user_id` 로 owner 를 확인한다.
...
create policy leads_non_anonymous_current_terms
on public.leads
as restrictive
for all
to authenticated
...
| `address_id` | 이 세션에서 사용한 정규화 주소 row. `session_addresses.id` 를 참조한다. |
| `selected_floorplan_id` | 사용자가 내부/외부 후보 catalog 에서 선택한 도면. 직접 업로드를 선택한 경우 null 일 수 있다. |
| `selected_floorplan_upload_id` | 사용자가 직접 업로드한 도면을 선택한 경우의 upload row. catalog 후보를 선택한 경우 null 일 수 있다. |
| `selected_floorplan_asset_id` | 실제 분석 입력으로 사용한 asset row. catalog 후보든 사용자 업로드든 최종 입력 파일은 asset 으로 추적한다. |
...
| `floorplan_id` | asset 이 연결된 catalog 도면. 사용자 업로드 asset 이면 null 가능하다. |
| `floorplan_upload_id` | asset 이 연결된 사용자 업로드 record. catalog asset 이면 null 가능하다. |
| `session_id` | 특정 세션에서만 쓰이는 asset 인 경우 연결한다. |
| `owner_user_id` | asset 소유 사용자. private upload 접근 제어에 사용한다. |
...
| `floorplan_upload_id` | 작업 대상 사용자 업로드 도면. |
| `asset_id` | 작업 대상 asset. 예: 원본 이미지 scan, 마스킹본 생성. |
...
| `user_id` | 메시지를 작성한 사용자. assistant/system/tool 메시지는 null 가능하다. |
...
| `message_id` | 이 tool call 을 유발했거나 이 tool call 결과를 사용한 chat message. 독립 실행이면 null 가능하다. |
| `parent_tool_call_id` | tool 이 내부적으로 다른 tool 을 호출한 경우 부모 tool call. |
...
| `session_id` | 판단 대상 세션. |
| `analysis_run_id` | 참고한 AI 분석 실행. 없으면 null 가능하다. |
...
| `next_action` | 다음 UX/API 동작 지시. 예: 물어볼 질문, overlay 재확인 요청. |
...
| `legal_references` | 적용 법령/고시/조문 근거 목록. |
...
| `rule_evaluation_id` | 리포트 근거가 된 룰 평가 결과. |
| `revision` | 같은 세션 내 리포트 개정 번호. publish 후 수정 대신 새 revision 발행을 권장한다. |
| `status` | draft/published/superseded/revoked. |
| `verdict` | 리포트 대표 판정값. `rule_evaluations.verdict` 와 같은 `ALLOW`, `WARN`, `DENY`, `HOLD` 값을 저장한다. |
| `report_json` | 화면/다운로드 산출물에 필요한 리포트 구조화 payload. |
| `legal_notice_included` | 필수 법적 고지 포함 여부. false 이면 publish 차단해야 한다. |
| `pdf_asset_id` | 생성된 PDF asset. |
| `share_token_hash` | 공유 링크 token 의 hash. 원문 token 은 저장하지 않는다. |
| `share_expires_at` | 공유 링크 만료 시각. |
...
| `floorplan_upload_id` | 샘플 출처가 사용자 업로드 도면인 경우의 upload row. |
| `asset_id` | 실제 학습에 쓸 도면/마스크 asset. |
