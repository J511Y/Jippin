...
| `original_asset_id` | 사용자가 올린 원본 파일 asset. 업로드 직후 asset 생성 순서 때문에 일시적으로 null 가능하다. 값이 있으면 같은 session/upload source 의 `floorplan_assets.kind='original'` 만 허용한다. |
...
| `owner_user_id` | asset 소유 사용자. private upload/session/generated asset 접근 제어에 사용하며, 지정된 private parent 의 owner 와 항상 같아야 한다. |
...
| `actor_snapshot` | actor email hash, provider/identity hint, role, status, display label 같은 최소 식별 snapshot. FK 가 null 이 되어도 누가 변경했는지 추적할 수 있게 immutable 로 저장한다. |
