...
- `(effective_from desc)`
- transition guard: `status='active'` requires `validated_at is not null` and validation/test audit evidence
...
| `status` | draft/active/archived. active 는 한 시점에 하나만 허용하는 것을 권장하며, `validated_at` 또는 동등한 validation/test/audit evidence 가 있어야 전이할 수 있다. |
