...
- trigger/check function: when `result_snapshot is not null`, `result_schema_version = result_snapshot->>'schema_version'`
...
- `result_schema_version` 은 `result_snapshot->>'schema_version'` projection 이며, snapshot 이 있으면 두 값이 일치해야 한다.
...
| `result_schema_version` | 저장한 RuleEvalResult contract schema version. `result_snapshot.schema_version` projection 이며 historical replay 와 contract migration 기준이다. |
