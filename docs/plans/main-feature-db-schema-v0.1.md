...
- public catalog direct read 가 가능한 catalog asset 은 verified floorplan 에 연결된 display-safe kind 이면서 `scan_status='clean'` 이어야 한다. `pending`, `failed`, `infected`, `not_required` 또는 debug/internal artifact 는 service/admin 경로에서만 읽는다.
...
- `status='active'` 전이는 현재 유효 기간 안의 룰셋에만 허용한다. service transition 은 `effective_from <= current_date` 와 `(effective_to is null or effective_to > current_date)` 를 검증한다. 시간 의존 조건은 일반 CHECK 로 고정하지 말고 activation transition/trigger 에서 검증하고, active partial unique index 로 동시 active row 를 막는다.
...
- publish 상태의 report 는 `rule_evaluation_id is not null` 이어야 한다. draft/preview 에서만 null 을 허용하고, publish transition 은 same-session FK 와 `reports.verdict = rule_evaluations.verdict` 를 같은 transaction 에서 함께 만족해야 한다.
...
- `floorplan_assets`: public catalog asset read 는 verified catalog floorplan 에 연결된 display-safe kind(`thumbnail`, `preview`, `original`) 이면서 `scan_status='clean'` 인 asset 만 허용한다. `pending`, `failed`, `infected`, OCR debug, segmentation mask/debug, internal artifact, private upload/session/generated asset(`masked`, `overlay`, `report_pdf`, `report_image` 등)은 owner/session 또는 service/admin 경로로만 접근한다. signed URL 발급은 서버 API 가 담당한다.
...
| `kind` | asset 종류. 원본, 썸네일, 마스킹본, overlay, PDF 등을 구분한다. upload 원본, 분석 input/masked/overlay pointer 같은 역할별 FK 는 이 kind 까지 검증한다. public catalog read 는 display-safe `thumbnail`, `preview`, `original` 같은 허용 kind 와 `scan_status='clean'` 을 모두 요구하고 OCR/debug/internal artifact 는 service/admin 전용이다. |
...
| `effective_to` | 룰셋 적용 종료일. 종료 전이면 null 가능하다. active 전이 시 null 이거나 `effective_to > current_date` 여야 한다. |
...
| `status` | draft/published/superseded/revoked. published 전이는 `rule_evaluation_id`, `published_at`, legal notice, share/report artifact 제약을 모두 만족해야 한다. |
