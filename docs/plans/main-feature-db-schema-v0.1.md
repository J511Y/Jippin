...
| `floorplan_snapshot` | catalog row 삭제 후에도 사용자가 본 후보를 재현하기 위한 표시명, 타입, 면적, thumbnail pointer 등 최소 snapshot. |
...
| `estimate_item_id` | 리드 전환 근거가 된 견적 line item. 없으면 null 가능하며 같은 세션/리포트 범위여야 한다. |
