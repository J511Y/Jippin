...
- API 의 `require_permanent_user` 와 RLS restrictive policy 는 Supabase user 가 non-anonymous 이고 `public.users.status='active'` 이며, 이 테이블의 active row 전체가 `terms_consents` 에 존재하는지 확인한다.
...
- trigger/check function: when `pdf_asset_id is not null`, referenced asset is same-session and `floorplan_assets.kind in ('report_pdf', 'report_image')`
...
- 학습 샘플은 하나의 source scope 만 대표해야 한다. `floorplan_upload_id` 또는 private/session `asset_id` 가 있으면 `session_id` 를 반드시 채우고 모두 같은 세션에 속해야 한다. catalog sample 은 `floorplan_id` 와 보존된 catalog asset/snapshot 경로로 분리하며, private upload/asset 과 같은 row 에서 조합하지 않는다.
...
- `evidence_asset_id` 가 private/session asset 을 가리키면 `session_id` 를 반드시 채우고, 같은 `session_id` 의 `floorplan_assets` 만 허용한다. catalog/public evidence 를 별도로 허용하는 경우에는 private evidence 경로와 분리된 catalog scope check 를 둔다. nullable composite FK 의 MATCH SIMPLE 동작으로 private evidence 검증이 우회되면 안 된다.
...
- `floorplans`: `visibility='public_catalog' and quality_status='verified'` 인 row 만 authenticated/anonymous 사용자에게 읽기 허용. `admin_only`, `unverified`, `needs_review`, `rejected` 는 admin 만.
...
- `floorplan_assets`: public catalog asset read 는 verified catalog floorplan 에 연결된 display-safe kind(`thumbnail`, `preview`, `original`) 만 허용한다. OCR debug, segmentation mask/debug, internal artifact, private upload/session/generated asset(`masked`, `overlay`, `report_pdf`, `report_image` 등)은 owner/session 또는 service/admin 경로로만 접근한다. signed URL 발급은 서버 API 가 담당한다.
...
| `judgment_schema` | 주소, 도면 해석, 사용자 선택, 추가 질의 응답을 병합한 공통 판단 스키마 현재값. owner direct update 대상이 아니며 service transition 결과만 저장한다. |
| `judgment_schema_version` | `judgment_schema` 의 schema version. `packages/contracts` 버전과 맞춰야 하며 service transition 이 snapshot 과 함께 설정한다. |
...
| `visibility` | catalog 접근 범위. 사용자에게 후보로 보여줄지, 관리자 전용으로 둘지 구분한다. public read 는 `public_catalog` 만으로 부족하며 `quality_status='verified'` 도 필요하다. |
...
| `original_asset_id` | 사용자가 올린 원본 파일 asset. 업로드 직후 asset 생성 순서 때문에 일시적으로 null 가능하다. 값이 있으면 같은 session/upload source 의 `floorplan_assets.kind='original'` 만 허용하며 service transition 만 설정한다. |
...
| `kind` | asset 종류. 원본, 썸네일, 마스킹본, overlay, PDF 등을 구분한다. upload 원본, 분석 input/masked/overlay pointer 같은 역할별 FK 는 이 kind 까지 검증한다. public catalog read 는 display-safe `thumbnail`, `preview`, `original` 같은 허용 kind 로 제한하고 OCR/debug/internal artifact 는 service/admin 전용이다. |
...
| `normalized_schema` | CommonJudgmentSchema 로 정규화한 결과. snapshot 에 `session_id` 가 있으면 row `session_id` 와 같아야 하며, contract 에서 `session_id` 를 제거하면 row scope 를 정본으로 둔다. |
...
| `completion_decision_schema_version` | 저장한 CompletionDecision schema version. contract 변경 시 snapshot 해석과 migration/backfill 기준으로 쓴다. |
...
| `session_id` | 평가 대상 세션. |
| `rule_set_id` | 사용한 룰셋. |
| `input_schema` | 룰 엔진에 들어간 CommonJudgmentSchema snapshot. snapshot 에 `session_id` 가 있으면 row `session_id` 와 같아야 하며, contract 에서 `session_id` 를 제거하면 row scope 를 정본으로 둔다. |
...
| `result_snapshot` | 전체 RuleEvalResult payload snapshot 정본. verdict, permit_required, required_facilities, legal_basis, ruleset_version 등 split column 은 generated/projection 또는 transition-validated cache 여야 한다. |
...
| `pdf_asset_id` | 생성된 PDF asset. 같은 report session 의 `report_pdf` 또는 `report_image` asset 만 허용하며 service render transition 만 설정한다. |
...
| `floorplan_upload_id` | 샘플 출처가 사용자 업로드 도면인 경우의 upload row. 값이 있으면 `session_id` 가 반드시 있어야 하며 같은 세션 upload 만 허용한다. |
| `asset_id` | 실제 학습에 쓸 도면/마스크 asset. private/session asset 은 `session_id` 가 반드시 있어야 하고 샘플의 `session_id` 와 같은 source scope 여야 한다. catalog sample 은 별도 catalog scope 로 분리한다. |
...
| `evidence_asset_id` | 허가서, 반려 통지 등 증빙 asset. private/session asset 이면 `session_id` 가 반드시 있어야 하고 같은 session asset 만 허용한다. |
