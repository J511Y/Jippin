...
- `leads`: 사용자는 자기 lead summary 조회 가능. admin 은 관리 가능. insert 는 non-anonymous + active profile + required terms 통과 필요.
...
| `kind` | asset 종류. 원본, 썸네일, 마스킹본, overlay, PDF 등을 구분한다. upload 원본, 분석 input/masked/overlay pointer 같은 역할별 FK 는 이 kind 까지 검증한다. |
